...
| Last Price | nominal valuasi (`totalEquity`) lewat `formatUnitPrice` — **tanpa prefix "Rp"**, karena kolom ini menampilkan angka telanjang di semua kelas lain (400.000.000, 4.170) dan prefix hanya pada baris venture membuat kolom tidak sejajar terbaca. Ditambah label kecil "Valuasi Bisnis" (`whitespace-nowrap` — label yang terpecah dua baris membuat baris venture jauh lebih tinggi dari baris lain) |
