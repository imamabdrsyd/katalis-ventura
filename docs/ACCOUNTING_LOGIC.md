...
> **DEFERRED**: deposit/partial payment (field settlement di `TransactionMeta` sudah ada bila perlu),
> appointment/time-slot (tabel `bookings` sudah generik untuk menyusul), channel manager penuh.
> Integrasi lanjutan (AI availability di concierge, Lead→Booking, iCal sync OTA) dibangun terpisah.
