...
## 28. Asset Console (Portofolio Investasi Konsolidasi, migr 125)

Dashboard portofolio investasi (saham, kripto, properti, emas) yang
...
### 28.2 Data model (migrasi 125)

Tidak ada tabel baru. Tiga kolom ditambahkan ke `catalog_items`:

| Kolom | Tipe | Catatan |
|-------|------|---------|
| `asset_class` | TEXT NULL | `'stock'` \| `'crypto'` \| `'property'` \| `'gold'`. **NULL = item katalog biasa**, tidak muncul di Asset Console (opt-in) |
...
| Unit test | `tests/unit/assetConsole.test.ts` (9 test) |
