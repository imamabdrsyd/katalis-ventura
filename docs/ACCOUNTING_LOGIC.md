...
> Terakhir diaudit: 11 Juni 2026 | Terakhir diupdate: 22 September 2026 (**§33.1 — link mengikuti tujuannya**. Menyembunyikan section lewat /admin dulu menyisakan menu navbar yang menunjuk ke jangkar yang sudah tidak dirender (dilaporkan untuk Ecommerce Integration); menu & tombol hero kini ikut hilang, aturannya satu tempat di `landingNav.ts`. Sebelumnya: **§33 — Site CMS, migr 151**. Landing page, metadata SEO, halaman legal, dan indeks blog/market-insights kini disunting platform admin dari `/admin` tanpa deploy. Pola section slot: isi pindah ke DB, layout & animasi tetap di kode. Otoritas baru `is_platform_admin()` SENGAJA dipisah dari `is_business_manager()` — migr 072 membuat superadmin membership-scoped, sementara konten situs bukan milik bisnis mana pun. Halaman publik selalu dirender dari merge(default kode, isi DB) dengan aturan array-diganti-utuh, jadi tidak pernah blank. href dari form admin disaring dua kali (tulis & render) supaya bukan jalur stored XSS. Artikel blog lama tetap di kode dan hidup bersama artikel CMS; `isCodePostSlug()` mencegah tabrakan slug yang gagal diam-diam. Sebelumnya: migr 150 — regresi RLS dari migr 148 ditambah. (**migr 150 — regresi RLS dari migr 148 ditambal**. Policy `Creator can claim own business` menguji kepemilikan lewat subquery ke `businesses`; subquery itu tunduk RLS, dan `businesses_read` menanyakan balik `user_business_roles` → rekursi tak hingga, `POST /api/businesses` gagal 500 alias pembuatan bisnis baru patah. Diperbaiki dengan helper SECURITY DEFINER `is_business_creator()`. Ditemukan lewat uji browser, bukan uji DB — ekspresi policy yang dijalankan sebagai query biasa TIDAK memicu rekursinya. Uji ulang end-to-end: investor baca 4 baris OK / UPDATE 0 baris / INSERT 403 / eskalasi superadmin 403 / panen `businesses` 2 baris bukan 15. Sebelumnya: **§19 Issue #39 — tiga lubang hak akses di lapisan RLS**, temuan audit mandiri lewat Supabase advisor. (a) policy tangkap-semua di `transactions` membatalkan RBAC di sebelahnya sehingga **investor bisa menulis transaksi**; (b) dua policy INSERT di `user_business_roles` hanya menguji `user_id = auth.uid()` sehingga **siapa pun bisa mengangkat diri jadi superadmin di bisnis mana pun** — tak ada trigger penjaga di tabel itu; (c) dua policy SELECT selimut di `businesses` membocorkan seluruh kolom semua bisnis ke tiap user yang login, termasuk `ical_feed_token` & `qris_image_url`. Migr 148/149 menambalnya; daftar `/join-business` pindah ke `GET /api/businesses/discoverable` (admin client, 4 kolom). Migr 146 juga membungkus 72 policy `auth.uid()` → `(select auth.uid())` (lint `auth_rls_initplan` 72 → 0) dan migr 147 memasang `search_path` + mencabut EXECUTE `anon` dari 23 function SECURITY DEFINER. 70 index FK yang disarankan advisor sengaja TIDAK ditambah — tak satu pun dibenarkan pola query nyata. Sebelumnya: **§32 — Galeri Lampiran lintas transaksi**. Semua bukti dokumen satu bisnis kini bisa dilihat sekaligus di `/transactions/attachments`, dengan dua tab Gambar/File; batas lampiran per transaksi naik 3 → 5. Sengaja tanpa menu sidebar baru — pintunya di header halaman Transaksi, modal detail, dan ⌘K. Jenis lampiran ditentukan mime dulu, bukan `resource_type`, karena 41 PDF lama tersimpan sebagai `resource_type: 'image'`. Sebelumnya: **§26.9 — token Instagram mati senyap selama 17 hari**. Long-lived token 60 hari tidak pernah di-refresh; sejak 11 Agustus semua panggilan Graph API gagal sehingga nama lead jatuh ke fallback `@<IGSID>` dan auto-send diam-diam jadi draft, sementara UI tetap menampilkan "Terhubung" hijau. Ditambahkan auto-refresh di jalur webhook, badge status + tombol "Sambungkan ulang", dan route backfill nama. Callback OAuth juga diperbaiki: reconnect dulu menimpa `config` bulat-bulat dan menghapus `ai_tier`. Sebelumnya: **§31.3a — relasi `contact` tidak pernah di-join**. Tidak satu pun query transaksi menyertakan embed `contact`, sehingga `tx.contact` selalu `undefined`: `contactType` di AR/AP selalu null dan `customerPhone` di invoice selalu kosong. Keempat query kini meng-embed `business_contacts`. Kolom "Tipe Kontak" di ekspor juga membedakan "(belum ditautkan)" dari sel kosong — 66% transaksi aktif memang belum punya `contact_id`. Sebelumnya: **§31 — export transaksi, melengkapi import**. Tab "Export" baru di modal transaksi (tombol jadi "Import / Export"). Kolomnya mengikuti data nyata aplikasi — termasuk kontak, nomor transaksi, mata uang asing, dan sheet Jurnal untuk multi-line — dan sengaja TIDAK meniru template Import Lengkap yang bentuknya disederhanakan. Pengambilan data terpaginasi; pratinjau jumlah pakai count murni. Sebelumnya: **§30 — fitur backup data per-bisnis (JSON + Excel)**. Jalur baru untuk mengeluarkan data mentah satu bisnis: 36 tabel ikut, 13 dikecualikan — empat di antaranya tabel kredensial yang tidak boleh ikut ke file unduhan. Manifest deklaratif di `src/lib/backup/manifest.ts` jadi sumber kebenaran tunggal, dan tes menolak tabel baru yang belum diputuskan nasibnya. Murni baca, tanpa migrasi, tanpa sentuhan ke logic akuntansi. Sebelumnya: **§14.8 — deteksi hutang tidak lagi menguji nama akun**. `isPayableTransaction()` jalur double-entry dulu mensyaratkan nama akun kredit cocok `/hutang|utang|payable/i` sementara jalur multi-line cukup menguji `account_type`; akibatnya pinjaman ber-akun "Flexi Cash"/"Credit Card" hanya settleable kalau kebetulan dicatat multi-line. Kedua jalur kini memakai `isAnyPayableAccount()` di `classification.ts` — Issue #38, kambuhnya Issue #22. Sebelumnya: **§27.5 — yang mengunci harga booking adalah adanya jurnal, bukan `payment_status`**. Booking berpendapatan piutang (payout OTA belum cair) berstatus `unpaid` tapi angkanya sudah jadi angka buku; sebelumnya ia ikut auto-quote kalender harga sehingga totalnya bertentangan dengan jurnal, dan tombol "Tandai Lunas" bisa merakit transaksi EARN kedua. `priceLocked` & guard `markBookingPaid` kini memakai `transaction_id`. Sebelumnya: **§27 — nama item katalog di transaksi kini di-resolve saat baca**. `meta.catalog_item.name` tidak lagi diperlakukan sebagai snapshot beku: `resolveCatalogNames()` di `src/lib/api/transactions.ts` menimpanya dengan nama terkini dari `catalog_items` di ketiga jalur baca, sehingga rename item ("Bitcoin" → "BTC") tidak lagi memecah satu item jadi dua nama antara transaksi lama dan baru. Tanpa menulis ulang baris ledger — `id` yang jadi sumber kebenaran. Sebelumnya: **§4.6 & §28.4a diperluas — trade crypto berdenominasi USD + dust setelah jual**. Tiga aturan transaksi multi-line mata uang asing didokumentasikan di §4.6 (`amount` = total debit, baris jurnal tetap IDR, `price_per_unit` ikut currency header). §28.4a dapat paragraf "Konsekuensi saat JUAL": override Unit crypto tidak berkurang sendiri saat jual — `catalog_items.asset_lot_size` wajib di-update manual ke sisa posisi. Migrasi 145 menambah `binance` ke CHECK `transactions_sales_channel_check` (pola migr 103). Sebelumnya: **§29.13 baru — jam & lokasi event + urutan tim dinamis di Lobby** (migr 144). Tiga kolom baru di `event_sessions` (`start_time`/`end_time` TIME, `location` TEXT) mengisi baris kedua header "Players Lobby", MENGGANTIKAN pengulangan `session.title` yang nol-informasi di titik itu; kosong = judul tetap jadi cadangan supaya tinggi header tidak melompat. Per SESI, bukan per tanggal kandidat (satu event = satu tempat & jam; yang divariasikan cuma harinya). Perapihan render terpusat di `src/lib/events/schedule.ts`. Kotak tim kini diurutkan by pendaftaran TERAKHIR (paling baru di atas, tim kosong di bawah) — alasannya sosial: kotak yang ramai harus terlihat lebih dulu, bukan terdampar di bawah lipatan; `PublicEventSlot` ikut membawa `created_at` (stempel waktu, bukan data pribadi). Sebelumnya: **§29.12 — satu kontak = satu slot per tanggal** lewat index unik parsial `event_registrations_contact_per_date_unique` (migr 141), pola sama `event_registrations_slot_unique`. MENGGANTI perilaku lama yang sengaja membolehkan 1 kontak ambil sampai 6 slot per tanggal (pola "kapten tim daftarkan semua rekannya") — sekarang tiap pemain wajib kontak sendiri per tanggal. Lintas tanggal TETAP tidak dibatasi (beda aturan, dikonfirmasi terpisah). Sebelumnya: **§29.11 baru — avatar pendaftar dari galeri tetap** lewat `event_registrations.avatar_key` (migr 140, CHECK constraint ke set tetap) dan `register_event_slot` dapat parameter baru `p_avatar_key` — ditambahkan via DROP FUNCTION eksplisit dulu, bukan CREATE OR REPLACE langsung (Postgres mencocokkan overload dari tipe parameter, jadi CREATE OR REPLACE dgn parameter tambahan menambah overload menggantung, bukan mengganti). Set avatarnya sengaja avatar karakter AXION Agent (Bianca, Sri Mulyani, Stanley, dst) — keputusan eksplisit user, bukan aset baru. Tampil konsisten di 3 tempat (Lobby publik, grid slot admin, picker-nya sendiri) mengikuti pola §29.8. Sebelumnya: **§29.10 baru — label eyebrow "Book Your Spot" bisa diedit** lewat `event_sessions.eyebrow_text` (migr 139), form Event Manager. Kosong = fallback default di komponen, bukan di DB. Bug UI ikut diperbaiki di sesi ini: bubble chat inbox Leads bisa melebar keluar container untuk pesan berisi kata panjang tanpa spasi (URL, dll) — flex item defaultnya `min-width:auto` menolak menyusut di bawah `max-w`; fix-nya `min-w-0` + `[overflow-wrap:anywhere]`, plus `minmax(0,1fr)` di kolom grid panel thread sebagai jaring pengaman satu level di atasnya. Sebelumnya: **§29.8 — override manual warna teks tim (migr 138) + fix kontras gradien**. `readableTextColor()` cukup untuk warna solid tapi salah dipakai di tombol CTA yang memakai `brandGradient()` — titik bawah gradien (82% campur hitam) bisa membalik keputusan yang benar untuk titik atas, dilaporkan user 2× dengan warna brand berbeda sebelum ketemu; perbaikannya `readableTextColorOnGradient()` yang mengambil kontras terlemah dari kedua ujung. `event_sessions.team_text_colors` baru mengizinkan owner memaksa chip nama tim hitam/putih via klik, menimpa kontras otomatis — nilainya dibatasi 'light'|'dark' saja (bukan hex) supaya tak bisa jadi tak terbaca. Sidebar dashboard: `getPosNavItem()` sekarang label/ikonnya "Events"+PartyPopper (bukan "Calendar") untuk jasa sektor creative_agency, sinkron dengan isi hub `/calendar` yang memang Event Manager di sektor itu. Sebelumnya: **§29.9 — Lobby publik jadi drill-down 2 layar + halaman publik dikunci light mode**. Mengetuk tanggal kini mengganti isi layar (header lengket + jalan kembali + banner konfirmasi setelah slot terkunci), bukan menaruh grid slot di bawah daftar tanggal yang di HP berarti responsnya tak terlihat sama sekali. Baris tanggal dipadatkan sampai 4 tanggal + header muat satu layar iPhone 13 — perbandingan antar tanggal itu inti fiturnya dan hilang kalau harus di-scroll. Sebelumnya: **§29.8 — warna brand & warna tim event** lewat migrasi 137: `event_sessions.team_colors` (JSONB, key = nomor tim) plus komponen kanonik `ColorPickerField` (DESIGN_SYSTEM §3.10) yang kini juga menggantikan picker ad-hoc di OmniChannelPageConfig. Seluruh aksen halaman publik event diturunkan dari `business_omni_channels.button_color` — tidak ada indigo AXION yang tersisa di permukaan publik. Bug yang ketahuan saat verifikasi render dan sudah diperbaiki: penentu warna teks sempat memakai ambang luminansi, sehingga chip tim kuning (`#eab308`, luminansi 0,50) dapat teks putih berkontras 1,9:1; sekarang dihitung dari rasio kontras WCAG (`readableTextColor`). Sebelumnya: **§29 baru — modul Event Registration "Book Your Spot"** lewat migrasi 136: pendaftaran event komunitas untuk bisnis jasa sektor `creative_agency` (Telcantik, TELYUFOLK). Manager membuka BEBERAPA tanggal kandidat sekaligus, audience mengunci slot tim × player, dan tanggal yang duluan penuh cuma jadi SINYAL — penutupan tanggal lain baru terjadi saat manager menekan "Jadikan pemenang". **Dampak akuntansi nol**: tidak ada baris `transactions` yang dibuat/diubah dan CoA tidak disentuh (§29.7) — didokumentasikan di sini karena berbagi fondasi RLS `is_business_manager()`, pipeline leads, dan hub `/calendar`. Tiga keputusan yang membedakannya dari draf spec awal: (1) jalur publik lewat API route + service role, BUKAN `supabase.rpc()` dari browser — nol grant anon, `contact_value` mustahil bocor, dan tidak perlu view publik; (2) Lobby memakai polling, bukan Realtime `postgres_changes`, yang payload-nya membawa row lengkap termasuk kontak pendaftar; (3) kedua RPC `SECURITY INVOKER` + pinned `search_path`, sejalan hardening migrasi 102, bukan `SECURITY DEFINER`. Pendaftar otomatis jadi lead + pesan inbox dalam satu transaksi, dengan kontak dinormalkan ke bentuk `wa_id` webhook supaya menyatu ke thread lead yang sudah ada; `last_message_at` DAN `last_inbound_at` sama-sama di-set sehingga lonceng notifikasi bekerja tanpa infrastruktur notifikasi baru. Sebelumnya: **§19 Issue #37 tindak lanjut + §2.4 — akun `1500 Persediaan` disediakan untuk bisnis yang melacak stok** lewat migrasi 135, kena tepat satu bisnis (elvéa). Migrasi tidak mengubah angka apa pun; akun lahir bersaldo nol. Sengaja BUKAN bagian CoA default karena CoA dibuat sebelum diketahui bisnisnya melacak stok atau tidak. Temuan yang mengubah bentuk solusinya: elvéa PRODUSEN — 46 transaksi VAR-nya bahan baku & kemasan, nol tertaut item katalog, jadi harga pokok per produk jadi butuh bill of materials yang belum ada; `cost_price` sengaja dibiarkan 0 dan tidak ditebak. §13.5 menambah **urutan prasyarat**: akun ada → pembelian didebit ke sana → baru isi `cost_price`; salah urutan = beban dobel + persediaan minus, dan teks bantuan field harga pokok kini menyatakan syarat itu eksplisit. Sebelumnya: **§26.8 OLAP & semantic memory dicabut permanen** — tool `run_olap_analytics` + `recall_memory`, `gcpSync.ts`, `semanticMemory.ts`, cron `sync-olap`, route sync/test GCP, tabel `agent_memories` + `olap_*` di `gcpSchema.ts`, dan tombol Sinkronisasi di Settings semuanya dihapus; instance Cloud SQL `axion-agents` sudah dihapus 10 Agt dan keputusan itu kini permanen. Tak ada data hilang — memori agent primary store-nya Supabase, replika `olap_*` turunan `transactions`. `search_knowledge_base` sengaja TETAP (fitur produk tersendiri, inert sampai instance dibangun ulang). Sebelumnya di hari yang sama: **§13.5 & §19 Issue #37 — jembatan stok ↔ ledger: penjualan POS kini menjurnal HPP-nya sendiri**. Checkout kasir selama ini hanya mencatat Dr Kas / Cr Pendapatan lalu mengurangi `catalog_items.stock_qty` lewat RPC — angka kuantitas, bukan entri buku besar — sehingga Persediaan tetap penuh di Neraca dan HPP tak pernah diakui di periode penjualan. Migrasi 134 menambah `catalog_items.cost_price` (standard cost per satuan, default 0, tanpa backfill) sehingga `planCogsPosting()` bisa merakit transaksi VAR TERPISAH Dr HPP / Cr Persediaan bertanggal sama, tertaut lewat `meta.cogs_of_transaction_id` — bukan memutasi jurnal pembelian lama seperti `handleConvertStockToCOGS`, yang memindahkan beban ke periode pembelian. Terpisah, bukan baris tambahan di transaksi penjualan, karena `amount` multi-line = total debit → nominal penjualan akan terbaca sebesar harga jual + harga pokok. Tiga gerbang wajib lolos: bisnis punya akun Persediaan (tanpa itu pembelian sudah dibebankan saat beli → auto-HPP = dobel hitung; kasus nyata elvéa), ada akun beban sah penampung HPP (struktural, tidak jatuh ke sembarang akun beban), dan item `track_stock` ber-`cost_price > 0`. Gagal menjurnal → `cogsWarning` (toast), error tidak dilempar supaya kasir tidak mengulang checkout dan membuat penjualan dobel. Sebelumnya: **§14.9 panel jenis transaksi bisa dikustom per bisnis** — toggle "Tampilkan Lainnya" diganti tombol "Tambah Jenis" (dashed + plus) yang membuka modal picker; jenis pilihan user disimpan di localStorage per bisnis, toggle lama dimigrasi sekali. Sebelumnya di hari yang sama: **§28.1 field kelas aset investasi digerbangi ke bisnis dagang sektor finance** — blok "Kelas Aset Investasi" di `CatalogItemForm` dulu muncul di semua item produk, termasuk katalog F&B/personal care yang tak pernah menjual instrumen; kini lewat `isAssetCatalogBusiness(business_type, business_sector)` dengan escape hatch untuk item yang terlanjur punya `asset_class`. Data real mengonfirmasi hanya bisnis "Ventures" (dagang+finance) yang memakainya. Sebelumnya di hari yang sama: **§19 Issue #36 — card "Business Capital" diselaraskan dengan labelnya**: isinya selama ini `capital_investment + CAPEX`, padahal CAPEX = belanja modal (aset tetap) sementara modal usaha mencakup modal kerja — kas, piutang, dan persediaan. Migrasi 128 yang memperketat definisi CAPEX justru menyingkap mismatch yang sudah ada sejak migrasi 077: Ventures, yang seluruh modalnya berputar di persediaan, tampil Rp 0. Migrasi 132 mengganti sumbernya dengan RPC `get_total_assets_by_business()` → **total aset** (kas + piutang + persediaan + aset tetap neto), meniru `calculateBalanceSheet()` termasuk `journal_lines`, filter posted, `isClosingEntry`, dan penyusutan; `capital_investment` turun jadi fallback saat bisnis belum punya transaksi. Nilai negatif sengaja tidak di-clamp — More&Tea −6,3jt menyingkap belanja yang tercatat tanpa sumber dana. Sebelumnya di hari yang sama: **§14.9 Jenis transaksi pelunasan digabung + panel Journal Entry dwibahasa** — `bayar_hutang`+`cicil_hutang` jadi satu kartu **Bayar Hutang**, dan `terima_kembali_talangan` jadi **Terima Pelunasan** yang kini mencakup piutang usaha DAN talangan (2 tab, default ke tab yang ada isinya). Penuh vs sebagian dipilih lewat dua tombol di setiap baris daftar — dari sisi user kejadiannya sama, yang beda hanya nominal, dan kategori jurnalnya (EARN piutang usaha / FIN talangan) sudah diturunkan otomatis dari akun transaksi asal. Panel jenis transaksi + semua picker dwibahasa lewat `t.journalEntry`; `ENTRY_TYPE_CONFIGS` tinggal menyimpan konfigurasi non-teks. Sebelumnya di hari yang sama: **§14.8 Pelunasan hutang (AP) akhirnya punya UI** — `payableSettlement.ts` sudah lama ada tapi hanya dipakai AR/AP aging & tab "Belum Lunas", jadi hutang tak punya tombol pelunasan sama sekali (piutang punya); `handleSettlePayable`/`handlePartialSettlePayable` di `useTransactions` kini di-export ke halaman transaksi dan `TransactionDetailModal` merender section "Pelunasan Hutang" (Bayar Lunas / Cicil + riwayat pembayaran, total dibayar pakai `getPayableLineAmount` bukan header amount). Section tidak dirender untuk dividend declaration (§14.6) & transaksi piutang agar tak ada dua panel berbagi state konfirmasi. **§14.9 Journal Entry pilih-dulu (list-first)** — `bayar_hutang`/`cicil_hutang` menampilkan daftar hutang outstanding, `terima_kembali_talangan` daftar talangan belum kembali (keduanya lewat RPC `settle_transaction`, bukan `createTransaction`, supaya pelunasan tertaut ke transaksi asal), `penjualan` menampilkan katalog produk/jasa untuk mem-prefill form; daftar kosong → form manual seperti sebelumnya, plus escape hatch "Catat manual". Migrasi 131: hard delete bisnis tidak lagi gagal FK 23503 — trigger `bump_business_transaction_version` skip bump saat cascade penghapusan bisnis.**) | Sebelumnya: 12 Agustus 2026 (**§19 Issue #29 — siklus hidup jejak pelunasan (ACC-M4): `restore_transaction` kini memasang kembali jejak lunas yang dicabut saat hapus (migr 129) — sebelumnya klik "Urungkan" menghidupkan entri pelunasan di buku besar tapi tagihannya tetap tampil belum lunas dan bisa dilunasi dua kali; plus migr 130 memperbaiki cast JSONB di cabang partial migrasi 108 yang membuat penghapusan cicilan gagal total dengan HTTP 500. §19 Issue #28 — batch Medium/Low #1 dari audit 11 Juni: ACC-M3 (closing entry kini difilter di seluruh jalur Income Statement lewat helper `isClosingEntry()`, bukan cuma Neraca), ACC-M9 (dashboard meneruskan `accounts` + `reportDate` ke `calculateBalanceSheet` sehingga depresiasi ikut terhitung dan angkanya berhenti bertentangan dengan /balance-sheet), ACC-M12 (migrasi 128 — RPC `get_capex_by_business` tidak lagi menghitung persediaan & piutang sebagai belanja modal; `calculateTotalCapex` yang sudah jadi dead code dihapus), ACC-L1 (`groupTransactionsByMonth` sortir pakai key `YYYY-MM`, bukan nama bulan terlokalisasi).**) | Sebelumnya: 4 Agustus 2026 (**§28.4b Riwayat transaksi venture kini TERISI dari buku besar bisnis target: `getVentureSnapshots()` mengembalikan `events` — mutasi akun `linked_stock_account_id` (kredit=Setor Modal, debit=Tarik Modal) + debit akun `is_dividend` milik pemilik itu, aturan bacanya disalin persis dari `calculateCapTable`/`accumulateDividendsByOwner` supaya daftar transaksi tidak bisa bercerita lain dari KPI Total Modal & Dividen Diterima di atasnya. Transaksi yang tidak menyentuh kedua akun itu tidak diikutkan (menampilkan seluruh buku besar bisnis lain = menyalin laporan orang lain ke halaman instrumen). Kolom Kuantitas & P/L Terealisasi disembunyikan utk venture (persen kepemilikan saat sebuah setoran terjadi tidak bisa diturunkan dari transaksi itu sendiri). Kustodian venture = `Self Custody · {nama pemilik}` — stake dipegang langsung, bukan dititipkan ke broker. Kolom Avg/Last Price di tabel utama tidak lagi digabung `colSpan={2}`: Last Price = nominal valuasi (via `formatUnitPrice`, tanpa prefix "Rp" — kolom ini angka telanjang di semua kelas lain) + label 1 baris, Avg Price = RUMUS asalnya dalam chip netral ("Aset − Liabilitas") supaya valuasi tidak salah dibaca sebagai harga pasar. `formatOwnershipPct()` menyembunyikan desimal nol (100% bukan 100,00%). Garis antar-baris disamakan dgn tabel Transaksi (gray-200/gray-700). Kolom Last Price venture kini "valuasi per 1%" (totalEquity/100), bukan valuasi penuh, supaya identitas Kepemilikan × Harga = Nilai Pasar berlaku di semua baris; kolom Kepemilikan saham menaruh LEMBAR (satuan harga) di baris utama & Lot di baris kedua -- harga dikutip per lembar, kalau Lot di atas pembaca meleset 100×. Header: "Laba/Rugi (P/L)" (label "(Rp/unit)" pada Harga Terakhir sempat dicoba lalu dilepas lagi — kolomnya jadi terlalu lebar dan mendesak chevron di ujung baris).**) | Sebelumnya: 3 Agustus 2026 (**§28.4b Asset Console — kelas aset baru `venture`: kepemilikan owner di bisnis LAIN yang juga ada di AXION (mis. posisi Imam di "Hillside Studio"). Satu-satunya kelas yang tidak membaca transaksi bisnis pemantau: Unit = `%` kepemilikan dari `calculateCapTable()` bisnis target, Total Modal = `contributed` akun EQUITY `is_stock` user di sana, Nilai Pasar = `%` × `calculateBalanceSheet().equity.totalEquity` (valuasi Fase 1 = nilai buku) — fungsi yang sama persis dengan halaman Cap Table/SCE/Neraca bisnis itu, jadi tidak mungkin beda. Migr 126 menambah `catalog_items.linked_business_id` + `linked_stock_account_id` (CHECK dua arah: venture wajib punya keduanya, kelas lain dilarang; UNIQUE partial cegah tautan ganda yang menggandakan KPI). Menautkan **BUKAN lewat form Katalog** (venture bukan barang jualan: tak ada SKU/stok/harga jual) tapi lewat tombol "Hubungkan Venture" di halaman Asset Console → 2 dropdown berurutan (Bisnis → Akun Ekuitas, dicari via flag `is_stock` bukan nama akun); baris venture disembunyikan dari `getCatalogItems()` (`.or(asset_class.is.null,asset_class.neq.venture)` — `.neq()` saja membuang item biasa karena `NULL <> 'venture'` = NULL) + `is_active=false`. `hasLivePrice` menggantikan gate `lastPrice > 0` karena total ekuitas SAH bernilai nol/negatif (bisnis rugi melebihi modal) — gate lama menyembunyikan justru kasus yang paling perlu dilihat. `lastPrice` venture = valuasi per 1% sehingga identitas `kuantitas × harga = nilai pasar` tetap berlaku tanpa cabang khusus. RLS otomatis membatasi akses lintas bisnis; tautan yang tak terbaca ditandai `unresolved`, bukan Rp0. Lihat §28.4b. Sebelumnya 2 Agustus 2026 (**§28.4a Asset Console — crypto: Unit di-canon dari Katalog** — 1 BTC bernilai ratusan juta–miliar rupiah, kepemilikan riil selalu pecahan mikro (mis. 0,00302599 BTC); mengagregasi `unit_breakdown` transaksi seperti kelas lain rawan salah (kasus nyata: transaksi eksperimen "beli 3 Coin @1jt" bikin kartu Unit tampil "3" yang tidak masuk akal). Untuk `asset_class='crypto'`, `catalog_items.asset_lot_size` sekarang dipakai sebagai ANGKA TOTAL unit yang dipegang (diisi manual, field "Jumlah yang Dipegang" — beda makna dari stock yang tetap rasio lembar-per-lot), bukan hasil SUM transaksi; `totalCostBasis`/`realizedPl` tetap dari transaksi. Breakdown per-custodian menyembunyikan kolom Unit/Avg Price untuk crypto (data per-broker belum akurat). Kolom Balance di tabel konsolidasi di-rename jadi "Unit" + tampilkan nama satuan dari Katalog (`quantityUnitLabel()` — "Lot" utk stock, unit Katalog apa adanya utk kelas lain). Lihat §28.4a. Sebelumnya sehari yang sama (**§28 Asset Console (migr 125)** — portofolio investasi konsolidasi lintas broker sebagai **read-model di atas `transactions`**, bukan ledger paralel (4 tabel `asset_*` yang sempat diusulkan ditolak: posisi ADALAH buku besar, jadi drift tak mungkin terjadi). `catalog_items` dapat 3 kolom: `asset_class` (opt-in; NULL = item biasa), `asset_lot_size` (jembatan lot↔lembar, saham IDX=100), `asset_price_updated_at`. Average cost per (instrumen × kustodian), kustodian = `transactions.name`. Kuantitas jual diturunkan eksak dari `cost_basis_dilepas / avg_cost_saat_itu` bila `unit_breakdown` kosong. Klasifikasi beli/jual/dividen via account_type + flag (`is_cash_equivalent`/`is_trade_receivable`), bukan nama akun. Backfill meta-only menautkan transaksi JUAL legacy ke item katalog (tanpanya posisi terbaca bruto: BBCA 7 lot vs realita 1 lot); `deriveCatalogItemFromStock()` mencegah berulang + memperbaiki bug lama `sold_stock_ids` hilang saat pindah ke mode multi-line. Nav gated `business_sector='finance'`, sengaja BUKAN `business_type='dagang'`. Sebelumnya 23 Juli 2026: **i18n modal kalender + bug-hunt bundle Calendar↔Service↔Omnichannel (Issue #35)** — (A) SEMUA modal kalender kini dwibahasa (`t.calendar` prefix `bm/hp/um/ic/re*`): BookingModal (label state/status/channel via helper lokal, ganti konstanta ID `BOOKING_*_LABELS`), HoldingPanel, UnitManagerModal, IcalSyncModal, RateEditorPanel. (B) **Bug-hunt bundle pasca migr 124 — 4 fix:** (1) `BookingModal` auto-quote booking belum-lunas masih pakai `unit.rate_item_id`+`quoteStay` flat (deprecated) → salah harga weekday/weekend & tak jalan utk unit tanpa rate_item; kini terima prop `baseRates` + `quoteStayV2` (weekday/weekend/monthly). (2) `markBookingPaid` (`bookings.ts`) resolve akun pendapatan dari `unit.rate_item` deprecated → kini `resolveUnitRevenueAccountId()` query item main-service unit (weekday→weekend→any) lalu fallback 4100. (3) `CatalogPanel` catch handler menampilkan "SKU dipakai" utk SEMUA unique-violation, padahal migr 124 tambah index `idx_catalog_items_unit_rate_kind` (1 item weekday/weekend/monthly per unit) → kini bedakan via `msg.includes('unit_rate_kind')` → `errorRateKindTaken`. (4) Verifikasi: unit tanpa base rate (Kosanku/elvéa, 0 item main) degradasi mulus di grid/publik/booking-inquiry (total 0, tak crash). Known-limitation: field "Harga Default" di `OmniChannelPricing` = fallback bila unit tak punya base (bisnis akomodasi pakai base item; tak diubah). Sebelumnya sehari yang sama (**Base price akomodasi per-unit dari item layanan weekday/weekend/monthly (migr 124)** — model harga kalender dirombak: (1) `catalog_items` dapat `unit_id` (scope layanan per unit fisik), `service_role` ('main'=sewa kamar menyetir harga | 'addon'=mis. Cleaning), `rate_kind` ('weekday' Sen–Jum | 'weekend' Sab+Min | 'monthly' >27 malam). Backfill Hillside: Weekday 350k/Weekend 450k/Monthly 4.4jt→main, Cleaning→addon, semua ke Unit 1026. (2) Resolver baru di `src/lib/rates.ts`: `UnitBaseRates` + `baseRateForDate` (Sab+Min=weekend, fallback weekday) + `resolveNightPriceV2` (override>base by hari) + `quoteStayV2` (>27 malam=monthly) + `buildUnitBaseRates`; 11 test baru. (3) **Unit = level teratas**: `CalendarUnitContext` (provider di HubPage) berbagi unit aktif antara tab Kalender & Services — switch/add unit di `UnitManagerModal` (kini switch/add saja, dropdown sumber-harga & URL iCal DIHAPUS dari sini). Services scoped per-unit (`catalog_items.unit_id`); `CatalogItemForm` dapat pemilih main/addon + weekday/weekend/monthly (bisnis akomodasi). (4) URL impor iCal OTA dipindah ke `IcalSyncModal` ("Connect to another website") bareng URL ekspor — impor+ekspor sekarang di satu tempat. (5) Halaman publik `/[slug]`: harga per tanggal diekspansi dari base weekday/weekend + override → pricing_rules (first-match-wins), `monthly_price` diteruskan; widget `computeRangePricing` pakai harga bulanan bila rentang >27 malam. `booking-inquiry` & AI concierge (`leadAssistant` RateInfo weekday/weekend/monthly) ikut resolver baru. `business_units.rate_item_id` deprecated (tak dipakai, tak dihapus). Sebelumnya 19 Juli 2026 (**Chip "Penjualan" di Riwayat Stok (migr 123) + form katalog gaya Material + tab tipe by business_type** — (1) `metadata.source` di audit_log: RPC `decrement_catalog_stock`/`increment_catalog_stock` men-set GUC transaksi-lokal `app.audit_source` (`pos_sale`/`stock_add`) via `set_config(..., is_local=true)` dan `log_audit_trail()` menyalinnya ke metadata (`NULLIF(current_setting(...), '')`); jalur form edit → NULL. `StockLogPanel` menampilkan chip netral abu "Penjualan"/"Sale" bila `source='pos_sale'`. Backfill 2 baris pengurangan lama sbg pos_sale (diverifikasi keduanya checkout POS). (2) `CatalogItemForm` dirombak ke gaya Material underline (FloatingField/FloatingSelect + textarea peer-float custom; harga pakai label prop CurrencyInputWithCalculator, grid items-end): nama, harga+satuan, SKU, jumlah stok (trailing satuan), akun pendapatan, deskripsi, link CTA. String `revenueAccountPlaceholder` dihapus (FloatingSelect pakai label sbg placeholder, opsi kosong). (3) **Selector tipe item disembunyikan bila tipe bisnis hanya mengizinkan satu**: `businessType` prop — 'jasa'→service saja, 'produk'/'dagang'→product saja, null→keduanya; default `item_type` mengikuti; item lama bertipe di luar aturan tetap diakomodasi. (4) Harga list katalog makin rapat ke ikon aksi saat hover (`group-hover:-mr-1`→`-mr-2`). Lihat Section 27.) Sebelumnya sehari yang sama (**Fix `audit_log.metadata.business_id` NULL untuk tabel pasca-migr 004 (migr 122)** — `log_audit_trail()` meng-HARDCODE daftar tabel saat mengambil business_id (hanya transactions/businesses/accounts/investor_metrics); tabel yang lahir belakangan — `catalog_items` (migr 099), `bookings`, `business_units` — jatuh tanpa cabang → `metadata.business_id` selalu NULL. Terungkap saat panel Riwayat Stok tampil "belum ada perubahan" padahal 32 baris audit `catalog_items` ada (10 di antaranya perubahan stok elvéa). Migr 122 (sudah di-apply): ambil business_id GENERIK dari snapshot `to_jsonb(NEW/OLD)->>'business_id'` (khusus tabel `businesses` pakai `id` sendiri), plus backfill baris lama dari snapshot yang sudah tersimpan. Tabel tanpa kolom business_id tetap NULL (tidak regresi). Juga polish `AiKnowledgePanel`: pensil edit pindah ke baris deskripsi & chevron dimepetkan pojok (sebelumnya tumpang-tindih di header), animasi collapse pakai trik `grid-rows-[1fr↔0fr]` + opacity 300ms (tinggi konten bisa ditransisikan tanpa max-height tebakan). Lihat Section 27.) Sebelumnya sehari yang sama (**Riwayat Stok + Concierge collapsible** — panel baru `StockLogPanel` di aside kanan hub POS (di bawah Concierge Agent, hanya `variant='pos'`): daftar perubahan stok — delta +hijau/−merah, `dari → ke`, pelaku, waktu. TANPA tabel log baru: `getStockLogs()` membaca `audit_trail_with_users` (trigger audit `catalog_items` aktif sejak migr 099) sehingga riwayat lama ikut terbaca & semua sumber tercakup (tambah stok manual, checkout POS, koreksi form). Baris non-stok difilter client-side (PostgREST tak bisa bandingkan 2 kolom JSONB) — ambil `limit×10` lalu dipotong. Refresh reaktif: `CatalogPanel.onStockChanged` → `HubPage` naikkan `refreshKey`. `AiKnowledgePanel` kini **collapsible** (judul jadi tombol + chevron, preferensi di `localStorage` `katalis_ai_knowledge_collapsed`) agar riwayat stok lebih terlihat. Juga: harga list katalog didekatkan ke ikon aksi saat hover (`-mr-3` → `-mr-1`, slot aksi `6.75rem` → `6.25rem`). Lihat Section 27.) Sebelumnya sehari yang sama (**Uniqueness katalog pindah dari NAMA ke SKU (migr 121)** — keputusan produk user: nama item TIDAK wajib unik karena varian sah bernama sama ("Castor Oil" ukuran L & S, harga normal vs reseller); yang unik hanya **SKU** sbg identitas item untuk matching import & stok. Migr 121 (sudah di-apply): DROP `idx_catalog_items_unique_name`, CREATE `idx_catalog_items_unique_sku` — UNIQUE `(business_id, LOWER(btrim(sku)))` partial `WHERE deleted_at IS NULL AND sku IS NOT NULL AND btrim(sku) <> ''` sehingga SKU tetap opsional (banyak item tanpa SKU tak bentrok). Diverifikasi 0 SKU duplikat sebelum apply. Sisi app: validasi nama unik + prop `existingNames` + string i18n `errorNameTaken` DIHAPUS (CatalogItemForm/CatalogPanel), toast konflik unique kini menunjuk `errorSkuTaken`. UI: SKU dipindah jadi **chip mono di KIRI nama** (grid & list) — sebelumnya di baris bawah bareng stok; helper `renderStockSku` dipecah jadi `renderSku` + `renderStock`. Lihat Section 27.) Sebelumnya 18 Juli 2026 (**Stok & SKU di halaman Katalog (migr 120)** — form item produk (`CatalogItemForm`) kini punya field SKU (unik per bisnis, cek client-side via `existingSkus`) + toggle "Lacak stok" (`track_stock`) + input jumlah stok (edit di form = koreksi manual); kartu grid & list `CatalogPanel` menampilkan baris sisa stok ("Stok: N unit", merah "Stok habis" bila 0) + SKU mono; aksi cepat **"Tambah Stok"** (ikon PackagePlus di hover, manager-only) buka modal jumlah barang masuk → RPC baru `increment_catalog_stock(p_item_id, p_qty)` (SECURITY DEFINER, atomik, guard `is_business_manager` — lebih ketat dari decrement yang cukup member karena dipanggil alur checkout; qty ≤ 0 → exception, guard gagal → NULL → error ramah). SKU & stok HANYA untuk `item_type='product'` — submit form membersihkan keduanya untuk jasa. String i18n baru blok `t.catalog` (id/en). Jembatan qty ↔ nilai persediaan ledger (auto-HPP POS) masih DEFERRED. Lihat Section 27.) Sebelumnya 16 Juli 2026 (**Katalog ↔ transaksi stok: chip di kolom deskripsi + icon lampiran + backfill migr 119** — Transaksi VAR→stok (debit akun persediaan, badge STOCK) kini bisa di-link ke `catalog_items` lewat picker di form: `QuickTransactionForm` saat `isInventoryAccount(selectedAccount)`, `TransactionForm` saat debit = akun persediaan; link disimpan di `meta.catalog_item {id,name}` (field existing dari picker EARN). `CatalogItemPicker` dapat prop `allowCreate` — buat item baru inline dari teks pencarian (`product`, harga 0). Utk stok, pick TIDAK mengisi amount/akun (harga beli ≠ `default_price` jual). Kolom Description `TransactionList` kini merender chip indigo nama katalog + icon `FileText` bila ada `meta.attachments`/`attachment` (murni visual). Migr 119 backfill: nama bersih = deskripsi dipotong di kurung buka pertama ("Yamaha NMAX (dibayar dari …)" → "Yamaha NMAX"); 11 item terbentuk, 50/50 transaksi stok lama ter-link; trigger user `transactions` off sementara (jaga `updated_by`, hindari banjir audit_log). Plus filter pseudo-kategori **STOCK** di dropdown kategori list (server-side `!inner` join debit_account mirror `isInventoryAccount()`; tab Unsettled client-side); badge STOCK di list/detail modal kini pakai `isStockTransaction()` bersama (hapus copy lokal 13xx/nama — ekuivalen di data real 52=52). Lihat Section 27.) Sebelumnya 13 Juli 2026 (**BookingModal: mode view + fix underline "Tamu (org)" + pemisah ribuan harga** — (1) `BookingModal` kini buka di **mode VIEW read-only** dulu (baris `ViewRow` label+nilai; tanggal diformat `dd MMM yyyy`, harga `formatCurrency`), tombol footer "Edit" (ikon Pencil) untuk masuk mode edit yg menampilkan form (FloatingField/Select) + "Simpan perubahan"; `isEditing` reset false tiap modal dibuka; blok OTA eksternal tetap view (tak ada tombol Edit). Aksi "Terima pembayaran"/Batalkan/Hapus tetap tampil di view (aksi, bukan field). (2) Field "Tamu" & "Tamu (org)" disamakan strukturnya (label atas + `input-underline`, grid `items-end`) supaya underline sejajar — sebelumnya "Tamu (org)" pakai FloatingField (`pt-5`) jadi lebih rendah. (3) Field Harga/malam mode edit kini `type=text inputMode=numeric` dgn tampilan `formatNumber` (pemisah ribuan `146.667`), simpan angka mentah via `replace(/\D/g,'')`. Sebelumnya 11 Juli 2026 (**Fix SCE tidak tie-out ke Neraca — tarik modal akun `is_stock` dihitung dua kali (Issue #34)** — data real Hillside Studio: transaksi `Dr Modal Papah (is_stock) / Cr Bank, Rp 2jt` ("Tarik modal renov") membuat halaman Changes in Equity warning "Does not match Balance Sheet" (selisih tepat Rp 2jt) padahal neraca sendiri balance. Sebab: `calculateBalanceSheet` melempar SEMUA debit ekuitas ke `totalEquityDebit` → kurangi retained earnings; sementara SCE cap table juga mengurangkan debit akun stock dari kolom modal → tarik modal dikurangi DUA kali di `sceClosingEquity`. Fix: `calculateBalanceSheet` memecah debit ekuitas jadi `totalStockDebit` (akun `is_stock` → kurangi CAPITAL, `netEquityMovements = totalEquityCredit − totalStockDebit`) vs `totalNonStockEquityDebit` (akun `is_dividend`/equity biasa → kurangi RE). `equity.capital` kini net. `totalEquity` identik (hanya pindah garis) → neraca tetap balance & SCE tie-out ✓. `netIncome` SCE jadi murni laba operasi (entitlement dividen lebih akurat). Efek tampilan: Retained Earnings naik sebesar tarik modal, owner capital turun, total tetap. Regression test di `calculateBalanceSheet.test.ts` & `calculateStatementOfChangesInEquity.test.ts`. Section 6.3/6.4/23.4 + Issue #34. Sebelumnya 10 Juli 2026 (**Kalender: chip Paid dark-mode invert + tombol Kelola unit di tab Services** — (1) chip Paid di dark mode di-INVERT (terang + teks gelap, hover makin putih) — `dark:bg-gray-800` tenggelam di background gelap; konsisten dgn circle hari-ini. (2) Komponen baru `UnitManagerButton` (mandiri, load unit+catalog sendiri) tampil di header tab Layanan/Services hub kalender — unit↔rate plan bisa dikelola tanpa pindah tab; guard manager+akomodasi. Sebelumnya 07 Juli 2026 (**Kalender: warna grid ala Airbnb (Paid chip hitam, harga hijau)** — penerapan warna di grid diubah (LEGENDA/dot TIDAK berubah): chip/bar booking **Paid** kini **hitam pekat + teks putih** (`BOOKING_BAR_CLASSES.paid`, ala kalender host Airbnb), dan indikator hijau **pindah ke NOMINAL harga** di sel tanggal — hanya untuk malam yg di-cover booking LUNAS (`paidPriceByDate` = filter `payment_status==='paid'`; Confirmed/Inquiry tak mewarnai harga, chip-nya yg bawa warna indigo/kuning). Circle tanggal hari-ini: indigo → **hitam** (`bg-gray-900`, dark: invert putih) karena indigo kini dipakai utk chip Confirmed (booking berjalan yg terkonfirmasi). `bookedDates` (blokir set-harga di tanggal terisi) tetap semua booking aktif. OTA block dibiarkan abu-abu dashed. Sebelumnya sehari yang sama (**Kalender: i18n parsial + tab Services + label Inquiry + swap tombol** — (1) Halaman kalender yg TERLIHAT (KPI strip, board/toolbar/legenda, unit toolbar, guard/banner) kini dwibahasa via blok i18n baru `t.calendar` (id/en) — modal (BookingModal/HoldingPanel/UnitManager/IcalSync/RateEditor) SENGAJA belum di-i18n (scope parsial, keputusan user "yang terlihat saja dulu"). (2) Di hub jasa `/calendar` tab "Katalog" di-brand **"Layanan"/"Services"** (`th.tabServices`); POS produk/dagang tetap "Katalog". (3) Urutan tab di-swap: **Kalender di kiri**, Katalog/Layanan di kanan (POS tetap Katalog kiri). (4) Indikator kuning `tentative` → label **"Inquiry"/"Pertanyaan"** (booking dari klik "cek ketersediaan" omnichannel, channel 'website'); status DB `tentative` tak berubah, hanya labelnya. (5) Tombol toolbar di-swap: "Perlu tindak lanjut" di kiri, "Kelola unit" di kanan. (6) Legenda "350rb harga khusus (override)" → "harga dasar (base price)". Sebelumnya sehari yang sama (**KPI kalender count-up + hapus indikator "Selesai"** — (1) `CalendarKpiStrip` kini pakai `AnimatedNumber` (count-up) yg re-animasi tiap ganti bulan (`replayKey`=kursor bulan), selaras KPI dashboard & sliding periode PNL; angka mentah dianimasikan, format per-jenis (currency/`%`/int). (2) Indikator/display-state `completed` ("Selesai", abu-abu) DIHAPUS dari legenda (`CalendarBoard`), dropdown status (`BookingModal` `EDITABLE_STATUSES`), dan `bookingStatus.ts` (`BookingDisplayState`) — dulu menutupi bahwa booking sudah dibayar (dicek sebelum `payment_status`), padahal "Lunas" (dibayar) lebih informatif dari "Selesai" (tamu check-out). Status DB `completed` tetap valid (tak ada migrasi; 0 baris memakainya) tapi booking `completed` kini render by payment/konfirmasinya. Sebelumnya sehari yang sama (**Fix sisa pembulatan "Pendapatan kamar"** — `calculateBookingMetrics` menghitung `roomRevenue` dari **prorata `total_amount` asli** (`total_amount × malamTerkliping / totalMalam`), bukan `malam × price_per_night`; `price_per_night` disimpan sebagai `round(total_amount/nights)` sehingga dikali balik memunculkan sisa (mis. sewa bulanan 4.4jt/30 malam → 30×146.667 = 4.400.010). Booking penuh-dalam-bulan kini menyumbang `total_amount` eksak, lintas bulan dibagi proporsional. Test baru di `calculateBookingMetrics.test.ts`. Juga hapus teks hint "Klik tanggal kosong…" di atas grid kalender (`CalendarLauncher`). Sebelumnya 05 Juli 2026 (**Kalender = read-model, booking mengalir dari transaksi & omnichannel (migr 118, Issue #33)** — keputusan produk: kalender adalah tampilan owner/investor, BUKAN pintu input booking. Tombol "+ Booking" & klik-buat di grid DIHAPUS (grid booking read-only; klik hanya aktif di mode Harga). Booking masuk dari 2 sumber: (1) transaksi EARN yang di-flag lewat tombol "Masukkan ke kalender" (`AddToCalendarButton` di `TransactionDetailModal`) → `createBookingFromTransaction` bikin booking LUNAS; karena transaksi tak punya tanggal → PENAMPUNGAN (`check_in/check_out` NULL, migr 118) di panel "Perlu tindak lanjut" (`HoldingPanel`) sampai owner isi tanggal; data di-agregat: nama, harga, channel; (2) widget omnichannel `/[slug]` saat calon tamu klik CTA → `POST /api/public/booking-inquiry` bikin booking TENTATIF channel 'website' (tanggal dari widget; follow-up di WhatsApp). Migr 118: `check_in/check_out` nullable + CHECK berpasangan + exclusion di-guard `check_in IS NOT NULL` + channel 'website'. `BookingModal` kini EDIT-ONLY (harga/total/channel terkunci utk booking lunas, tanggal tetap bisa dikoreksi); Lead→Booking (button+modal di `/leads`) dihapus. Data lama: 59 booking estimasi Hillside → penampungan (tanggal di-NULL-kan). Bonus: item katalog yg dipilih di Quick Entry EARN kini chip `meta.catalog_item` (bukan digabung ke deskripsi). Section 27.5 diperbarui, Issue #33. Sebelumnya sehari yang sama (**Fix model kalender: `business_units` menggantikan `catalog_items` sbg unit fisik (migr 117, Issue #32)** — koreksi kesalahan model: catalog item (rate plan/layanan) sebelumnya diperlakukan sbg unit fisik yg dibooking, menyebabkan proteksi double-booking & occupancy salah hitung. Tabel baru `business_units` (properti fisik; `rate_item_id` per-unit); `bookings.unit_id` & `unit_daily_rates.unit_id` menggantikan `catalog_item_id`; `catalog_items.is_bookable_unit`/`ical_import_url` dan `businesses.calendar_rate_item_id` dihapus. Backfill 1 unit default/bisnis akomodasi + link seluruh booking existing. UI: pemilih unit (>1 unit aktif) via `<Tabs>`, board/KPI/rate/BookingModal di-scope per unit; `UnitManagerModal` baru; `BookingModal` tak lagi punya dropdown unit (unit = konteks kalender yg dibuka). AI concierge & halaman publik ikut disesuaikan. Exclusion constraint dikecualikan jg utk `date_estimated=true` (2 booking legacy overlap krn tanggal tebakan, bukan bukti multi-unit). Section 27.5/27.6 ditulis ulang, Issue #32 (Section 19). Sebelumnya sehari yang sama (**Kalender Harga / pricing calendar (migr 116)** — Section 27.6 baru: kalender booking akomodasi kini juga katalog harga per tanggal (mode Harga, pola Airbnb). `businesses.calendar_rate_item_id` = item sumber harga dasar; `unit_daily_rates` = override per tanggal (hapus = kembali default); resolusi override>default di lib murni `src/lib/rates.ts` (`quoteStay` Σ per malam + breakdown, `groupIntoRanges`, `listDatesInRange` + filter hari). UI: harga di tiap sel grid, seleksi rentang → `RateEditorPanel` (input harga + filter hari Jum/Sab utk pola weekend + Reset ke default). 4 konsumen satu sumber kebenaran: BookingModal create auto-total Σ per malam (edit manual = keluar mode otomatis; `total_amount`=Σ eksak masuk ledger), AI concierge quote harga per tanggal (blok prompt baru, override 60 hari), halaman publik `/[slug]` (override dikonversi ke `pricing_rules` widget + default/price_unit dari item sumber — menang atas setting manual omni-channel), dan grid kalender. 9 unit test `tests/unit/rates.test.ts`. Sebelumnya sehari yang sama (**Quick Add tampilkan akun piutang** — `getQuickAddAccounts()` tidak lagi mengecualikan akun piutang/receivable/talangan/advance (dead code `isReceivableOrAdvanceAccount` dihapus); user bisa memilih akun piutang langsung di Quick Entry (counter-account Kas/Bank tetap auto). Akun Hutang Dividen (`is_dividend_payable`) tetap dikecualikan; akun ekuitas prive/dividen owner (mis. 3300/3400) memang tampil karena bukan liabilitas hutang dividen. Section 12.3 diperbarui. Sebelumnya 03 Juli 2026 (**Bug-hunt ronde 2 (Issue #31)** — (1) PRIVASI: hapus riwayat sesi agent kini juga menghapus copy GCP (`deleteSessionMemories()` — ringkasan sesi + embedding) supaya percakapan terhapus tidak muncul lagi via `recall_memory`; (2) POS checkout set `sales_channel: 'offline'`; (3) `skipped` bulk-post kini hanya menghitung draft yang gagal `isPostableDraft` (bukan id non-draft/not-found); (4) banner "Tarik ke kalender" disamakan kriterianya dgn `reconcileStayTransactions` (nights ATAU check_in, exclude settlement); (5) guard SSRF `isSafeFeedUrl()` di sync iCal (wajib https, tolak host lokal/privat); dead code `getUpcomingBookings` dihapus. Lihat Issue #31. Sebelumnya sehari yang sama (**Hardening kalender (Issue #30, migr 115)** — (1) flag `catalog_items.is_bookable_unit` (default TRUE, checkbox form Katalog khusus sektor akomodasi): rate plan/add-on (data real: "Cleaning Service"/"Monthly Rent" Hillside) tidak lagi dihitung sebagai unit di occupancy/RevPAR & dropdown BookingModal — klasifikasi eksplisit via flag, bukan keyword; difilter di CalendarLauncher, `/leads`, backfill. (2) Exclusion constraint `bookings_no_double_booking` (btree_gist, per `catalog_item_id` + `daterange &&`, partial non-eksternal/cancelled/deleted) — backstop DB untuk race yang lolos cek overlap client; 23P01 → pesan ramah di `createBooking`/`updateBooking`. (3) `markBookingPaid` dimitigasi: re-fetch state booking dulu (tolak yang sudah lunas/terhapus) + kompensasi soft-delete transaksi bila penautan gagal (cegah EARN dobel saat retry). Section 27.5 + Issue #30. Sebelumnya sehari yang sama (**Bug-hunt batch fix (Issue #29)** — (1) guard `isPostableDraft()` kini sadar multi-line: row multi-line WAJIB akun header NULL (constraint `transactions_account_rules` migr 036), guard lama memblokir SEMUA draft jurnal multi-line dari posting; kini `is_multi_line=true` cukup `amount>0`, kedua route posting ikut select `is_multi_line`, regression test `tests/unit/isPostableDraft.test.ts`. (2) Save Draft + stok terjual: konversi Persediaan→HPP yang ditunda kini dijalankan saat draft diposting — `convertPendingSoldStock()` di `useTransactions` (single & bulk, sebelum status flip; hanya stok yang masih debit ASSET → idempoten); sebelumnya penjualan bisa diposting tanpa HPP & persediaan tetap di neraca. (3) BookingModal "Tandai Lunas" kini menyimpan nilai form dulu (`onUpdate` return `Booking`) lalu `markBookingPaid(saved)` — sebelumnya ledger mencatat snapshot booking lama & edit hilang diam-diam; guard unit/tanggal/bentrok/total>0. Section 4.3, 13.0, 17.1, 27.5 + Issue #29. Sebelumnya 02 Juli 2026 (**"Save Draft" di Quick Entry + guard posting draft belum lengkap** — tombol "Cancel" di `QuickTransactionForm` diganti "Save Draft" (batal/tutup form kini via tombol X modal): simpan apa yang sudah diisi tanpa memaksa field mandatory (akun boleh kosong), batas bawah tetap `amount>0`; akun kosong → draft single-sided `is_double_entry=false` (kategori placeholder OPEX, nama fallback). Guard baru `isPostableDraft()` (`src/lib/api/server/postableDraft.ts`): draft belum lengkap TIDAK bisa di-post — `/[id]/post` tolak, `/bulk-post` skip (kembalikan `skipped`, toast peringatan). Section 4.3 + 17.1 diperbarui. Sebelumnya 01 Juli 2026 (**Calendar: rekonsiliasi revenue + gating akomodasi (migr 114)** — kalender booking dibatasi **sektor akomodasi** (`isAccommodationSector`, jasa lain lihat placeholder); revenue menginap yang sudah di ledger di-wire ke kalender lewat `reconcileStayTransactions()`/endpoint `/api/calendar/backfill` (link 2-arah `booking.transaction_id`↔`transaction.meta.booking_id`, presisi bila ada `meta.check_in/out`, perkiraan bila hanya `meta.nights` → check-in=tanggal transaksi + `date_estimated` migr 114, ring amber+`~` di kalender, edit→clear; lewati settlement; unit auto hanya bila 1 unit); banner "Tarik ke kalender" bila `countUnlinkedStayTransactions>0`. Section 27.5 diperbarui. Sebelumnya sehari yang sama (**Calendar Booking (migr 113)** — Section 27.5 baru: sistem booking menginap di tab Kalender hub `/calendar` (bisnis jasa akomodasi); tabel `bookings` (unit=`catalog_items`, `nights` GENERATED, `status`/`payment_status`/`channel`, `is_external` blok impor iCal OTA, `transaction_id` link EARN UNIQUE); saat **ditandai Lunas** → `markBookingPaid()` merakit transaksi **EARN multi-line langsung lunas** persis pola `useCashier.checkout()` (resolver akun di-share via `src/lib/accounting/salesCheckout.ts`: Tunai→1100/QRIS→1200, Cr `revenue_account_id`/fallback 4100), `meta.unit_breakdown={price_per_unit:harga/malam, quantity:nights, unit:'malam'}` sbg jembatan "harga rata-rata/malam"; proteksi double-booking `findOverlappingBookings` (rentang beririsan); metrik hospitality `calculateBookingMetrics` (ADR/Occupancy/RevPAR/revenuePerBooking, kliping per bulan) di `CalendarKpiStrip`; month grid `CalendarBoard` (span multi-malam, warna per state `src/lib/bookingStatus.ts`); migr 113 juga tambah `catalog_items.ical_import_url`+`businesses.ical_feed_token` (disiapkan utk iCal sync). Sebelumnya 30 Juni 2026 (**POS Cashier (migr 112)** — Section 27.4 baru: kasir full-screen di tab Cashier hub `/point-of-sales`; checkout merakit transaksi EARN multi-line langsung lunas (`status: 'posted'`) via `createMultiLineTransaction` — kredit pendapatan di-grup per `revenue_account_id`, debit Kas/Bank otomatis by kode (Tunai→1100, QRIS→1200, pola Quick Entry); migr 112 tambah `catalog_items.track_stock`+`stock_qty` (stok opt-in), `businesses.qris_image_url`, RPC `decrement_catalog_stock` (SECURITY DEFINER); customer di-wire ke kelola kontak via nama (`saveContactFromTransaction` tipe `customer`); pembayaran Tunai (kembalian) / QRIS statis upload Cloudinary — bukan payment gateway; MVP tanpa PPN. Sebelumnya 26 Juni 2026 (**Fix DB drift: crash `column a.is_trade_payable does not exist` saat settlement (migr 110)**) — fungsi RPC `settle_transaction` yang TER-DEPLOY di database ditemukan sudah regress (versi lama/rusak, TIDAK cocok dengan repo migr 107): mereferensikan kolom `accounts.is_trade_payable` yang tidak ada (nama benar `is_operating_payable`, migr 085) sehingga SEMUA pelunasan (full & partial) gagal; juga kehilangan pengecekan otorisasi role `business_manager` (regresi keamanan), tidak menulis `meta.settlement_of_transaction_id`, serta logika FX gain/loss & multi-line outstanding. Migr 110 me-`CREATE OR REPLACE` `settle_transaction` PERSIS seperti migr 107 (sumber kebenaran repo) → DB kembali sinkron, crash hilang, otorisasi & FX/multi-line pulih. Lihat Section 19 / Issue #28. Sebelumnya 21 Juni 2026 (**Fix Legacy Partial Settlements missing remaining_amount**) — transaksi legacy partial settlement yang kehilangan `remaining_amount` di `meta` kini memiliki mekanisme fallback otomatis. `getOutstandingAmount` (client) dan `v_outstanding` (server RPC `settle_transaction`) akan menghitung sisa tagihan secara real-time dari total `partial_settlements` jika field `remaining_amount` absen, mencegah overstate settlement pada full settlement. Sebelumnya 12 Juni 2026 (**Batch fix audit 2026-06-11 (Issue #27)** — semua temuan High + ACC-M1/M11 dari `docs/AUDIT_2026-06-11.md` diperbaiki: retur penjualan & reimbursement & pelunasan akrual kini benar di IS (`calculateFinancialSummary`, `extractIncomeStatementLineItems`, `groupTransactionsByMonth` — ACC-H3/H4/H5/H6/M1/M11); konvensi bulan depresiasi disamakan full-month (Neraca = IS, ACC-H7, Section 16.3); payable settlement & AP aging pakai net baris hutang via `getPayableLineAmount`/`getPayableOutstandingAmount` (ACC-H1, Section 22.2); pemilihan akun piutang settlement pakai `isAnyReceivableAccount` bukan ASSET pertama (ACC-H2); migr 102 hardening (SEC-H1/L1/L2: multi-line RPC kembali SECURITY INVOKER + search_path, helper RLS di-pin, cache write manager-only); period-lock + whitelist field di `/api/sync/push` (SEC-M1); allowlist skema URL omni-channel + `safeLinkUrl()` anti stored-XSS (SEC-M2). Lihat Section 19 / Issue #27. Sebelumnya 11 Juni: **Fix settlement piutang multi-line sisi server (Issue #26, migr 100)** — audit menyeluruh 11 Juni 2026 (temuan ACC-C1) menemukan fix Issue #26 sebelumnya hanya di client: RPC server `settle_transaction` (migr 079) masih hitung outstanding dari header `amount` (gross), sementara client kirim net → sync-check menolak pelunasan piutang multi-line (net≠gross), atau over-book di gross bila outstanding NULL. Migr 100 me-`CREATE OR REPLACE` `settle_transaction` (body identik migr 079 kecuali blok `v_outstanding`): multi-line → `SUM(debit-credit)` baris akun piutang dari `journal_lines` (replika `isAnyReceivableAccount()` — SQL↔TS wajib sinkron), partial → `remaining_amount`, single double-entry → header amount; tetap SECURITY INVOKER + search_path. Laporan audit lengkap: `docs/AUDIT_2026-06-11.md`. Section 19 / Issue #26 diperbarui. Sebelumnya 10 Juni: **Katalog Produk/Jasa + Sales Channel field** — Section 27 baru: migr 099 tabel `catalog_items` (master data produk/jasa per bisnis, harga saja, `revenue_account_id` + `sku` disiapkan untuk fase matching); halaman `/catalog` (manager-only, CRUD); `CatalogItemPicker` 2 mode — `single` di Quick Entry (muncul saat akun REVENUE dipilih, isi amount+nama+akun) & `multi` di MultiLineJournalForm (tombol "Tambah dari Katalog" saat EARN → keranjang qty → generate baris Dr penampung kosong + Cr per item ke akun pendapatannya); soft-delete via `deleted_at`; RLS pola `business_contacts`. Juga: field `sales_channel` di `transactions` (migr 097/098, 13 channel: tiktok/tokopedia/shopee/lazada/blibli/airbnb/booking_com/traveloka/instagram/whatsapp/website/offline/other) dengan badge logo di list & detail modal; importer agent auto-set channel; kolom CASH FLOW di list transaksi kini tampilkan akun kas multi-line (akrual non-kas → akun debit terbesar) seperti double-entry. Sebelumnya 09 Juni: **AXION Agent — channel TikTok Shop / Tokopedia + instruksi NL** — Section 26.7 baru: importer CSV ekspor pesanan Seller Center gabungan TikTok+Tokopedia (`tiktokTokopediaParser.ts`); 1 transaksi per order (multi-SKU digabung, rincian di `meta.line_items`), jurnal 2-baris Dr Kas/Bank · Cr Pendapatan = Σ SKU Subtotal After Discount; pendapatan subtotal-net saja (ongkir disubsidi platform → audit di meta, bukan revenue); tanggal Paid Time; filter hanya status Selesai; idempoten via `meta.order_id`; parser tangani BOM/trailing-TAB/quoted-multiline; `resolveMarketplaceAccounts()` (4100 + Kas/Bank); UI channel digabung jadi satu opsi "TikTok Shop / Tokopedia". **Field instruksi tambahan opsional** (`instructionInterpreter.ts`, provider Gemini Vertex + fallback rule-based) menerjemahkan NL → config impor (status posted/draft, **debitMode bank/receivable** = Dr Piutang Usaha bila dana belum cair, channel/date filter, bank hint) — hanya mengatur perilaku, TIDAK mengubah angka. Sebelumnya 06 Juni: **AXION Agent interaktif tanya nominal** — Section 26.6 baru + 26.4/26.5 diperbarui: kalau user ketik transaksi tanpa nominal (mis. "bayar listrik"), AI tidak lagi error tapi balik tanya "Berapa nominalnya?"; `extractTransactionFromText()` kini return 3 status (`complete`/`needs_amount`/`null`), `PARSE_SYSTEM_PROMPT` izinkan `amount:null`; web simpan `pendingTx` + Telegram simpan `pending_transaction{_type:'needs_amount'}`, jawaban nominal berikutnya digabung + bawa `category_hint`; sebelumnya 05 Juni: **AXION Agent ke Telegram** — Section 26.5 diperbarui (tersambung): command `/tanya` analitik (reuse buildFinancialContext + provider chain, non-streaming, toTelegramMarkdown konversi bold); input transaksi Telegram upgrade ke `extractTransactionFromText()` shared helper (`src/lib/ai/parseTransaction.ts`, Gemini→Groq→regex) yang dipakai web & Telegram; fix import file chat `[object Object]` (validateFile return object); sebelumnya: **AXION Agent multi-provider** — Section 26.1b baru: provider abstraction `src/lib/ai/provider.ts` dengan chain Gemini→Groq→rule-based; `generateText`/`streamText` normalisasi format Gemini native vs Groq OpenAI-compatible; system prompt dipusatkan di `src/lib/ai/prompts.ts` (ACCOUNTING_DOMAIN reusable, nuansa klasifikasi VAR/CAPEX/FIN ditingkatkan); UI tampilkan badge model aktif via header X-AI-Model; env `GROQ_API_KEY` ditambah; sebelumnya 05 Juni: **AXION Agent Opsi B (aksi tulis)** — Section 26.4 baru: (1) catat transaksi via natural language di chat (`/api/ai/parse-transaction`, Gemini extract + smartResolveTransaction, fallback parseTransactionMessage), preview DraftCard → simpan; (2) impor XLS/CSV via lampiran chat (parse client-side reuse engine import, ImportPreviewCard → createTransactionsBulk); (3) Smart Import AI-assist di `/transactions` (`/api/ai/smart-import-assist` klasifikasi batch baris low-confidence, badge "AI"); semua fitur AI=enhancement dgn fallback rule-based; sebelumnya 04 Juni: **AXION Agent (AI Assistant)** — Section 26 baru: chatbot keuangan read-only via FAB kanan-bawah (gantikan FAB Quick Entry), `POST /api/ai/chat` streaming Gemini 2.5 Flash Lite, `buildFinancialContext()` di `src/lib/ai/financialContext.ts` inject ringkasan P&L per-bulan + neraca (transaksi mentah TIDAK dikirim ke AI); `computeSummary()` mereplikasi pipeline useIncomeStatement (calculateFinancialSummary → applyDepreciationToSummary) supaya netProfit ikut kurangi depresiasi & match Income Statement; 8 unit test regression di `aiFinancialContext.test.ts`; Telegram belum tersambung tapi context builder sudah reusable; sebelumnya: **OCR Struk → Gemini multimodal** — `scanReceipt()` kini pakai Gemini `gemini-2.5-flash` sebagai provider utama: gambar struk → JSON `OcrParsed` terstruktur langsung (vendor/total/date/category/line_items/charges) tanpa regex; Vision/OCR.space + `parseReceipt()` tetap sebagai fallback bila quota habis/error/API key kosong; `geminiOcr.ts` + `parseGeminiJson()` baru, enrich keyword via fungsi rule-based `parser.ts` agar matcher CoA tetap jalan; `OcrProvider` tambah `'gemini'`; cache `ocr_scan_cache` re-parse per-provider; Section 25.2 diperbarui; sebelumnya: **Fix outstanding piutang multi-line ambil gross bukan net (Issue #26)** — `getOutstandingAmount` kini hitung net debit baris akun receivable via helper baru `getReceivableLineAmount`, bukan header `amount` (gross); settlement OTA tidak lagi overstate kas & overclear piutang; Section 14.2b + Issue #26 ditambahkan; koreksi data historis Hillside Studio (4 settlement gross→net + 1 transaksi pajak Cr Piutang→Cr Bank); **Fix tombol pelunasan hilang setelah edit transaksi multi-line (Issue #25)** — detector `isReceivableTransaction`/`isTradeReceivableTransaction`/`isPayableTransaction` mengecek cabang `is_double_entry` lebih dulu, padahal transaksi multi-line punya `is_double_entry=TRUE` + `debit_account`/`credit_account` NULL (akun ada di `journal_lines`); urutan cabang dibalik → cek `is_multi_line` dulu; Section 14.2 + Issue #25 ditambahkan; regression test `tests/unit/settlementDetection.test.ts`; sebelumnya: **Fix capital double-count di Neraca (Issue #24)** — legacy branch `calculateBalanceSheet` hanya menyuntik `capital_investment` bila ekuitas belum dibukukan dari double-entry/multi-line (`capitalAlreadyBooked`), mencegah aset & ekuitas overstated sebesar modal awal saat data legacy bercampur dengan transaksi "Modal Investasi Awal" otomatis; Section 6.1B + Issue #24 ditambahkan; sebelumnya: **Promosi double-entry → multi-line** — migr 095: `update_multi_line_transaction` kini mengizinkan upgrade transaksi double-entry sederhana menjadi multi-line via tombol "Tambah Baris" di Edit Transaction (set is_multi_line=TRUE + kosongkan debit/credit_account_id); Section 3.2 diperbarui; **AccountDropdown** kini render via portal (createPortal) agar tidak terpotong overflow modal; sebelumnya: **Statement of Changes in Equity (SCE) + Profit-Sharing & Rekonsiliasi Dividen** — migr 094 tambah kolom `profit_share_pct`, `owner_stock_account_id`, `contact_id` di `accounts`; fungsi `calculateStatementOfChangesInEquity`, hook + halaman `/statement-of-changes-in-equity`, export PDF/Excel; header "Equity (Capital)" di Neraca kini clickable ke SCE; Section 23 baru + Section 6.3 diperbarui; menjawab temuan audit MEDIUM "Tidak Ada SOCE"; sebelumnya: **Retained Earnings = auto-calculate; jurnal penutup manual DIHAPUS** — halaman `/closing-entry`, lib `closingEntry.ts`, dan link nav dihapus karena bertabrakan dengan model auto-calculate dan merusak presentasi ekuitas neraca; `calculateBalanceSheet` kini mem-filter `meta.entry_type.id === 'closing_entry'` sebagai pengaman data historis; Period Lock tetap sebagai soft-close; Section 6.4 & Issue #23 diperbarui; sebelumnya: **Template Jurnal Multi-Baris** — migr 093 kolom `journal_lines` JSONB di `transaction_templates`; "Simpan sebagai Template" & "Gunakan Template" kini tersedia di mode multi-baris Journal Entry; memuat template multi-baris mengganti semua baris (tetap editable); Section 12 diperbarui; sebelumnya: **Bank Statement Import — Phase B** — Section 25 dilengkapi dengan CSV/XLSX parser (Indonesian + English number/date format detection, dua format kolom: Debit+Kredit terpisah atau Mutasi+Type), side-by-side matching UI di `/reconciliation` dengan mode toggle Saldo vs Cocokkan Mutasi, hook `useBankTransactions`, API `/api/bank-transactions` + `/match` + `/unmatch` (auto un-reconcile transaksi ledger kalau tidak ada bank line lain ter-link); sebelumnya: **Bank Statement Import & OCR** — migr 092 + Section 25 — upload mutasi PDF/image BCA, pipeline OCR `runOcr()`, dedup hash UNIQUE(account_id, dedup_hash), 2 tabel `bank_statement_imports` + `bank_transactions`; **Invoice dari Transaksi Piutang** — migr 086 + Section 24; audit fix flag `is_trade_receivable` & `is_operating_payable` — migr 085; depresiasi fix; multi-currency — migr 079))))))))))))
...
**`1500 Persediaan` — provisioning ON DEMAND, sengaja bukan bagian CoA default.** CoA default tidak punya akun persediaan karena dibuat saat setup bisnis, sebelum diketahui apakah bisnisnya akan melacak stok — memberikannya ke semua bisnis (termasuk jasa) hanya mengotori CoA. Migrasi `135_provision_inventory_account.sql` membuatnya hanya untuk bisnis yang **terbukti** melacak stok (punya `catalog_items.track_stock`) tapi belum punya akun ASSET yang terbaca sebagai persediaan (kriteria identik `resolveInventoryAccount`, §13.5). Bisnis baru yang mulai melacak stok belakangan membuat akunnya lewat CoA seperti biasa; form katalog memberi tahu bila belum ada (lihat §13.5).
...
**Transaksi multi-line berdenominasi asing** (mis. trade crypto di exchange
yang di-settle dalam USDT) punya tiga aturan yang mudah keliru:

1. `amount` = **total sisi debit** (diisi route `PATCH /api/transactions/[id]/multi-line`
   dari `journal_lines`), bukan hasil `original_amount × fx_rate`. Angka FX
   header hanya dinormalisasi (`normalizeCurrencyFields`), tidak menimpa
   `amount` — jadi selisih pembulatan rupiah antar-baris tidak pernah membuat
   jurnal jadi tak balance.
2. `journal_lines` ditulis dalam **mata uang fungsional (IDR)**; field FX
   per-baris cuma mirror (ditulis, belum pernah dibaca UI mana pun). Kurs asli
   tiap kaki ditaruh di `journal_lines.description` supaya jejaknya tetap ada.
3. `meta.unit_breakdown.price_per_unit` dirender `TransactionDetailModal`
   memakai `transaction.currency_code`. Untuk transaksi USD, harga per unit
   WAJIB diisi dalam USD (mis. 77.500 USDT/BTC), bukan rupiah — kalau tidak,
   angka rupiah akan tampil berlabel `US$`.
...
> - **Debit akun `is_stock`** (tarik modal / pengembalian setoran owner) → mengurangi **CAPITAL**
>   (`netEquityMovements`), sama seperti cap table `net = credit − debit`.
> - **Debit akun `is_dividend` + equity biasa** (prive/dividen, distribusi laba) → mengurangi
>   **RETAINED EARNINGS** (Section 6.4).
>
> Sebelumnya SEMUA debit ekuitas masuk `totalEquityDebit` dan mengurangi retained earnings,
> sementara SCE cap table juga mengurangkan debit akun stock dari kolom modal → tarik modal akun
> stock **dihitung dua kali** di SCE (`sceClosingEquity` lebih rendah dari equity neraca) sehingga
> tie-out gagal. Lihat Section 19. **`totalEquity` nilainya tidak berubah** oleh split ini —
> tarik modal hanya pindah dari garis RE ke garis Capital.

`equity.capital` yang dilaporkan = **net** (`totalEquityCredit − totalStockDebit`) supaya header
"Paid-in Capital" di neraca = Σ baris per-pemilik dari cap table (yang juga net).
...
> **Tarik modal (debit akun `is_stock`) TIDAK mengurangi retained earnings** — itu pengembalian
...
### 13.5 Auto-HPP saat Checkout POS — Jembatan Stok ke Ledger (migr 134)

**Masalah yang ditutup.** Checkout kasir POS mencatat sisi penjualan (Dr Kas / Cr
Pendapatan) lalu mengurangi `catalog_items.stock_qty` lewat RPC
`decrement_catalog_stock`. Pengurangan itu murni **kuantitas** — nilainya tidak pernah
dilepas dari akun Persediaan. Untuk bisnis yang mengkapitalisasi pembelian stok ke akun
ASSET, akibatnya dua-duanya salah arah: Persediaan di Neraca tetap penuh padahal barang
sudah keluar (aset overstated), dan HPP tak pernah diakui di periode penjualan (Laba Kotor
overstated).

Jembatan yang ada sebelumnya hanya `handleConvertStockToCOGS` (§13.0), yang **memutasi
`debit_account_id` transaksi pembelian lama** dari Persediaan ke HPP. Itu memindahkan beban
ke *periode pembelian*, bukan periode penjualan — melanggar matching principle dan mengubah
jurnal historis.

**Mekanisme.** `planCogsPosting()` (`src/lib/accounting/salesCheckout.ts`) merakit rencana
jurnal dari keranjang; `useCashier.checkout()` menyimpannya sebagai **transaksi VAR
tersendiri** bertanggal sama dengan penjualan:

```
Dr  HPP          (cost_price x qty, dijumlah seluruh item)
  Cr  Persediaan
```

Sumber angkanya `catalog_items.cost_price` — **standard cost** per satuan, input manual
(bukan moving average / FIFO; costing berbasis riwayat pembelian = fase berikutnya).

**Kenapa transaksi terpisah, bukan baris tambahan di transaksi penjualan.** `amount`
transaksi multi-line diisi server sebagai **total debit** (`app/api/transactions/route.ts`).
Menumpuk baris HPP di transaksi penjualan yang sama memang tetap seimbang dan tetap benar di
semua engine laporan (multi-line diturunkan dari `journal_lines`, bukan dari `category`) —
tapi `amount`-nya jadi harga jual + harga pokok, sehingga nominal penjualan terbaca salah di
mana pun `amount` ditampilkan.

**Tiga gerbang, semuanya harus lolos.** `planCogsPosting` mengembalikan `null` (= tidak
menjurnal apa-apa, dan itu jalur normal bukan kegagalan) bila:

| Gerbang | Alasan |
|---------|--------|
| Bisnis punya akun **Persediaan** (`resolveInventoryAccount`) | **Yang terpenting.** Tanpa akun Persediaan, pembelian stok sudah dibebankan langsung ke HPP saat beli — menjurnal HPP lagi saat jual = **dobel hitung**. Auto-HPP hanya benar untuk bisnis yang mengkapitalisasi pembelian ke aset. |
| Ada akun beban sah penampung HPP (`resolveCogsAccount`) | Resolusi **struktural**: `income_statement_section === 'cost_of_revenue'`, lalu `default_category === 'VAR'`, nama akun hanya cadangan terakhir. Sengaja **tidak** jatuh ke sembarang akun beban — HPP yang mendarat di akun OPEX diam-diam merusak Laba Kotor. |
| Item `track_stock = true` **dan** `cost_price > 0` | Opt-in per item. Default `cost_price` = 0 → fitur mati sampai pemilik mengisinya. |

**Efek di laporan** (transaksi VAR double-entry, Dr EXPENSE / Cr ASSET):
- **Income Statement**: `calculateFinancialSummary` — debit ke EXPENSE dengan seksi
  `cost_of_revenue` → `totalVar`. Laba Kotor akhirnya memotong harga pokok.
- **Balance Sheet**: kredit ke ASSET → `totalAssets -= amt` dan `classifyAsset(-amt)` →
  `totalInventory` turun.
- **Cash Flow**: tidak menyentuh akun kas → **tidak** muncul di arus kas mana pun, sesuai
  sifatnya sebagai jurnal non-kas.

**Penautan & kegagalan.** Entry HPP membawa `meta.cogs_of_transaction_id` (menunjuk
transaksi penjualan) + `meta.cogs_items[]` (rincian per item). Dibuat **sesudah** penjualan
supaya penautannya menunjuk transaksi yang sudah pasti ada — urutan sebaliknya berisiko
meninggalkan beban HPP yatim bila penyimpanan penjualan gagal. Bila entry HPP gagal,
`checkout()` mengembalikan `cogsWarning` dan kasir melihat toast peringatan; error
**tidak dilempar**, karena melempar setelah penjualan tersimpan membuat kasir mengira
checkout gagal lalu mengulang — dan itu menghasilkan penjualan dobel.

**Tidak ada backfill.** Migrasi 134 hanya menambah kolom dengan default 0; buku yang sudah
berjalan tidak berubah sama sekali.

**Urutan prasyarat — penting, salah urutan justru merusak.** Gerbang akun Persediaan bekerja
sebagai proksi untuk "pembelian dikapitalisasi". Begitu akun itu ada, proksinya bisa bohong:
akun boleh ada sementara pembelian masih dibebankan langsung ke HPP. Kalau `cost_price` diisi
dalam keadaan itu, tiap penjualan menambah HPP yang sudah diakui saat beli (**dobel hitung**)
dan mengkredit persediaan bersaldo nol (**persediaan minus**). Karena itu urutannya wajib:

1. Akun Persediaan ada (migrasi 135 atau dibuat sendiri lewat CoA),
2. **pembelian stok berikutnya didebit ke akun itu**, baru
3. `cost_price` per item diisi.

Teks bantuan field harga pokok di `CatalogItemForm` menyatakan syarat (2) secara eksplisit —
sebelumnya ia hanya menjelaskan apa yang akan dijurnal, bukan apa yang harus benar lebih dulu.
Saldo persediaan negatif sengaja **tidak** di-clamp, konsisten dengan §19 Issue #36: angka
minus menyingkap urutan yang terlanjur salah, menyembunyikannya tidak.
...
### 14.8 Payable Settlement (Pelunasan Hutang)

File: `src/lib/accounting/guidance/payableSettlement.ts`

Mirror sisi AP dari receivable settlement. Sejak 13 Agustus 2026 modul ini terpasang di UI (sebelumnya hanya dipakai AR/AP aging & tab "Belum Lunas", sehingga hutang tidak punya tombol pelunasan sama sekali).

**Deteksi** — `isPayableTransaction(transaction)`, cek multi-line dulu (alasan sama seperti §14.2):
```
1. Jika is_multi_line && journal_lines ada:
     return ada baris credit_amount > 0 yang akunnya isAnyPayableAccount()

2. Else jika is_double_entry:
     return isAnyPayableAccount(credit_account)

3. Else → FALSE
```

Kedua jalur memakai classifier yang sama, `isAnyPayableAccount()` di
`src/lib/accounting/classification.ts` — mirror dari `isAnyReceivableAccount()` di sisi
piutang. Classifier itu **sengaja tidak menguji nama akun**: sebuah liabilitas bisa dilunasi
karena `account_type`-nya LIABILITY, bukan karena kebetulan dinamai "hutang".

Sampai 27 Agustus 2026 jalur double-entry menambahkan tes nama `/hutang|utang|payable/i`
sementara jalur multi-line tidak, sehingga pinjaman ber-akun "Flexi Cash" atau "Credit Card"
hanya terdeteksi bila kebetulan dicatat multi-line — lihat Issue #38.

**Outstanding** — `getPayableOutstandingAmount()` memakai `getPayableLineAmount()`: net **credit** pada baris akun LIABILITY saja, bukan header `amount`. Penting untuk multi-line seperti pencairan pinjaman `Dr Bank 1.960.000 + Dr Fee 40.000 / Cr Hutang 2.000.000` — yang dilunasi adalah 2.000.000 (baris hutang), bukan angka lain.

**Settlement entry** — `buildPayableSettlementPrefill()`:
```
Original (pencatatan hutang):
  Dr Aset/Beban  / Cr Hutang (LIABILITY)

Settlement (pelunasan):
  Dr Hutang (LIABILITY) / Cr Kas/Bank
  category: FIN
  status: posted
  meta.settlement_of_transaction_id = original.id
```
...
### 14.9 Journal Entry — pilih-dulu (list-first)

File: `app/(dashboard)/transactions/journal-entry/page.tsx`, `src/components/transactions/OutstandingSettlementPicker.tsx`, `src/components/transactions/CatalogQuickPicker.tsx`

Beberapa jenis transaksi di Journal Entry sebenarnya bukan entry baru, melainkan tindak lanjut atas data yang sudah ada. Mengetik ulang akun + nominal di form mentah membuat pelunasan **tidak pernah tertaut** ke transaksi asalnya (meta kosong → tagihan tetap tampil belum lunas). Karena itu jenis-jenis berikut menampilkan daftar lebih dulu:

| Jenis transaksi | Daftar yang ditampilkan | Aksi |
|-----------------|-------------------------|------|
...
Tab default di sisi piutang jatuh ke tab yang ada isinya, supaya user tidak mendarat di daftar kosong padahal tab sebelah penuh.
...
Khusus `tarik_dividen`, di atas daftar juga tampil **panel Hak Bagi Hasil** (`ShareholderEntitlementPanel`): porsi tiap pemilik, nominal hak, yang sudah ditarik, dan sisa yang masih boleh ditarik untuk tahun berjalan. Angkanya bukan hitungan baru — memanggil `calculateStatementOfChangesInEquity()` yang sama persis dengan halaman SCE (§23), jadi kedua halaman tidak mungkin bercerita beda. Tujuannya menutup celah kepatuhan: porsi sudah disepakati di `accounts.profit_share_pct`, tapi tanpa panel ini penarikan dilakukan berdasarkan ingatan dan kelebihannya baru ketahuan di SCE setelah uangnya keluar. Klik baris pemilik → form terisi (akun debit = akun dividen pemilik itu lewat `owner_stock_account_id`, nominal = sisa haknya). Baris yang sudah kelebihan tarik ditandai merah, dan bila laba tahun berjalan ≤ 0 muncul peringatan bahwa penarikan menggerus modal, bukan membagi laba.
...
**Panel jenis transaksi bisa dikustom per bisnis (13 Agt 2026).** Lima jenis di `DEFAULT_VISIBLE_ORDER` selalu tampil; sisanya dulu bersembunyi di balik toggle **"Tampilkan Lainnya"** yang sekadar buka-tutup semuanya sekaligus. Toggle itu diganti tombol **"Tambah Jenis"** (dashed + ikon plus, gaya sama dengan tombol "Buat Invoice") yang membuka modal berisi jenis-jenis sisanya. Klik kartu = jenis itu ikut tampil di panel; klik lagi = dilepas. Efeknya langsung (tanpa tombol simpan) dan disimpan di `localStorage` per bisnis (`katalis_journal_pinned_entry_types_{businessId}`) — kebutuhan tiap bisnis beda: trading butuh talangan, F&B butuh akrual. Toggle lama (`katalis_journal_entry_types_expanded`) dibaca sekali untuk migrasi: yang sebelumnya expand langsung mendapat semua jenis ter-pin. Melepas jenis yang sedang dipilih mengembalikan pilihan ke jenis pertama (`penjualan`); **menambah** jenis sengaja tidak ikut memilihnya supaya form yang sedang diisi tidak ter-reset. Ini menuntaskan TODO "make default visible types user-configurable per business".
...
| #34 | **SCE tidak tie-out ke Neraca — tarik modal akun `is_stock` dihitung dua kali** (11 Jul 2026, data real Hillside Studio): transaksi `Dr Modal Papah (is_stock) / Cr Bank, Rp 2jt` ("Tarik modal renov"). `calculateBalanceSheet` melempar SEMUA debit ekuitas ke `totalEquityDebit` → mengurangi retained earnings; sementara SCE cap table (`accumulateStockMovements`) juga mengurangkan debit akun stock dari kolom modal pemilik. Karena `sceClosingEquity = totalClosingCapital + retainedClosing` (RE dibaca dari neraca yang sudah dikurangi 2jt), tarik modal dikurangi **dua kali** → SCE lebih rendah Rp 2jt dari equity neraca → warning "Does not match Balance Sheet" (padahal neraca sendiri balance). | RESOLVED | `calculateBalanceSheet` memecah debit ekuitas: debit akun `is_stock` → `totalStockDebit` (kurangi CAPITAL, `netEquityMovements = totalEquityCredit − totalStockDebit`); debit `is_dividend`/equity biasa → `totalNonStockEquityDebit` (kurangi RE). `equity.capital` kini net. `totalEquity` nilainya identik (hanya pindah garis) → neraca tetap balance & SCE tie-out. `netIncome` SCE kini murni laba operasi (tak tercemar tarik modal → entitlement dividen lebih akurat). Efek tampilan: Retained Earnings naik sebesar tarik modal (mis. −29.970.383 → −27.970.383), owner capital turun; total tetap. Section 6.3/6.4/23.4. Regression test di `calculateBalanceSheet.test.ts` & `calculateStatementOfChangesInEquity.test.ts` |
| #35 | **Bug-hunt bundle Calendar↔Service↔Omnichannel pasca migr 124** (23 Jul 2026, data real: Hillside punya base weekday/weekend; Kosanku & elvéa punya unit tapi 0 item main): 3 tempat masih memakai `business_units.rate_item_id`/`unit.rate_item` yang DEPRECATED sejak migr 124 → harga salah / config diabaikan. (1) `BookingModal` auto-quote booking belum-lunas: `rateQuoteActive = !!unit.rate_item_id` + `quoteStay(default_price flat)` → utk unit tanpa rate_item auto-quote mati; utk yang punya (Unit 1026 masih menunjuk item weekday lama) → semua malam dihitung 350k, weekend 450k diabaikan. (2) `markBookingPaid` pilih akun pendapatan dari `unit.rate_item.revenue_account_id` → item weekday host dg akun pendapatan kustom diabaikan (jatuh ke 4100). (3) `CatalogPanel` catch semua unique-violation → toast "SKU dipakai", padahal migr 124 menambah index `idx_catalog_items_unit_rate_kind` (dobel weekday/weekend/monthly per unit) → pesan menyesatkan. | RESOLVED | (1) `BookingModal` terima prop `baseRates` (dari `CalendarLauncher`) + pakai `quoteStayV2` (weekday/weekend by hari + monthly >27 mlm). (2) helper `resolveUnitRevenueAccountId(unitId)` di `bookings.ts`: query item `service_role='main'` unit (weekday→weekend→any) yg punya `revenue_account_id`, fallback default 4100. (3) handler bedakan `msg.includes('unit_rate_kind')` → `tc.errorRateKindTaken`, selain itu SKU. Verifikasi: unit tanpa base (Kosanku/elvéa) degradasi mulus (grid sembunyikan harga, publik skip blok, booking-inquiry total 0 tanpa crash) — 3+3 unit test resolver/widget. `unit.rate_item` tak lagi jadi dependensi fungsional di mana pun. |
...
| #36 | **Card "Business Capital" salah label, bukan salah hitung** (13 Agt 2026, laporan user data real Ventures): sejak migr 077 card diisi `capital_investment + CAPEX` — CAPEX = belanja modal (aset tetap), jauh lebih sempit dari "modal usaha" yang mencakup modal kerja (kas, piutang, **persediaan**). Migr 128 memperketat definisi CAPEX dengan benar dan justru menyingkap mismatch-nya: bisnis yang seluruh modalnya berputar di persediaan tampil **Rp 0** (Ventures 432,1jt → 0; isinya 407jt persediaan + 25,5jt piutang) | RESOLVED | Migr 132: RPC baru `get_total_assets_by_business()` → **total aset** (kas + piutang + persediaan + aset tetap neto), meniru `calculateBalanceSheet()` termasuk `journal_lines`, filter posted, `isClosingEntry`, & penyusutan. `capital_investment` turun jadi fallback (hanya bila bisnis belum punya transaksi) — modal awal sudah dibukukan otomatis, senada Issue #24. Nilai negatif tidak di-clamp: More&Tea −6,3jt menyingkap belanja tercatat tanpa sumber dana. Lihat detail di bawah |
| #38 | **Deteksi hutang masih keyword-based di satu jalur — kambuhnya Issue #22** (27 Agt 2026, laporan user data real Ventures): `isPayableTransaction()` jalur multi-line hanya menguji `account_type === 'LIABILITY'`, tapi jalur double-entry sederhana menambah tes nama `/hutang|utang|payable/i`. Pinjaman Jenius Flexi Cash yang dicatat 2 baris (`Dr Bank / Cr 2100 Flexi Cash`) gagal tes nama → hilang dari picker "Pilih hutang yang mau dibayar", padahal pencairan sejenis yang dicatat 3 baris (Crisis 1/2, PaCCko) muncul normal. Migrasi 085 (Issue #22) memasang flag `is_operating_payable` tapi tidak menjangkau detektor settlement ini | RESOLVED | `isAnyPayableAccount()` baru di `classification.ts` (mirror `isAnyReceivableAccount()`), dipakai kedua jalur `isPayableTransaction()` + `getPayableAccountId()`. Regression test `tests/unit/isPayableTransaction.test.ts` mengunci kesetaraan kedua jalur. Lihat detail di bawah |
| #39 | **Tiga lubang hak akses di lapisan RLS** (17 Sep 2026, audit mandiri via Supabase advisor — bukan laporan user). (a) `transactions` punya policy tangkap-semua `Users can manage transactions` (FOR ALL, hanya menguji keanggotaan tanpa filter role) yang membatalkan tiga policy RBAC di sebelahnya → **investor bisa UPDATE/DELETE transaksi** lewat PostgREST; UI & route handler menahan, RLS tidak. (b) `user_business_roles` punya dua policy INSERT yang syaratnya hanya `user_id = auth.uid()` — tanpa batasan business_id maupun role → **siapa pun yang terdaftar bisa mengangkat diri jadi `superadmin` di bisnis mana pun**; tidak ada trigger penjaga (`prevent_default_role_self_promotion` hanya menempel di `profiles.default_role`). (c) `businesses` punya dua policy SELECT selimut (`USING (true)` dan `USING (is_archived = false)`) yang membatalkan `businesses_read` → tiap user yang login membaca SELURUH kolom SEMUA bisnis, termasuk `ical_feed_token`, `qris_image_url`, `property_address`, `registered_address` | RESOLVED | Migr 148 & 149. (a) policy tangkap-semua dicabut; aman karena peran `'both'` tidak bisa ada (CHECK constraint + `normalizeRole()` memetakannya ke `superadmin`), jadi ketiga policy tersisa sudah mencakup semua peran. (b) kedua policy self-insert dicabut, diganti `Creator can claim own business` yang mensyaratkan `businesses.created_by = auth.uid()`; jalur gabung yang sah tidak terpengaruh karena `use_invite_code()` SECURITY DEFINER menembus RLS dan mengambil role dari kode undangan. (c) kedua policy selimut dicabut; daftar untuk `/join-business` pindah ke `GET /api/businesses/discoverable` (admin client, hanya `id`/`business_name`/`logo_url`/`capital_investment`). Lihat detail di bawah |

### Issue #39 — Tiga lubang hak akses di lapisan RLS (17 September 2026)

Ditemukan saat menelusuri lint `multiple_permissive_policies` (94 kombinasi) dari
Supabase advisor. Jumlah itu ternyata bukan sekadar soal kecepatan: policy yang
bertumpuk menyembunyikan tiga policy tangkap-semua warisan migrasi lama.

**Kenapa policy bertumpuk itu berbahaya.** Policy permisif di-**OR**. Satu policy
longgar cukup untuk membatalkan semua policy ketat di sebelahnya, dan itu tidak
terlihat dari membaca policy ketatnya saja.

#### (a) Investor bisa menulis transaksi

`transactions` punya empat policy. Tiga menyusun RBAC dengan benar:

| Policy | Cmd | Syarat |
|---|---|---|
| `Investors can view transactions` | SELECT | role ∈ {investor, superadmin}, `deleted_at IS NULL` |
| `Managers can manage transactions` | ALL | role ∈ {business_manager, superadmin}, `deleted_at IS NULL` |
| `Managers can soft delete transactions` | UPDATE | role ∈ {business_manager, superadmin} |

Yang keempat, `Users can manage transactions` (FOR ALL), hanya berbunyi:

```sql
business_id IN (SELECT business_id FROM user_business_roles WHERE user_id = auth.uid())
```

Tanpa filter role, tanpa filter `deleted_at`. Dikonfirmasi lewat `EXPLAIN` sebagai
investor murni: rencana UPDATE lolos tanpa `One-Time Filter: false`, dan salah satu
cabang OR-nya adalah `Index Only Scan … Index Cond: (user_id = auth.uid())` tanpa
tes role sama sekali.

Setelah dicabut, investor yang sama: **436 baris terbaca, 0 baris bisa ditulis**.

#### (b) Eskalasi diri jadi superadmin

`user_business_roles` punya dua policy INSERT (`Users can join businesses` dan
`Users can insert own roles`) yang `WITH CHECK`-nya hanya `user_id = auth.uid()`.
Karena `business_id` dan `role` tidak dibatasi, baris berikut lolos:

```json
{ "user_id": "<diri sendiri>", "business_id": "<bisnis siapa pun>", "role": "superadmin" }
```

CHECK constraint kolom memang mengizinkan `'superadmin'`, dan **tidak ada trigger**
di tabel ini — `prevent_default_role_self_promotion` ternyata hanya menempel di
`profiles.default_role`, yang sifatnya kosmetik; otoritas sesungguhnya ada di kolom
`user_business_roles.role`.

Digabung dengan (a), rantainya: user baru mana pun → gabung bisnis siapa pun →
akses tulis penuh ke ledger bisnis itu.

Penggantinya hanya mengizinkan satu self-insert yang sah — mengklaim bisnis yang
baru kita buat sendiri (dipakai `POST /api/businesses`):

```sql
CREATE POLICY "Creator can claim own business" ON user_business_roles
  FOR INSERT TO authenticated
  WITH CHECK (
    user_id = (select auth.uid())
    AND public.is_business_creator(business_id)   -- SECURITY DEFINER, wajib
  );
```

`is_business_creator()` **harus** SECURITY DEFINER (migr 150). Versi pertama
memakai subquery biasa ke `businesses` dan memicu rekursi tak hingga lewat
`businesses_read` — sama seperti Issue #11 dulu.

Jalur gabung lain tidak terpengaruh: `use_invite_code()` adalah SECURITY DEFINER
yang melakukan INSERT sendiri (menembus RLS) dan mengambil role dari **kode
undangan**, bukan dari input user; undangan oleh manager tetap lewat
`Business managers can insert roles`.

#### (c) Isolasi tenant `businesses` bocor

Dua policy SELECT selimut membatalkan `businesses_read`:
...
Diukur langsung: seorang manager yang berhak atas 2 bisnis membaca semua 15.
Yang ikut terbawa bukan cuma nama — `getAvailableBusinesses()` memanggil
`select('*')` dari browser, sehingga `ical_feed_token` (token feed .ics; pemegangnya
bisa berlangganan kalender booking bisnis mana pun), `qris_image_url`,
`property_address`, `registered_address`, dan `invoice_settings` ikut terkirim.

Policy itu ada untuk menopang daftar pilihan di `/join-business`. **Menyempitkan
`select()` di sisi klien tidak menutup apa pun** — RLS tidak bisa membatasi kolom,
dan PostgREST melayani `select=*` dari siapa pun. Jadi penyaringannya dipindah ke
server: `GET /api/businesses/discoverable` (admin client) mengembalikan hanya
`id`, `business_name`, `logo_url`, `capital_investment`. Kolom modal sengaja
dipertahankan karena kartu pilihan memang menampilkan "Modal: Rp …" untuk calon
investor. Setelah policy selimut dicabut, manager tadi kembali membaca **2** bisnis.

#### Yang sengaja TIDAK dikerjakan

* **70 FK tanpa index** (lint `unindexed_foreign_keys`) — mayoritas kolom audit
  (`created_by`/`updated_by`) ke `auth.users` yang tidak pernah di-join, pada tabel
  di bawah 350 baris. Satu-satunya kandidat besar, `journal_lines.account_id`,
  ternyata tidak pernah dipakai sebagai filter DB: general ledger meng-index
  `account_id` di memori JavaScript dan mengambil `journal_lines` embedded lewat
  `transaction_id` — yang indexnya sudah ada dan terpakai 2,7 juta kali. Menambah
  70 index hanya memperlambat tulis tanpa manfaat baca.
* **25 index nganggur** — mayoritas milik skema `auth`/`storage` bawaan Supabase,
  sisanya UNIQUE constraint yang menjaga integritas (bukan kecepatan), jadi tidak
  boleh di-drop.
* **`btree_gist` di skema public** — memindahkannya butuh drop constraint yang
  bergantung padanya (exclusion constraint anti-double-booking); risikonya tidak
  sepadan dengan manfaatnya.
* **Leaked password protection** (HaveIBeenPwned) masih mati — hanya bisa
  dinyalakan lewat Supabase Dashboard, bukan migrasi.

#### Pelajaran: verifikasi ekspresi policy ≠ verifikasi policy

Migrasi 148 diverifikasi dengan menjalankan ekspresi `WITH CHECK`-nya sebagai
query biasa (`SELECT EXISTS (SELECT 1 FROM businesses WHERE created_by = auth.uid())`)
— hasilnya benar, dan rekursinya **tidak terpicu**. Rekursi baru muncul ketika
ekspresi itu dievaluasi Postgres *sebagai policy*, karena barulah subquery ke
`businesses` ikut tunduk pada RLS `businesses`.

Artinya: menguji ekspresi policy di luar konteksnya tidak membuktikan apa pun
tentang policy tersebut. Policy yang menyentuh tabel lain harus diuji dengan
DML sungguhan (INSERT/UPDATE) sebagai role target, atau — kalau tabel yang
disentuh punya policy yang menunjuk balik — langsung pakai helper SECURITY
DEFINER sejak awal. Aturan praktisnya: **policy RLS yang menanyakan tabel
ber-RLS lain hampir selalu butuh SECURITY DEFINER.**

#### Migrasi terkait

| Migr | Isi |
|---|---|
| 146 | Bungkus 72 policy: `auth.uid()` → `(select auth.uid())`. Subquery skalar tanpa korelasi di-hoist jadi InitPlan, dievaluasi sekali per query alih-alih sekali per baris. Lint `auth_rls_initplan` 72 → 0 |
| 147 | `SET search_path = public` pada 13 function; cabut EXECUTE dari `PUBLIC` + `anon` (keduanya perlu — hak anon menetes lewat PUBLIC); function trigger dikunci juga dari `authenticated`; grant tabel `ocr_*` dicabut. Lint `anon_security_definer_function_executable` 23 → 0 |
| 148 | Lubang (a) & (b) + cabut 9 policy kembar yang ekspresinya identik/subset |
| 149 | Lubang (c) + `GET /api/businesses/discoverable` |
| 150 | **Regresi dari 148**: policy pengganti menguji kepemilikan lewat subquery langsung ke `businesses`, dan subquery itu tunduk RLS → `businesses_read` menanyakan `user_business_roles` → policy itu lagi. `ERROR: infinite recursion detected in policy for relation "user_business_roles"`, POST /api/businesses gagal 500. Diperbaiki dengan helper SECURITY DEFINER `is_business_creator(uuid)` |


### Issue #38 — Deteksi hutang masih keyword-based di satu jalur (27 Agustus 2026)

**Gejala.** Di Journal Entry → "Pay Debt", daftar "Pilih hutang yang mau dibayar" hanya
menampilkan tiga pencairan Flexi Cash (Crisis 1, Crisis 2, PaCCko). Pencairan keempat —
`#TXN-2023-0001 "Flexi Cash - aa pinjem"`, Rp 2.000.000 — tidak pernah muncul, sehingga
hutangnya tidak bisa dilunasi lewat jalur yang tertaut (`meta.settlement_of_transaction_id`)
dan selamanya tampil belum dibayar.

**Akar masalah.** Bukan datanya. Jurnal "aa pinjem" sudah benar: `Dr 1200 Bank 2.000.000 /
Cr 2100 Flexi Cash 2.000.000` — sesuai loan advice-nya yang memang **service fee 0%**,
sehingga withdrawal = disbursement dan tidak ada baris fee untuk dipecah. Yang membedakannya
dari Crisis 1/2 hanyalah **bentuk pencatatan**: ia double-entry 2 baris, sementara yang lain
multi-line 3 baris (ada baris fee 2%).

`isPayableTransaction()` memperlakukan kedua bentuk itu dengan standar berbeda:

```
multi-line   → line.account?.account_type === 'LIABILITY'          ← lolos
double-entry → account_type === 'LIABILITY'
               DAN /hutang|utang|payable/i.test(account_name)      ← "Flexi Cash" gagal
```

Akun `2100 Flexi Cash` bertipe LIABILITY tetapi namanya tidak mengandung kata
"hutang"/"utang"/"payable", jadi tes nama membuangnya. Konsekuensinya sebuah pinjaman bisa
settleable atau tidak **tergantung berapa baris jurnalnya** — properti yang sama sekali tidak
relevan dengan apakah ia hutang.

Ini kambuhnya **Issue #22** (deteksi receivable/payable via keyword heuristic). Migrasi 085
sudah memasang flag `is_trade_receivable`/`is_operating_payable` dan sisi piutang sudah
direfaktor ke `classification.ts`, tapi detektor settlement sisi hutang terlewat dan masih
memegang regex mentahnya sendiri.

**Perbaikan.** `isAnyPayableAccount()` baru di `src/lib/accounting/classification.ts`, mirror
dari `isAnyReceivableAccount()` di sisi piutang: sebuah liabilitas bisa dilunasi karena
`account_type`-nya LIABILITY, titik — tanpa tes nama. `isPayableTransaction()` dan
`getPayableAccountId()` kini memakainya di **kedua** jalur, sehingga bentuk pencatatan tidak
lagi mengubah hasil klasifikasi.

**Cakupan efek.** `isPayableTransaction()` juga dipakai `useArApAging` (AP aging),
`TransactionDetailModal` (section "Pelunasan Hutang"), dan tab "Belum Lunas" di halaman
transaksi. Ketiganya ikut melebar secara konsisten: hutang double-entry ber-nama non-standar
yang selama ini tak terlihat kini muncul. Tidak ada yang menyempit — perubahan ini murni
aditif terhadap perilaku sebelumnya.

**Catatan simetri yang masih terbuka.** AR aging memakai `isTradeReceivableTransaction`
(trade saja), sedangkan AP aging memakai `isPayableTransaction` (semua liabilitas termasuk
pinjaman). Jadi laporan AR/AP Aging belum benar-benar simetris: sisi AP ikut memuat hutang
pembiayaan. Membuatnya simetris butuh `isOperatingPayableTransaction` tersendiri dan
**mengeluarkan** pinjaman dari AP aging — perubahan perilaku laporan yang belum diputuskan,
jadi sengaja tidak dilakukan bersama fix ini.

### Issue #37 — Penjualan POS mengurangi stok tanpa menyentuh ledger (19 Agustus 2026)

**Gejala.** Untuk bisnis yang mengkapitalisasi pembelian stok ke akun Persediaan, tiap
penjualan lewat kasir POS membuat Neraca dan Laba Rugi meleset searah: Persediaan tetap
penuh padahal barang sudah keluar, dan HPP tak pernah muncul di periode penjualan. Laba
Kotor overstated sebesar harga pokok barang yang terjual.

**Akar masalah.** `useCashier.checkout()` menjurnal hanya sisi penjualan (Dr Kas / Cr
Pendapatan) lalu memanggil `decrement_catalog_stock`. RPC itu mengurangi
`catalog_items.stock_qty` — angka kuantitas di tabel katalog, **bukan** entri buku besar.
Tidak ada jalur apa pun yang melepas nilai persediaan. Kuantitas dan nilai hidup di dua
dunia yang tak pernah bertemu.

Satu-satunya jembatan yang ada, `handleConvertStockToCOGS` (§13.0), justru punya cacatnya
sendiri: ia **mengubah `debit_account_id` transaksi pembelian lama** dari Persediaan ke HPP,
sehingga beban mendarat di periode pembelian dan jurnal historis termutasi.

**Perbaikan** (migrasi 134 + `planCogsPosting`): kolom baru `catalog_items.cost_price`
(standard cost per satuan) memungkinkan checkout merakit jurnal HPP **tersendiri**,
Dr HPP / Cr Persediaan, bertanggal hari penjualan — tanpa menyentuh jurnal pembelian mana
pun. Mekanisme, tiga gerbangnya, dan efek per laporan: **§13.5**.

**Kasus elvéa Indonesia — kenapa gerbangnya menentukan kebenaran.** Data real mengonfirmasi
elvéa (satu-satunya bisnis yang benar-benar melacak stok, 11 item) mencatat **seluruh 46
transaksi VAR-nya langsung ke `5200 Variable Cost (COGS)`** dan tidak punya akun Persediaan
sama sekali. Model beban-saat-beli itu konsisten dengan dirinya sendiri; menambahkan auto-HPP
saat jual justru **menghitung beban dua kali**. Jadi gerbang "punya akun Persediaan" bukan
sekadar penjaga null — ia yang menentukan kebenaran.

**Tidak ada backfill**, jadi tidak ada buku berjalan yang berubah oleh perbaikan ini.

#### Tindak lanjut 20 Agustus 2026 — akun Persediaan disediakan (migrasi 135)

Membiarkan elvéa tanpa akun Persediaan berarti satu-satunya pengguna nyata fitur ini tidak
akan pernah kebagian. Migrasi `135_provision_inventory_account.sql` membuat `1500 Persediaan`
(ASSET, `default_category='VAR'`, induk `1000 Assets`, bukan akun sistem) untuk setiap bisnis
aktif yang punya item ber-`track_stock` tapi belum punya akun persediaan. Kriterianya kena
tepat satu bisnis: elvéa. More&Tea (juga `produk`) tidak punya item ber-`track_stock`, jadi
sengaja dilewati — akun kosong yang tak dipakai hanya mengotori CoA.

**Migrasi ini tidak mengubah satu pun angka.** Akun lahir bersaldo nol dan tak ada transaksi
yang disentuh. Yang berubah hanya: gerbang pertama kini terbuka, sehingga urutan prasyarat di
§13.5 bisa dijalankan pemilik.

**Temuan yang mengubah bentuk solusinya: elvéa PRODUSEN, bukan pengecer.** 46 transaksi VAR
mereka adalah **bahan baku & kemasan** — castor oil 10L, shampo kemiri 60L, botol 100/250ml,
label, shrink, spoolie — dan **nol** di antaranya tertaut ke item katalog
(`meta.catalog_item` kosong di semua baris). Artinya harga pokok per produk jadi (mis. "Shampo
Kemiri Hitam 250ml") tidak bisa diturunkan dari transaksi pembelian mana pun; ia butuh **bill
of materials** — isi cair per ml + botol + label + shrink — yang belum ada di aplikasi.
Persediaan mereka juga sebenarnya tiga jenis (bahan baku, kemasan, barang jadi), bukan satu.

Karena itu `cost_price` ke-11 item sengaja dibiarkan 0 dan **tidak ditebak**. Memposting angka
harga pokok hasil tebakan ke buku besar nyata bukan wewenang migrasi; nilainya diisi pemilik
lewat stock opname, atau menyusul bila BOM dibangun. Sampai itu terjadi, elvéa tetap berjalan
dengan model beban-saat-beli dan auto-HPP tetap diam — yang benar, bukan kompromi.

### Issue #36 — Card "Business Capital" menampilkan CAPEX, bukan modal usaha (13 Agustus 2026)

**Gejala** (dilaporkan user, data real Ventures): card Manage Business yang sebelumnya
menunjukkan ~432jt mendadak jadi **Rp 0** setelah migrasi 128, padahal tidak ada transaksi
yang dihapus. More&Tea 57,7jt → juga anjlok.

**Akar masalah — salah label, bukan salah hitung.** Sejak migrasi 077 card berlabel
"Business Capital" diisi `capital_investment + CAPEX`. CAPEX = *belanja modal*, khusus aset
tetap. "Business capital"/modal usaha jauh lebih luas: mencakup **modal kerja** — kas,
piutang, dan persediaan. Motor yang dibeli untuk dijual lagi adalah modal yang berubah wujud
dari kas jadi barang dagangan, tetap modal.

Migrasi 128 (ACC-M12) benar sebagai perbaikan CAPEX — persediaan & piutang memang bukan
belanja modal — dan justru migrasi itulah yang **menyingkap** mismatch yang sudah ada 51
migrasi sebelumnya: begitu definisi CAPEX diperketat, bisnis yang seluruh modalnya berputar
di persediaan tidak punya apa pun lagi untuk ditampilkan. Ventures (sektor `finance`,
`capital_investment` = 0) isinya persis 407jt persediaan + 13,3jt piutang usaha + 12,2jt
piutang talangan — ketiganya dikecualikan 128, sisanya nol.

**Perbaikan** (migrasi 132): isi card diselaraskan dengan labelnya — RPC baru
`get_total_assets_by_business()` mengembalikan **total aset** (kas + piutang + persediaan +
aset tetap neto), yakni sisi penggunaan modal. Angkanya wajib sama dengan baris "Total Aset"
di Neraca, jadi RPC meniru tiap filter `calculateBalanceSheet()`:

| Aspek | 077/128 (CAPEX) | 132 (total aset) |
|-------|-----------------|------------------|
| Cakupan akun | ASSET non-kas, bukan persediaan/piutang | seluruh akun ASSET |
| Jurnal multi-baris | diabaikan (`journal_lines` di luar cakupan) | ikut, via `journal_lines` |
| Status transaksi | semua non-deleted | hanya `posted`/NULL (= `useReportData`) |
| Jurnal penutup | ikut terhitung | dikecualikan (`isClosingEntry`, ACC-M3) |
| Penyusutan | tidak ada | dikurangkan (straight-line, `depreciation.ts`) |
| `capital_investment` | selalu ditambahkan | **fallback saja**, hanya bila bisnis belum punya transaksi |

Identitas yang dipakai: `total aset = Σ saldo akun ASSET (debit − kredit) − akumulasi
penyusutan`, setara `adjustedTotalAssets` di `calculations.ts` karena `classifyAsset()`
mempartisi seluruh nilai aset ke lancar + tetap. Fallback `capital_investment` dipersempit
karena modal awal sudah otomatis dibukukan sebagai transaksi "Modal Investasi Awal"
(`app/api/businesses/route.ts`) — menambahkannya lagi = double-count, senada Issue #24.

**Dampak angka** (13 Agt 2026): Ventures 0 → 407.131.742 · Hillside Studio 369.034.797 →
370.365.608 (kini net setelah 17,5jt penyusutan) · TELYUFOLK 11.525.500 → 24.835.888 ·
Telcantik 0 → 400.000 · elvéa 39.319 → 1.459.273 · **More&Tea 57.730.000 → −6.338.354**.

> ⚠️ Nilai negatif sengaja **tidak** di-clamp ke nol. More&Tea: aset tetap 57,4jt dengan
> Kas −4,9jt dan Bank −58,8jt — belanja tercatat tapi sumber dananya (setoran modal/pinjaman)
> tidak pernah dibukukan. Ventures punya gejala sama (Bank −53jt). Card yang menampilkan
> minus adalah sinyal celah pembukuan yang memang perlu ditutup, bukan bug hitung.

**Catatan implementasi**: penyusutan di SQL memakai `CURRENT_DATE` (zona waktu server)
sedangkan versi TS memakai jam browser — selisih maksimum satu bulan penyusutan, hanya
tepat di pergantian bulan. `get_capex_by_business()` tidak di-drop: definisinya benar,
hanya sudah tidak dipanggil UI.
...
Temuan audit yang **masih OPEN** (Medium/Low) per 12 Agt 2026: ACC-M2, M5–M8, M10,
M13, M14, ACC-L2–L7, SEC-L3–L8 — lihat `docs/AUDIT_2026-06-11.md` untuk status terkini.
(ACC-M3, M9, M12, L1 selesai di Issue #28; ACC-M4 di Issue #29.)

### Issue #29 — Siklus hidup jejak pelunasan: restore & cabang partial (12 Agustus 2026)

**Sumber**: `docs/AUDIT_2026-06-11.md` ACC-M4. Temuan aslinya — soft-delete pelunasan
meninggalkan tagihan asal "lunas selamanya" — **sudah diperbaiki 22 Juni 2026** lewat
migrasi 108 & 109 (commit `76329ad`); dokumen audit saja yang belum diupdate. Yang ditutup
sekarang adalah dua celah tersisa di area yang sama.

**(a) `restore_transaction` tidak punya pasangan (migrasi 129).** Migrasi 108/109 membuat
penghapusan mencabut jejak pelunasan, tapi pemulihan tidak memasangnya kembali. Alur yang
terpapar bukan alur langka: tombol **"Urungkan"** di toast setelah hapus
(`src/lib/transactionToast.tsx` → `useTransactions.ts:427`) dan restore massal
(`useTransactions.ts:616`). Akibatnya entri pelunasan hidup lagi di buku besar — Dr Bank /
Cr Piutang tetap menggerakkan Neraca & Arus Kas — sementara tagihan asal tetap tampil
**belum lunas**, masih muncul di AR/AP aging, dan **masih bisa dilunasi sekali lagi**
sehingga piutang ter-kredit dua kali dan saldonya jadi negatif.

Perbaikan: `restore_transaction` menerapkan ulang efek pelunasan — hitung sisa tagihan,
lalu tandai full (`settled_by_transaction_id` + `remaining_amount = 0`) bila nominal yang
dipulihkan menutup seluruh sisa, atau partial (`partial_settlements` + sisa baru) bila
tidak. Aturan sisa tagihan diekstrak ke helper `calc_settlement_outstanding()`.

> ⚠️ `settle_transaction` (migr 110) masih memakai salinan in-line dari aturan sisa tagihan
> yang sama dan sengaja tidak disentuh — fungsi itu ~340 baris dan migrasi 110 lahir justru
> karena drift saat menulis ulangnya. **Kalau aturan sisa tagihan berubah, ubah keduanya.**

**(b) Cabang partial di migrasi 108 crash (migrasi 130).** Ditemukan saat menguji (a).
Filter array memakai `jsonb_array_elements` (tanpa `_text`), sehingga elemen masih JSONB dan
`val::text` membawa tanda kutip — cast ke UUID meledak dengan
`22P02: invalid input syntax for type uuid: ""5e52b973-...""`. Klausa `WHERE` di bawahnya
sudah memakai `_text` yang benar, jadi barisnya terpilih lalu ekspresi `SET`-nya gagal:
RPC melempar exception dan **seluruh penghapusan dibatalkan**. Efek nyata bagi user —
entri pelunasan sebagian **tidak bisa dihapus sama sekali** (HTTP 500), bukan sekadar meta
yang tidak ter-update. Saat diperbaiki ada 8 tagihan dengan 14 cicilan hidup yang terkena.
Cabang full settlement tidak menyentuh array, itu sebabnya bug ini lolos: jalur yang paling
sering dipakai justru yang sehat.

**Verifikasi**: uji round-trip di DB (delete → restore) untuk full & partial settlement,
dijalankan dalam blok yang di-rollback otomatis. Kedua skenario kembali persis ke meta
semula — full: `{settled_by_transaction_id, remaining_amount: 0}`; partial 300rb dari 1jt:
`{partial_settlements: [id], remaining_amount: 700000}`.

### Issue #28 — Batch Medium/Low #1 audit 11 Juni (12 Agustus 2026)

**Sumber**: `docs/AUDIT_2026-06-11.md`. Tema: angka yang saling bertentangan antar halaman
padahal berasal dari ledger yang sama.

| ID Audit | Fix |
|----------|-----|
| ACC-M3 | Jurnal penutup (`meta.entry_type.id === 'closing_entry'`) sebelumnya hanya difilter di `calculateBalanceSheet`, sehingga closing entry historis **tetap masuk Income Statement** dan mengurangkan seluruh pendapatan & beban periode dua kali. Filter dipusatkan ke helper `isClosingEntry()` yang kini juga dipakai `calculateFinancialSummary`, `extractIncomeStatementLineItems`, dan `groupTransactionsByMonth` — jadi IS, drill-down/export, dan chart bulanan konsisten dengan Neraca |
| ACC-M9 | `useDashboard` memanggil `calculateBalanceSheet(transactions, capital)` tanpa `accounts`/`reportDate`, sehingga blok depresiasi (`calculations.ts:1499`) dilewati dan aset tetap + ekuitas di dashboard **overstated** dibanding `/balance-sheet` yang mengirim keduanya. Kini `accounts` (sudah tersedia di hook untuk KPI P&L) + `new Date()` diteruskan. `accounts.length` ditambahkan ke dep effect write-through cache: accounts di-fetch di query terpisah dan bisa tiba setelah transaksi, tanpa itu cache membekukan neraca pra-depresiasi yang lalu di-hydrate di kunjungan berikutnya |
| ACC-M12 | Total CAPEX di card Manage Business menghitung **setiap** debit ASSET non-kas sebagai belanja modal — persediaan & piutang ikut terjaring. Bug-nya ada di RPC `get_capex_by_business` (migr 077), bukan lagi di `calculateTotalCapex` yang per Agt 2026 sudah nol pemanggil (fungsi itu dihapus). Migrasi 128 menambah pengecualian yang meniru `isInventoryAccount()` & `isAnyReceivableAccount()` agar server dan browser mengklasifikasi sama. Dampak nyata: Kopi Nusantara 618,8jt → 135jt (47 trx restock), Ventures 432,1jt → 0 (407jt persediaan + 25jt piutang/talangan) |
| ACC-L1 | `groupTransactionsByMonth` mengurutkan hasil dengan `month.localeCompare()` padahal `month` berisi nama bulan terlokalisasi ("Agu 2026") → urutan alfabetis, deret waktu kacau begitu melewati batas tahun. Kini sortir pakai key `YYYY-MM`. Catatan: dampaknya laten — satu-satunya konsumen (`useScenarioModeling`) hanya memakai hasilnya untuk rata-rata yang order-independent dan tidak me-render `monthlyData` |
...
### 21.5 Matriks RLS setelah pengerasan migr 146–149 (17 Sep 2026)

Peran yang mungkin ada hanya tiga. `'both'` **tidak bisa** tersimpan — CHECK
constraint `user_business_roles.role` membatasi ke
`business_manager | investor | superadmin`, dan `normalizeRole()` di
`src/lib/roles.ts` memetakan nilai legacy `'both'` → `'superadmin'`. Policy yang
masih menyebut `'both'` di tabel lain adalah sisa warisan, bukan jalur aktif.

| Tabel | Investor | Business manager / superadmin |
|---|---|---|
| `transactions` | SELECT saja (`deleted_at IS NULL`) | penuh |
| `businesses` | hanya bisnis tempat ia anggota atau pembuat | sama |
| `user_business_roles` | lihat peran di bisnisnya | + INSERT (mengundang) |
| `accounts` | SELECT | penuh |

Tiga hal yang berubah dan mudah terlupa:

1. **Tidak ada lagi policy tangkap-semua di `transactions`.** Sebelumnya
   `Users can manage transactions` (FOR ALL, tanpa filter role) membatalkan RBAC
   di atas. Kalau suatu saat investor perlu menulis sesuatu, tambahkan policy
   sempit untuk kolom/aksi itu — **jangan** kembalikan policy berbasis keanggotaan.
2. **Self-insert peran hanya untuk bisnis sendiri.** `Creator can claim own business`
   mensyaratkan `businesses.created_by = auth.uid()`. Gabung lewat undangan berjalan
   di `use_invite_code()` (SECURITY DEFINER, menembus RLS, role diambil dari kode
   undangan). Kode baru yang perlu menyisipkan peran untuk **orang lain** harus lewat
   admin client atau policy manager, bukan menambah policy self-insert.
3. **`businesses` bukan sumber daftar publik.** Barisnya memuat `ical_feed_token`
   dan `qris_image_url`. Daftar untuk `/join-business` mengalir lewat
   `GET /api/businesses/discoverable` (admin client, 4 kolom). Jangan query tabel
   `businesses` langsung dari browser untuk kebutuhan lintas-tenant — RLS tidak bisa
   membatasi kolom, jadi penyaringan harus di server.

Detail temuan dan buktinya ada di Section 19, Issue #39.
...
```
totalClosingCapital = Σ capitalClosing per pemilik (cap table: opening + credit − debit, akun is_stock)
retainedClosing     = balanceSheet.equity.retainedEarnings (closing) — sudah net prive/dividen
...
> - **Tarik modal (debit akun `is_stock`)** → di kedua sisi mengurangi **CAPITAL**: SCE lewat
>   `capitalClosing = opening + credit − debit`; Neraca lewat `netEquityMovements =
>   totalEquityCredit − totalStockDebit` (Section 6.3). TIDAK menyentuh RE.
> - **Dividen/prive (debit akun `is_dividend`)** → di kedua sisi mengurangi **RETAINED EARNINGS**:
>   `retainedClosing` dari neraca sudah dikurangi (`netIncome − totalNonStockEquityDebit`), dan SCE
...
> neraca (karena semua debit ekuitas dulu menurunkan RE) → dikurangi dua kali di SCE → tie-out gagal.

**Nuansa akuntansi penting:** dividen/prive **tidak** masuk Income Statement. `netIncome` periode
SCE = `(retainedClosing − retainedOpening) + periodDividends` = revenue − expense murni (dividen
...
### 26.8 Fitur GCP Cloud SQL yang Dicabut (19 Agustus 2026)

Instance Cloud SQL GCP `axion-agents` **dihapus 10 Agustus 2026** untuk menolkan tagihan
(tier N-8 mulai ~Rp7jt/bulan setelah Free Trial berakhir, dan credit GenAI Rp17,8jt hanya
menanggung Vertex/Gemini — bukan database). Kode yang bergantung padanya sempat dipertahankan
sembilan hari untuk kemungkinan rebuild; per 19 Agustus keputusannya **permanen**, jadi
kodenya dicabut daripada menyisakan jalur yang selalu gagal.

| Dicabut | Isinya |
|---------|--------|
| Tool `run_olap_analytics` | Query agregasi (GROUP BY) atas replika `olap_transactions` — tren waktu, ranking, pivot. Penggantinya `get_financial_summary` / `query_transactions` yang membaca data live Supabase |
| Tool `recall_memory` | Pencarian semantik memori percakapan lampau (vector 768d, `text-embedding-004`) |
| `src/lib/gcpSync.ts` + `/api/cron/sync-olap` + `/api/admin/gcp/sync` | Sync harian & manual Supabase → replika `olap_*` |
| `src/lib/ai/semanticMemory.ts` | `embedText` / `searchMemories` / `ingestVaultMemory` / `ingestSessionSummary` / `deleteSessionMemories` |
| `/api/test-gcp` | Endpoint uji koneksi |
| Tabel `agent_memories` + lima tabel `olap_*` di `gcpSchema.ts` | DDL bootstrap |
| Tombol "Sinkronkan Data Bisnis" di Settings | Kartu GCP kini superadmin-only (tinggal bootstrap skema knowledge base) |

**Tidak ada data yang hilang.** Isi memori agent (Memory Vault + riwayat chat) selalu
berada di Supabase `agent_memories` sebagai primary store; yang lenyap bersama instance hanya
**embedding** dan ringkasan sesi — dua-duanya turunan yang bisa dibangkitkan ulang. Replika
`olap_*` seluruhnya turunan `transactions` Supabase.

**Yang TETAP ada:** `search_knowledge_base` + `/api/ai/upload-knowledge` masih menulis/membaca
`business_knowledge_embeddings` lewat `src/lib/gcp.ts`. Fitur ini **inert sejak 10 Agustus**
(tak ada instance untuk dihubungi) tapi sengaja tidak ikut dicabut — ia fitur produk
tersendiri (unggah file ke agent), bukan bagian OLAP/semantic memory. Circuit breaker di
`gcp.ts` membuatnya gagal instan (0 ms) alih-alih menggantung sampai hard-kill 60 detik
Vercel.

---

### 26.9 Siklus Hidup Token Instagram (29 Agustus 2026)

Long-lived token Instagram berumur **60 hari** dan Meta tidak memperpanjangnya sendiri.
Sampai 29 Agustus 2026 tidak ada mekanisme refresh di kode sama sekali, sehingga token
Telcantik & elvéa yang dibuat 12 Juni **mati 11 Agustus** dan tidak ada yang menyadarinya
selama 17 hari.

**Kenapa senyap.** Webhook DM tidak memakai token, jadi gejalanya nyaris tak terlihat:
pesan tetap masuk dan lead tetap terbentuk. Yang mati hanya panggilan Graph API —
`getInstagramSenderName()` gagal lalu nama lead jatuh ke fallback `@<IGSID>`, dan
`sendInstagramMessage()` gagal sehingga auto-send diam-diam berubah jadi draft. Card di tab
Integrasi pun tetap hijau "Terhubung" karena tidak pernah membaca `token_expires_at`.
Bukti di data: username asli terakhir ter-resolve 11 Agustus 10:08 UTC — ±8 jam sebelum
token kedaluwarsa — dan 34 lead sesudahnya bernama numerik semua.

| Lapisan | Berkas | Perilaku |
|---------|--------|----------|
| Refresh otomatis | `src/lib/instagram/token.ts` | `getFreshToken()` memperpanjang token saat sisa umur < 10 hari, lalu menyimpannya. Dipanggil di jalur webhook — satu-satunya jalur yang rutin jalan |
...
bulat-bulat dengan hasil `buildTokenConfig()`, sehingga reconnect **menghapus tier Pro**
tanpa pemberitahuan (terjadi sungguhan pada Telcantik, 29 Agustus 2026). Callback dan
`getFreshToken()` kini sama-sama mengoper `config` lama sebagai `extra`.

**Disconnect bukan jalan untuk reconnect.** `DELETE /api/integrations/[id]` menyetel
`is_active = false`, dan `findActiveIntegration()` menyaring tepat di kolom itu — selama
jeda tersebut **setiap DM yang masuk hilang total**, tidak tersimpan sebagai lead maupun
pesan. Sambungkan ulang langsung lewat `/api/integrations/instagram/auth`.

**Sisa risiko.** Refresh menumpang webhook, jadi akun yang tidak menerima DM sama sekali
selama 50 hari tetap bisa kedaluwarsa. Badge + banner adalah jaring pengaman untuk kasus
itu; cron harian belum dibuat.
...
Katalog" di bawah + §27.4).

**Migrasi 099 — tabel `catalog_items`:**

| Kolom | Tipe | Catatan |
|-------|------|---------|
| `name` | TEXT | **TIDAK unik** sejak migr 121 — varian sah bernama sama (mis. "Castor Oil" L & S) |
...
| `sku` | TEXT NULL | kode item (produk saja), **identitas unik item** sejak migr 121: UNIQUE per bisnis (case-insensitive + trim, abaikan NULL/string kosong, `deleted_at IS NULL`). Opsional — item tanpa SKU tidak saling bentrok. Auto-match import TikTok/Tokopedia masih fase berikutnya |
...
**Picker — `CatalogItemPicker.tsx`** (grid kotak-kotak item), 2 mode + prop `allowCreate`:

| Mode | Dipakai di | Perilaku |
|------|-----------|----------|
| `single` | `QuickTransactionForm` (REVENUE) | Muncul saat akun **REVENUE** dipilih (`isRevenueSelected`). Klik 1 item → isi `amount` + `name`; bila item punya `revenue_account_id` beda, pindah `selectedAccountId` ke akun itu. |
| `single` + `allowCreate` | `QuickTransactionForm` (inventory) & `TransactionForm` (debit = persediaan) | Alur **VAR→stok**: muncul saat akun persediaan dipilih (`isInventoryAccount`). Klik item → HANYA set `meta.catalog_item {id, name}` (chip) — amount/akun TIDAK disentuh (harga beli ≠ `default_price` jual). `allowCreate`: teks pencarian tanpa exact match memunculkan tombol "Buat item …" → `createCatalogItem` inline (`product`, harga 0). |
...
deskripsi (murni visual — teks deskripsi tidak diubah), plus icon `FileText` abu-abu
bila transaksi punya `meta.attachments`/`meta.attachment`. `TransactionDetailModal`
sudah lebih dulu menampilkan chip yang sama.

**Nama katalog di-resolve saat baca, bukan snapshot (24 Agt 2026).** `meta.catalog_item`
tetap disimpan sebagai `{ id, name }`, tapi hanya `id` yang jadi sumber kebenaran —
`name` cuma label tampilan. Sebelumnya nama itu dibekukan saat entry, jadi rename item
di Katalog (kasus nyata: "Bitcoin" → "BTC") menghasilkan DUA nama untuk satu item yang
sama: transaksi lama tetap "Bitcoin", transaksi baru "BTC". Perbaikannya
`resolveCatalogNames()` di `src/lib/api/transactions.ts`, dipanggil oleh ketiga jalur
baca (`getTransactions`, `getTransactionsPaginated`, `getTransactionsByDateRange`):
kumpulkan id unik dari `meta.catalog_item`, satu query `catalog_items(id, name)`, lalu
timpa `name` di memori. **Baris `transactions` TIDAK ditulis ulang** — sengaja, supaya
rename tidak memicu ribuan UPDATE ledger plus banjir `audit_log` (bandingkan migr 119
yang harus mematikan trigger untuk hal serupa). Item yang sudah di-soft-delete ikut
di-resolve (tanpa filter `deleted_at`) agar histori tidak kehilangan label; id yang
tidak ditemukan atau query gagal → snapshot lama dipakai apa adanya (best-effort, tidak
pernah melempar). Cache TanStack Query bertahan 5 menit, jadi `CatalogPanel` meng-invalidate
`['transactions']` + `['transactions-paginated']` saat — dan hanya saat — nama berubah.
Konsumen lain otomatis ikut benar karena semua lewat ketiga fungsi itu; Asset Console
memang sudah kebal sejak awal (agregasinya berbasis `meta.catalog_item.id`).
Yang MASIH snapshot: `meta.cogs_items[].name` (rincian HPP POS, migr 134) — belum
di-resolve karena butuh rebuild array JSONB, dan nilainya memang rekaman harga pokok
saat transaksi, bukan sekadar label.
...
transaksi stok — sebelumnya stok cuma bisa difilter lewat VAR (tercampur HPP).
Server-side di `getTransactionsPaginated`: `category=STOCK` → `eq('category','VAR')`
+ **INNER join** `debit_account` (`!inner` — wajib, default PostgREST LEFT join tidak
...
dari `inventoryHelper` (sebelumnya copy lokal berbasis kode 13xx/nama — diverifikasi
ekuivalen di data real: 52 vs 52 transaksi, 0 divergensi) → badge, filter, form
picker, dan backfill migr 119 berbagi SATU definisi.

**Migrasi 119 — backfill link katalog transaksi stok lama.** Semua transaksi VAR
...
("Yamaha NMAX (dibayar dari …)" → "Yamaha NMAX") → insert `catalog_items`
(`ON CONFLICT` unique name per bisnis DO NOTHING) → update `meta.catalog_item`.
Trigger user tabel `transactions` dimatikan sementara selama UPDATE (jaga
`updated_at`/`updated_by` — `auth.uid()` NULL di konteks migrasi — dan hindari
...
kini bisa dilakukan langsung dari tab Katalog (`CatalogPanel`) — khusus item
`item_type='product'` (jasa tidak tersentuh; submit form membersihkan sku/stok
untuk jasa):
- **Form item** (`CatalogItemForm`): field `SKU` (opsional, unik per bisnis —
  ...
  **Nama TIDAK divalidasi unik** (migr 121) — prop `existingNames` dihapus.
...
- **Riwayat Stok** (`StockLogPanel`, aside kanan hub POS di bawah Concierge Agent,
  hanya `variant='pos'`): daftar perubahan stok terbaru — nama item, delta
  (+hijau / −merah), `dari → ke`, pelaku, waktu. **Sumber perubahan** (migr 123):
  RPC stok men-set GUC transaksi-lokal `app.audit_source` sebelum UPDATE dan
  `log_audit_trail()` menyalinnya ke `metadata.source` — `pos_sale` (checkout
  kasir, tampil chip netral "Penjualan"), `stock_add` (aksi Tambah Stok);
  jalur form edit tidak men-set apa-apa → NULL → tanpa chip. Backfill: 2 baris
  pengurangan lama ditandai `pos_sale` (diverifikasi keduanya dari checkout). **Tanpa tabel log baru** —
  ...
  JSONB) — query mengambil `limit × 10` lalu dipotong. `CatalogPanel` memanggil
  `onStockChanged` → `HubPage` menaikkan `refreshKey` → panel memuat ulang.
  **Prasyarat migr 122**: `log_audit_trail()` dulu meng-hardcode daftar tabel
  ...
  004 lain) selalu `metadata.business_id = NULL` → panel tampak kosong padahal
...
- **Aksi cepat "Tambah Stok"** (ikon `PackagePlus` di hover action, manager-only):
  modal jumlah barang masuk → RPC **`increment_catalog_stock(p_item_id, p_qty)`**
  (migr 120, SECURITY DEFINER) — penambahan **atomik** (aman terhadap race dengan
  ...
  `get_my_business_ids` karena dipanggil alur checkout). `p_qty <= 0` → exception;
  item tak memenuhi guard → return NULL → caller tampilkan error ramah.
...
> laporan penjualan per-item; jembatan stok qty ↔ nilai persediaan ledger
...
**Migrasi 134 — jembatan stok ke ledger:**

| Perubahan | Catatan |
|-----------|---------|
| `catalog_items.cost_price` NUMERIC(15,2) ≥ 0 default 0 | harga pokok per satuan (standard cost, input manual). 0 = HPP tidak dijurnal otomatis. Tidak di-backfill — buku lama tak berubah |
...
3. **Jurnal HPP** (migr 134): bila `planCogsPosting()` menghasilkan rencana, simpan
   transaksi **VAR terpisah** Dr HPP / Cr Persediaan bertanggal sama, tertaut lewat
   `meta.cogs_of_transaction_id`. Butuh tiga gerbang sekaligus — bisnis punya akun
   Persediaan, ada akun beban sah penampung HPP, dan item ber-`track_stock` dengan
   `cost_price > 0`. Gagal → `checkout()` mengembalikan `cogsWarning` (toast peringatan),
   penjualan tetap sah. **Detail lengkap & alasan tiap gerbang: §13.5.**
...
> POS khusus (pakai soft-delete transaksi umum), payment gateway, stock opname, costing
> moving-average/FIFO (migr 134 memakai standard cost per item), auto-HPP di luar kasir
> (Quick Entry EARN masih lewat InventoryPicker §13.0 — hanya POS yang mengurangi stok).
...
- **Yang mengunci harga = ADANYA jurnal, bukan `payment_status` (25 Agu 2026).**
  `priceLocked = is_external || payment_status==='paid' || transaction_id != null`. Booking
  yang pendapatannya diakui sebagai **piutang** (mis. payout OTA belum cair: Dr Piutang Usaha
  / Cr Short-term Rent) tetap berstatus `unpaid`, dan sebelum perbaikan ini ia ikut jalur
  auto-quote kalender harga — totalnya dihitung ulang dari rate weekday/weekend sehingga
  BERTENTANGAN dengan jurnalnya (kasus nyata: booking Airbnb Rp1.173.379 tampil Rp850.000).
  Selain itu `total` untuk booking terkunci kini dibaca dari `total_amount` apa adanya, bukan
  `nights × price_per_night` — `price_per_night` hanya turunan rata-rata yang dibulatkan,
  sedangkan `total_amount` yang benar-benar dijurnal.
- **`markBookingPaid` menolak booking yang sudah punya `transaction_id`** — cukup itu, tanpa
  menunggu `payment_status='paid'`. Sebelumnya guard-nya `paid && transaction_id`, sehingga
  booking-piutang lolos dan tombol "Tandai Lunas" akan merakit transaksi EARN KEDUA (Dr Kas /
  Cr Pendapatan) sekaligus menimpa `transaction_id` — pendapatan dobel dan tautan ke jurnal
  asli putus. Blok "Terima pembayaran" di `BookingModal` diganti keterangan yang mengarahkan
  pelunasan ke settle piutang di transaksinya.
...
`external`>`cancelled`>`paid`(Lunas)>`checked_in`>`tentative`>`confirmed` — status DB `completed`
...
di-label **"Inquiry"/"Pertanyaan"** (bukan "Tentatif") — statusnya tetap `tentative`, hanya labelnya.
...
(`BOOKING_BAR_CLASSES.paid`; dark mode di-INVERT jadi chip terang + teks gelap — chip gelap
...
Sejak **migr 124**, harga dasar (base) berasal dari **item layanan per unit** per kategori hari —
BUKAN lagi dari satu `business_units.rate_item_id` flat.

**Model data (migr 124):**
- **Unit = level teratas.** Pilih unit → kalender DAN daftar Services yang tampil = milik unit itu.
...
- `catalog_items.unit_id` — layanan di-scope ke unit fisik (NULL = milik bisnis, produk/addon).
  ...
  bila `service_role='main'`). Unique index `(unit_id, rate_kind)` per main → 1 item per kategori.
- **Base per hari:** Sabtu+Minggu → harga item `rate_kind='weekend'` (fallback ke `weekday` bila
  tak ada); Sen–Jum → item `weekday`. Item `monthly` = acuan sewa panjang (tak mewarnai grid harian).
- `unit_daily_rates` (business_id, **unit_id**, date, price; UNIQUE per unit+tanggal) — **layer
...
- Resolusi per tanggal (lib murni `src/lib/rates.ts`, migr 124): **override > base(weekday|weekend
  by hari) > 0**. `buildUnitBaseRates(items)` → `UnitBaseRates{weekday,weekend,monthly}`;
  `baseRateForDate`, `resolveNightPriceV2(date, base, overrideMap)`; `quoteStayV2(checkIn, checkOut,
  base, overrides)` → Σ per malam, KECUALI rentang > **27 malam** (`MONTHLY_STAY_THRESHOLD`) & ada
  `monthly` → total = harga bulanan. `groupIntoRanges` meringkas malam berurutan berharga sama.
...
klik tanggal → klik tanggal lain = pilih rentang → `RateEditorPanel` (input override + **filter
...
1. **BookingModal (create/edit)** — `useDailyRates.priceOf` pakai `resolveNightPriceV2(base,
   overrideMap)`; total otomatis = Σ per malam (weekday/weekend); `price_per_night` = rata-rata
   dibulatkan, `total_amount` = Σ eksak. `RateEditorPanel` menerima `defaultPrice` = base weekday.
2. **AI concierge** (`leadAssistant.ts`) — untuk tiap unit aktif, `RateInfo{weekday,weekend,monthly,
...
3. **Halaman publik `/[slug]`** — `app/[slug]/page.tsx` ambil **unit aktif pertama**, hitung
   `buildUnitBaseRates` + override 365 hari, **ekspansi tiap tanggal** (`resolveNightPriceV2`) →
   ...
   `computeRangePricing` widget: rentang > 27 malam & ada `monthly_price` → satu line harga bulanan.
4. **booking-inquiry** (`/api/public/booking-inquiry`) — tentatif website: `quoteStayV2` (base +
   override range; >27 malam=monthly) → `total_amount`/`price_per_night`.
...
## 28. Asset Console (Portofolio Investasi Konsolidasi, migr 125 & 126)

Dashboard portofolio investasi (saham, kripto, properti, emas, **venture**) yang
menggabungkan posisi dari **beberapa broker/kustodian sekaligus** ke satu
tampilan. Masalah yang diselesaikan: aplikasi broker hanya tahu posisinya
sendiri — Sinarmas tahu 1 lot BMRI, Stockbit tahu 2 lot BMRI, tak ada yang tahu
totalnya 3 lot dengan average cost gabungan Rp 4.186,13/lembar.

### 28.1 Keputusan arsitektur — read-model, BUKAN ledger paralel

Rancangan awal mengusulkan 4 tabel baru (`asset_instruments`,
`asset_custodians`, `asset_positions`, `asset_position_events`) plus trigger
average-cost dan pembuatan jurnal otomatis. **Ditolak.** Sebagai gantinya Asset
Console adalah murni agregasi di atas `transactions`.

Alasannya: posisi dan cost basis sudah tercatat lengkap di buku besar. Menyalin
angka yang sama ke tabel kedua menciptakan dua sumber kebenaran yang bisa
berbeda (posisi bilang 3 lot, neraca bilang lain) tanpa memberi informasi baru.
Dengan read-model, **posisi ADALAH buku besar** — drift secara struktural tidak
mungkin terjadi.

Konsekuensi yang perlu diketahui:
- Asset Console **tidak membuat transaksi**. Beli/jual/dividen dicatat lewat
  jalur normal (Journal Entry / Quick Entry) seperti transaksi lain.
- Instrumen didefinisikan di **Katalog**, bukan di halaman Asset Console —
  kecuali kelas `venture`, yang ditautkan lewat tombol "Hubungkan Venture" di
  halaman Asset Console (lihat §28.4b).

**Gating field kelas aset di form Katalog (13 Agt 2026).** Blok "Kelas Aset
Investasi" + lot size di `CatalogItemForm` hanya dirender bila
`isAssetCatalogBusiness(business_type, business_sector)` → **`business_type='dagang'`
DAN sektor Asset Console (`finance`)**. Sebelumnya ia muncul di semua item produk,
termasuk katalog F&B/personal care yang tak pernah menjual instrumen investasi —
memperpanjang form tanpa guna. Sengaja lebih sempit dari `isAssetConsoleSector`
(yang menggerbangi akses *halaman* Asset Console): "boleh lihat portofolio" dan
"boleh mendefinisikan instrumen baru" dua pertanyaan berbeda. **Escape hatch:**
item yang sudah terlanjur punya `asset_class` tetap menampilkan blok tersebut
walau bisnisnya di luar gate, supaya nilainya masih bisa diubah/dilepas.
`handleSubmit` memakai flag yang sama (`canMarkAsset`), jadi bisnis di luar gate
selalu mengirim `asset_class: null` + `asset_lot_size: 1`.

Prinsip yang sama berlaku untuk `venture` walau sumbernya berbeda: yang dibaca
tetap buku besar, hanya saja buku besar **bisnis lain**. Tidak ada nominal yang
disalin ke `catalog_items`.

### 28.2 Data model (migrasi 125 & 126)

Tidak ada tabel baru. Lima kolom ditambahkan ke `catalog_items`:

| Kolom | Tipe | Catatan |
|-------|------|---------|
| `asset_class` | TEXT NULL | `'stock'` \| `'crypto'` \| `'property'` \| `'gold'` \| `'venture'`. **NULL = item katalog biasa**, tidak muncul di Asset Console (opt-in) |
| `asset_lot_size` | NUMERIC > 0, default 1 | jembatan satuan: berapa satuan harga per 1 kuantitas transaksi. Saham IDX = 100 |
| `asset_price_updated_at` | TIMESTAMPTZ NULL | kapan `default_price` terakhir di-update sebagai harga pasar |
| `linked_business_id` | UUID NULL → `businesses` | **hanya venture**: bisnis lain yang kepemilikannya dipantau |
| `linked_stock_account_id` | UUID NULL → `accounts` | **hanya venture**: akun EQUITY `is_stock` milik user di bisnis target |

Dua constraint menjaga tautan venture (migr 126):
- `catalog_items_venture_link_check` — dua arah: `venture` **wajib** punya kedua
  pointer, kelas lain **dilarang** punya. Tanpa ini bisa lahir baris "venture
  tanpa target" yang tampil sebagai Rp0 misterius.
- `uq_catalog_items_venture_link` — UNIQUE `(business_id, linked_stock_account_id)`
  partial `WHERE asset_class='venture' AND deleted_at IS NULL`. Menautkan posisi
  yang sama dua kali akan menggandakan Total Modal & Nilai Pasar di KPI.

Baris `venture` **difilter keluar dari `getCatalogItems()`** (`.or('asset_class.is.null,asset_class.neq.venture')`
— `.neq()` saja membuang item biasa karena `NULL <> 'venture'` bernilai NULL di
SQL) dan disimpan `is_active=false`, sehingga tidak pernah muncul di halaman
Katalog, picker POS, AI concierge, maupun halaman publik.

`default_price` merangkap **harga pasar terakhir** (input manual, belum ada live
feed) supaya tidak ada dua kolom harga yang bisa berbeda.

**Kenapa `asset_lot_size` perlu:** transaksi mencatat kuantitas dalam LOT
(`meta.unit_breakdown.quantity` = 1, `unit` = "Lot BMRI"), sedangkan harga
dikutip per LEMBAR (`catalog_items.unit` = "Lembar", `default_price` = 4270).
Tanpa kolom ini, avg cost per lembar tidak bisa dihitung tanpa menebak dari
string "Lot" — dan menebak dari teks dilarang (§18).

### 28.3 Pemetaan transaksi → posisi

Sumber tautan instrumen adalah `meta.catalog_item` (pola yang sama dengan chip
katalog, migr 119). Kustodian = `transactions.name` (nama broker, mis.
"Sinarmas Sekuritas") — inilah yang memungkinkan konsolidasi lintas broker.

Klasifikasi event memakai **account_type + flag**, bukan pencocokan nama akun
(§18). Akun "posisi" = `account_type = 'ASSET'` AND NOT `is_cash_equivalent`
AND NOT `is_trade_receivable`:

| Kondisi | Event | Efek |
|---------|-------|------|
| Δ akun posisi > 0 | `buy` | qty += `unit_breakdown.quantity`; cost basis += Δ |
| Δ akun posisi < 0 | `sell` | cost basis −= \|Δ\|; realized P/L dari baris REVENUE/EXPENSE |
| Δ = 0, ada REVENUE/EXPENSE | `dividend` | realized P/L saja, posisi tak berubah |
| selain itu | `adjustment` | cost basis disesuaikan |

Realized P/L = (kredit REVENUE − debit REVENUE) − (debit EXPENSE − kredit
EXPENSE). Rumus ini menangani dua konvensi sekaligus: bisnis yang memakai satu
akun pendapatan gabungan, maupun yang memisah akun gain & loss.

### 28.4 Average cost & kuantitas jual yang diturunkan

Metode **average cost per (instrumen × kustodian)** — jual mengurangi cost basis
broker yang sama, karena secara riil aset disimpan di broker tertentu. Transaksi
diproses kronologis karena average cost bersifat *path-dependent*.

Transaksi jual sering tidak mencatat `unit_breakdown.quantity`. Kuantitasnya
**diturunkan dari metode average cost itu sendiri**:

```
qty_terjual = cost_basis_yang_dilepas / avg_cost_saat_itu
```

Eksak, bukan tebakan. Contoh nyata (BBCA, 20 Jul 2026): beli 4 lot seharga
Rp 2.313.241 → avg 578.310,25/lot. Jual melepas cost basis 1.734.931 →
1.734.931 / 578.310,25 = **3 lot** tepat, sisa 1 lot @ 578.310.

Penjagaan:
- Jual yang melepas **seluruh** cost basis menutup posisi persis (`qty = 0`),
...
- Hanya transaksi `status = 'posted'` yang dihitung, konsisten dengan aturan
  laporan keuangan (draft tidak masuk laporan).

### 28.4a Crypto: Unit di-canon dari Katalog, BUKAN SUM(transaksi) — sementara

**Pengecualian dari §28.3/28.4** khusus `asset_class = 'crypto'`. 1 BTC bernilai
ratusan juta–miliar rupiah, sehingga kepemilikan riil hampir selalu pecahan
mikro (mis. **0,00302599 BTC**). Mengagregasi `meta.unit_breakdown.quantity`
antar-transaksi seperti kelas lain rawan salah — ditemukan kasus nyata:
transaksi ditulis "beli 3 Coin @ Rp1.000.000" (jelas bukan harga pasar BTC,
kemungkinan data eksperimen/salah isi form), dan kalau dijumlahkan apa adanya
kartu Unit akan menampilkan **"3"** yang tidak masuk akal dibanding realita
0,00302599 coin.

**Solusi sementara:** untuk crypto, `catalog_items.asset_lot_size` dipakai
sebagai **angka total unit yang dipegang** — bukan rasio konversi seperti pada
stock (1 lot saham = 100 lembar). Diisi manual di form Katalog (field "Jumlah
yang Dipegang" — label & hint beda dari field yang sama untuk kelas lain, lihat
`CatalogItemForm.tsx`).

```ts
// src/lib/assetConsole.ts, dalam buildHolding()
const isCryptoUnitOverride = item.asset_class === 'crypto';
const totalQuantity = isCryptoUnitOverride ? lotSize : positionQuantity;
// lotSize SUDAH JADI unit itu sendiri saat override aktif — tidak boleh
// dikalikan lagi ke market value (beda dari stock, di mana lotSize tetap
// rasio lembar-per-lot yang harus dikalikan).
const effectiveLotSize = isCryptoUnitOverride ? 1 : lotSize;
```

**Yang TETAP dari transaksi (tidak di-override):** `totalCostBasis`,
`realizedPl`, dan seluruh tabel Riwayat Transaksi (murni log historis, bukan
sumber kebenaran Unit). Hanya `totalQuantity` (dan turunannya: `avgCost`,
`marketValue`, `unrealizedPl`) yang memakai angka catalog.

**Konsekuensi UI:** tabel "Breakdown by Custodian" di halaman detail
menyembunyikan kolom Unit & Avg Price per baris untuk crypto (kolom Cost Basis
& Realized P/L tetap tampil, murni dari transaksi tanpa butuh kuantitas) — data
per-broker belum punya sumber unit yang sama-sama valid dengan total di
catalog, jadi tidak ditampilkan berdampingan supaya tidak ada dua angka Unit
yang beda muncul di halaman yang sama.

**Kolom Unit di tabel konsolidasi** juga menampilkan nama satuan dari Katalog
di samping angkanya (`quantityUnitLabel()` — "Lot" hardcode untuk stock karena
beda dari `priceUnit`/"Lembar", kelas lain pakai `item.unit` apa adanya:
Coin, gram, unit).

**Konsekuensi saat JUAL:** karena Unit di-canon dari katalog, penjualan TIDAK
otomatis mengurangi angka Unit — `catalog_items.asset_lot_size` harus
di-update manual ke sisa posisi (form Katalog, field "Jumlah yang Dipegang").
Cost basis & realized P/L tetap ikut transaksi seperti biasa, jadi kalau
langkah ini terlewat halaman akan menampilkan Unit lama berdampingan dengan
cost basis yang sudah menyusut. Kasus nyata (BTC, 22 Agu 2026): jual 0,00302
dari 0,00302599 BTC menyisakan **dust 0,00000599** — sisa yang secara fisik
tidak bisa dijual karena di bawah step size 0,00001 BTC milik Binance, jadi
posisinya memang belum tertutup dan `asset_lot_size` diisi 0,00000599, bukan 0.

**Ini keputusan sementara**, bukan arsitektur final — kalau ke depan perlu
breakdown per-broker yang akurat untuk crypto, override total ini perlu
diganti mekanisme yang lebih baik (mis. wallet address per kustodian).

### 28.4b Venture: kepemilikan di bisnis sendiri (migr 126)

Kelas `venture` memantau saham/kepemilikan owner di **bisnis lain yang juga ada
di AXION** (mis. posisi Imam di "Hillside Studio"). Ini satu-satunya kelas yang
tidak membaca transaksi bisnis pemantau sama sekali.

**Rumus — semuanya turunan, nol input manual:**

| Kolom Asset Console | Sumber |
|---------------------|--------|
| Unit | `%` kepemilikan dari `calculateCapTable(txTarget)` — net credit akun `is_stock` user ÷ total semua akun stock |
...
Asset Console tidak mungkin berbeda dari laporan bisnis targetnya.

**Pemetaan ke bentuk holding yang sama** (`buildVentureHolding()`):
kuantitas = persen (2,65), `priceUnit` = `'%'`, `lastPrice` = valuasi per 1% =
`totalEquity / 100`. Ini bukan kosmetik — dengan pemetaan itu identitas
`kuantitas × harga = nilai pasar` yang dipakai seluruh tabel tetap berlaku apa
adanya, sehingga tidak ada cabang kalkulasi khusus yang bisa melenceng dari
kelas lain (ada unit test yang menjaganya).

**`hasLivePrice` menggantikan `lastPrice > 0`.** Kelas lain memakai "harga > 0"
sebagai penanda nilai pasar tersedia. Itu salah untuk venture: total ekuitas
**sah** bernilai nol atau negatif (bisnis rugi melebihi modal disetor), dan gate
lama akan menyembunyikan justru kasus yang paling perlu dilihat pemilik.
`hasLivePrice` = `lastPrice > 0` untuk kelas biasa, `!unresolved` untuk venture.

**Cara menautkan — BUKAN lewat form Katalog.** Venture bukan barang yang bisa
dijual: tidak ada SKU, stok, harga jual, atau akun pendapatan yang relevan.
Tombol **"Hubungkan Venture"** di header halaman Asset Console membuka modal
dengan dua dropdown berurutan:

1. **Bisnis** — dari `BusinessContext.businesses`, minus bisnis aktif itu
   sendiri (kepemilikan di diri sendiri sudah tampil di Neraca/SCE-nya) dan
   minus yang diarsipkan.
2. **Akun Ekuitas Kamu** — akun `account_type='EQUITY' AND is_stock=true` di
   bisnis itu. Dicari lewat **flag**, bukan nama akun (§18). Akun yang sudah
   ditautkan tampil disabled + label "sudah terhubung".

Aksinya menyisipkan satu baris `catalog_items` (`asset_class='venture'`,
`is_active=false`, nama = nama bisnis target). Melepas tautan = soft delete
baris itu lewat tombol "Putuskan Tautan" di halaman detail instrumen — data
bisnis & transaksinya sama sekali tidak tersentuh.

**Keamanan lintas bisnis** aman by construction: query tetap lewat client
biasa (bukan `createAdminClient()`), jadi RLS hanya melepas data bisnis yang
user memang anggotanya. Kalau akses dicabut belakangan, snapshot ditandai
`unresolved` dan barisnya menampilkan peringatan — bukan Rp0 yang menyesatkan
atau seluruh halaman gagal load.

**Valuasi = nilai buku (Fase 1).** "Nilai Pasar" untuk venture memakai total
ekuitas, bukan valuasi negosiasi/term sheet. Kelas ini memang disiapkan sebagai
pintu masuk fitur valuasi ke depan (§28.7).

**Kolom Avg Price / Last Price di tabel utama.** Venture tidak punya "harga per
unit" yang sebanding dengan lembar/coin/gram, tapi kedua sel TETAP dirender
terpisah (bukan `colSpan={2}`) — jumlah kolom tiap baris wajib identik, kalau
tidak seluruh kolom sesudahnya di baris lain ikut bergeser (pernah terjadi:
Studio Unit tampil Avg/Last tertukar dan P/L kosong). Isinya yang dibedakan:

| Kolom | Isi untuk venture |
|-------|-------------------|
| Avg Price | **rumus** asal valuasi: chip netral "Aset − Liabilitas" (tooltip menjelaskan = nilai buku bisnis target). Dibungkus chip supaya tidak terbaca sebagai nominal seperti kolom angka di kiri-kanannya |
| Last Price | **valuasi per 1%** (`h.lastPrice` = `totalEquity / 100`), bukan valuasi penuh — header kolomnya "Harga Terakhir (Rp/unit)" dan satuan venture adalah persen, jadi isinya wajib harga per SATU satuan seperti kelas lain. Dengan begitu identitas **Kepemilikan × Harga = Nilai Pasar** berlaku di semua baris (2,65 × 1.901.462 = 5.046.819, persis seperti BMRI 300 × 4.170). Dirender `formatUnitPrice` — **tanpa prefix "Rp"**, karena kolom ini angka telanjang di semua kelas lain. Label kecil "Valuasi per 1%" (`whitespace-nowrap`); nominal valuasi penuh tetap tampil di halaman detail dan di tooltip chip rumus |

`formatOwnershipPct()` menyembunyikan desimal bila nol: kepemilikan penuh
tampil **"100%"**, bukan "100,00%" — dua digit nol di sana tidak menambah
informasi dan membuat angka bulat terlihat seperti hasil pembulatan. Persen
pecahan tetap 2 desimal ("2,65%").

#### Kolom Kepemilikan pada saham: satuan HARGA jadi baris utama

Untuk instrumen ber-`lotSize > 1` (saham IDX: 1 lot = 100 lembar), baris utama
kolom Kepemilikan adalah **"300 Lembar"**, dengan "3 Lot" turun ke baris kedua —
sebelumnya terbalik. Alasannya aritmatika, bukan selera: kolom Avg/Last Price
dikutip **per lembar**, jadi angka yang sebaris dengan harga harus angka yang
dikalikan dengannya. BMRI: `4.170 × 300 lembar = Rp 1.251.000` = Nilai Pasar.
Kalau "3 Lot" yang tampil di baris utama, pembaca menghitung `4.170 × 3` dan
meleset 100×. Kelas lain (`lotSize = 1`) tidak punya dua satuan — kuantitas
sekaligus satuan harga — jadi tetap satu baris.

Header kolom P/L ikut diperjelas jadi **"Laba/Rugi (P/L)"** — singkatan "P/L"
saja tidak memberi tahu pembaca baru bahwa itu laba/rugi. Header "Harga
Terakhir" sempat dicoba ditambah suffix "(Rp/unit)" untuk menegaskan itu harga
per satuan, tapi dilepas lagi: kolomnya jadi terlalu lebar dan mendesak
chevron navigasi di ujung kanan baris. Makna "per satuan" tetap terjaga lewat
isi kolomnya sendiri (lihat di atas), bukan lewat teks header.

Rumusnya ditulis eksplisit supaya pembaca tidak salah mengira valuasi itu harga
pasar hasil transaksi — ia nilai buku yang dihitung ulang dari neraca target.

#### Riwayat transaksi venture — dari buku besar bisnis TARGET

Sebelumnya tabel Riwayat pada halaman detail venture selalu kosong ("riwayat ada
di buku besar bisnis yang ditautkan"). Itu membuat KPI **Total Modal** dan
**Dividen Diterima** tidak bisa ditelusuri ke satu peristiwa pun. Sekarang
`getVentureSnapshots()` ikut mengembalikan `events: VentureLedgerEvent[]` —
transaksi di buku besar target yang **membentuk** kedua angka itu:

| Sumber | Aturan baca (identik dengan fungsi agregatnya) |
|--------|-----------------------------------------------|
| Mutasi modal | baris jurnal yang menyentuh `linked_stock_account_id`; **kredit = setor, debit = tarik/prive** (sama seperti `calculateCapTable`) |
| Dividen | **debit** ke akun `is_dividend` yang `owner_stock_account_id`-nya menunjuk akun stock itu (sama seperti `accumulateDividendsByOwner`) |

Aturan akunnya sengaja disalin persis dari fungsi agregat, bukan ditulis ulang
dengan heuristik lain — kalau berbeda, daftar transaksi bisa bercerita lain dari
KPI di atasnya. Transaksi yang tidak menyentuh salah satu akun itu **tidak**
diikutkan: nilai pasar venture memang bergerak karena laba/rugi operasional
bisnis target, tapi menampilkan seluruh buku besar bisnis itu di sini berarti
menyalin laporan orang lain ke halaman instrumen. Selisih nilai pasar vs modal
sudah diringkas sebagai Unrealized P/L.

Satu transaksi yang menyentuh keduanya (mis. dividen yang langsung
dikapitalisasi jadi tambahan modal) dipecah jadi **dua baris**, supaya tiap
angka bisa ditelusuri ke KPI-nya masing-masing.

**Pemetaan ke `AssetEvent`** (`mapVentureEvents()`):

- `capitalDelta > 0` → `'buy'`, dilabeli **"Setor Modal"** (bukan "Beli")
- `capitalDelta < 0` → `'sell'`, dilabeli **"Tarik Modal"** (bukan "Jual")
- dividen → `'dividend'`

Kolom **Kuantitas** dan **P/L Terealisasi** disembunyikan untuk venture alih-alih
menampilkan dua kolom `—` di setiap baris. Kuantitas dihilangkan karena satuan
venture adalah *persen* kepemilikan, dan persen pada saat sebuah setoran terjadi
tidak bisa diturunkan dari transaksi itu sendiri — nilainya bergantung pada total
modal seluruh pemilik pada saat itu, yang berubah tiap ada setoran pemilik lain.
Menampilkan `—` jujur; menghitung angka yang tidak bisa dipertanggungjawabkan
tidak. P/L terealisasi selalu 0 (alasannya di tabel rumus di atas).

**Kustodian venture = `Self Custody · {nama pemilik}`.** Kelas lain menaruh nama
broker/exchange di kolom Kustodian (dari `transactions.name`). Venture tidak
dititipkan ke pihak ketiga mana pun — stake dipegang langsung atas nama pemilik
di buku besar bisnis target. Label "Self Custody" membuat kolom itu tetap
bermakna sama di semua kelas (*siapa yang memegang aset*), sementara nama
pemilik menjawab *atas nama siapa*. Nama diambil dari `profiles.full_name`
pembuat bisnis target (`businesses.created_by`), bukan `accounts.account_name`
yang bisa generik seperti "Owner's Capital".

### 28.5 Backfill tautan transaksi jual (migrasi 125)

Sebelum migrasi ini hanya transaksi BELI yang membawa `meta.catalog_item`.
Transaksi jual (EARN multi-line: Dr Bank / Cr Persediaan / Cr Pendapatan) tidak
tertaut ke instrumen apa pun, sehingga agregasi akan menghitung posisi **bruto**
— BBCA terbaca 7 lot / Rp 4.011.251 padahal tersisa 1 lot / Rp 578.310.

Backfill memakai dua jalur, **meta-only** (nominal, akun, dan `journal_lines`
tidak disentuh; trigger user dimatikan mengikuti preseden migrasi 119):
1. **Via `meta.sold_stock_ids`** — instrumen diambil dari transaksi beli yang
   dirujuk. Dilewat bila stok berasal dari >1 item katalog (jual campuran tidak
   ditebak).
2. **Fallback deskripsi** — hanya bila **persis satu** item ber-`asset_class`
   cocok (word boundary). Ini perbaikan data satu kali untuk record legacy,
   bukan logic klasifikasi runtime.

**Pencegahan ke depan:** `deriveCatalogItemFromStock()`
(`src/lib/utils/inventoryHelper.ts`) menstempel `meta.catalog_item` otomatis
saat penjualan dibuat, dipakai di jalur single-line & multi-line journal-entry
dan di `QuickTransactionForm`. Sekalian memperbaiki bug lama: pindah ke mode
multi-line dulu membuang `sold_stock_ids` diam-diam.

### 28.6 File terkait

| Peran | File |
|-------|------|
| Engine agregasi (murni, teruji) | `src/lib/assetConsole.ts` |
| Data access | `src/lib/api/assetConsole.ts` |
| Data access venture (cap table + neraca bisnis target) | `src/lib/api/venture.ts` |
| Hook halaman | `src/hooks/useAssetConsole.ts` |
| Konstanta kelas aset | `src/lib/assetClasses.ts` |
| Modal tautan venture | `src/components/assetConsole/ConnectVentureModal.tsx` |
| Halaman | `app/(dashboard)/asset-console/page.tsx` + `[itemId]/page.tsx` |
| Unit test | `tests/unit/assetConsole.test.ts` (20 test) |

Nav item muncul hanya untuk `business_sector = 'finance'`
(`isAssetConsoleEnabled()` di `src/lib/navigation.ts`). Sengaja **tidak** memakai
`business_type = 'dagang'`: badge "TRADING" di header memang berasal dari sana
(label EN untuk 'dagang'), tapi semantiknya perdagangan barang — UMKM retail
biasa juga 'dagang' dan tidak butuh Asset Console.

### 28.7 Belum dikerjakan (Fase berikutnya)

- Integrasi API broker (Sinarmas, Stockbit) — semua entry masih manual
- Live price feed — harga di-update manual per instrumen
- **Valuasi venture selain nilai buku** — mis. valuasi post-money dari term
  sheet, multiple revenue/EBITDA, atau valuasi manual bertanggal. Kini "Nilai
  Pasar" venture = `%` × total ekuitas (§28.4b)
- FIFO/LIFO per-lot — hanya average cost
- Multi-currency penuh untuk kripto USD (kini `currency_code` + rate manual)
- Form "Catat Transaksi Baru" khusus di dalam Asset Console (kini lewat Journal
  Entry / Quick Entry seperti transaksi lain, konsisten dengan prinsip §28.1)

---

## 29. Event Registration — "Book Your Spot" (migr 136)

Pendaftaran event komunitas lewat halaman publik omnichannel, untuk bisnis
**jasa sektor `creative_agency`** (saat ini Telcantik & TELYUFOLK; contoh
pertama: Fun Padel). Bukan modul akuntansi — **tidak menyentuh ledger sama
sekali** (lihat §29.7) — tapi didokumentasikan di sini karena ikut memakai
fondasi yang sama: RLS berbasis `is_business_manager()`, pipeline leads, dan
hub `/calendar`.

### 29.1 Model mentalnya: crowdtesting tanggal, bukan form minat

Manager membuat satu **sesi** (mis. "Fun Padel Batch 3"), menetapkan format tim
(`team_count × players_per_team`), lalu membuka **beberapa tanggal kandidat
sekaligus**. Audience memilih tanggal yang mereka bisa, lalu mengunci satu slot
player di dalam tim. Kapasitas dihitung **per tanggal**, jadi kecepatan
terisinya bisa dibandingkan antar tanggal — itulah sinyalnya.

Yang penting: **"penuh" hanya sinyal, bukan pemicu otomatis**. Tanggal lain baru
ditutup ketika manager menekan "Jadikan pemenang". Ini disengaja — sinyal
kuantitatif (slot terisi) tetap tunduk pada pertimbangan yang tidak ada di
database (ketersediaan lapangan, cuaca, jadwal talent).

### 29.2 Gating

```
business_type = 'jasa' AND business_sector = 'creative_agency'
```

Satu sumber: `supportsEventRegistration()` di `src/lib/businessSectors.ts`.
Dipakai di dua tempat — `HubPage` (memilih isi tab operasional `/calendar`) dan
`app/[slug]/page.tsx` (memuat kartu "Book Your Spot"). Sektor akomodasi tetap
mendapat kalender booking per malam; keduanya menempati slot tab yang sama
karena sama-sama menjawab "kapan orang datang", tapi modelnya berbeda (tanggal
pasti vs tanggal yang masih diperebutkan).

### 29.3 Skema & wasit slot

| Tabel | Isi |
|-------|-----|
| `event_sessions` | 1 event: judul, format tim, `team_labels` JSONB, `contact_method`, status (`draft`/`open`/`closed`/`cancelled`), soft-delete |
| `event_session_dates` | tanggal kandidat = 1 "Lobby"; status `candidate`/`won`/`discarded` |
| `event_registrations` | klaim slot per (tanggal, tim, player) + `contact_value` + `lead_id` |

Wasit anti-rebutan slot:

```sql
CREATE UNIQUE INDEX event_registrations_slot_unique
  ON event_registrations (session_date_id, team_number, player_number)
  WHERE status <> 'cancelled';
```

Parsial (`status <> 'cancelled'`) supaya slot yang dibatalkan manager kembali
bisa diisi. Dua submit bersamaan ke slot yang sama → yang kalah kena `23505`,
dipetakan jadi HTTP 409 + pesan "Slot ini baru saja diambil orang lain".

Batas `team_number ≤ team_count` dan `player_number ≤ players_per_team`
**tidak bisa** dijaga CHECK constraint (butuh baca tabel induk), jadi divalidasi
di dalam `register_event_slot` — bukan diasumsikan aman dari sisi client.

### 29.4 Jalur publik — kenapa API route, bukan anon RPC atau Realtime

Halaman publik AXION tidak pernah bicara langsung ke Postgres: `/[slug]`
dirender server dengan `createAdminClient`, dan widget reservasi menulis lewat
`/api/public/booking-inquiry`. Modul ini mengikuti pola yang sama:

```
baca  → GET  /api/public/events/[sessionId]     (polling Lobby, 8 detik)
tulis → POST /api/public/events/register        → RPC register_event_slot()
```

Konsekuensinya **nol grant anon**: tidak ada policy RLS publik, tidak ada view
publik, dan `contact_value` (nomor WA / username IG) tidak pernah bisa terbaca
browser lewat jalur mana pun. `loadPublicSession()` di
`src/lib/events/publicSession.ts` adalah satu-satunya tempat yang memutuskan
kolom apa yang boleh keluar.

Dua jebakan yang sengaja dihindari:

1. **Supabase Realtime `postgres_changes`** — payload-nya membawa row lengkap
   tabel dasar sesuai RLS, bukan sesuai kolom yang diproyeksikan view, jadi
   `contact_value` ikut terkirim ke browser publik. Karena itu Lobby memakai
   polling sederhana. Kalau nanti butuh live update sungguhan, jalurnya
   **Broadcast** (event manual berisi field aman saja), bukan WAL otomatis.
2. **`SECURITY DEFINER`** — tidak dipakai. `register_event_slot` dipanggil
   service_role (sudah BYPASSRLS) dan `mark_event_date_winner` dipanggil manager
   yang policy-nya sudah mengizinkan, jadi keduanya `SECURITY INVOKER` +
   `SET search_path = public`, sejalan dengan hardening migrasi 102.

### 29.5 Integrasi ke Leads — pendaftar = lead, otomatis

`register_event_slot` melakukan tiga tulisan dalam **satu transaksi**: upsert
`leads` → klaim slot → sisip `lead_messages`. Kalau slotnya keburu diambil,
seluruhnya batal — tidak ada lead setengah jadi.

Tiga detail yang menentukan modul ini terasa "nyambung" atau tidak:

- **Normalisasi kontak.** Nomor WA dinormalkan ke bentuk `wa_id` webhook
  (digit saja, `0…` → `62…`), username IG di-lowercase tanpa `@`. Tanpa ini,
  pendaftar yang sudah pernah chat WhatsApp akan membuat lead kembar alih-alih
  menyambung ke thread yang sudah ada.
- **`last_message_at` DAN `last_inbound_at` sama-sama di-set.** Inbox `/leads`
  mengurut dengan yang pertama; hitungan unread di lonceng notifikasi
  (`useLeadCounts`) membandingkan `last_read_at` vs yang kedua. Kalau salah satu
  ketinggalan, lead-nya "hilang" dari salah satu permukaan itu.
- **Nama lead yang sudah ada tidak ditimpa** (`COALESCE(NULLIF(leads.name,''), …)`)
  — kontak lama bisa jadi sudah dirapikan manager, sementara isian form event bebas.

Efek sampingnya: **notifikasi admin tidak butuh infrastruktur baru**. Setiap
pendaftaran menaikkan hitungan lead unread di lonceng header, dan isi pesannya
(`Daftar "<judul>" — <tanggal> · <tim> #<player>`) sudah cukup jadi konteks
follow-up manual.

### 29.6 Siklus status & aksi manager

```
draft ──"Buka pendaftaran"──► open ──"Jadikan pemenang"──► closed
                               │                            (1 tanggal won,
                               └──"Tutup pendaftaran"──► closed  sisanya discarded)
```

`mark_event_date_winner(p_session_date_id)` mengerjakan ketiganya sekaligus:
tanggal terpilih → `won`, kandidat lain → `discarded`, sesi → `closed`.
Pendaftar di tanggal yang di-discard **tetap punya `lead_id` valid** — mereka
tetap bisa di-follow-up dari inbox yang sama.

Membatalkan satu pendaftar (`status = 'cancelled'`) membebaskan slotnya karena
index uniknya parsial. Sesi yang dihapus manager adalah soft-delete: hilang dari
UI manager dan halaman publik, pendaftar & lead-nya utuh.

Lobby untuk sesi `closed` sengaja **tidak** 404 — link yang sudah tersebar tetap
terbuka dan menampilkan pengumuman tanggal pemenang. Pendaftaran barunya tetap
ditolak di sisi DB.

### 29.8 Warna brand & warna tim (migr 137)

Seluruh permukaan publik event mengambil warnanya dari **satu field yang sudah
ada**: `business_omni_channels.button_color`. Tidak ada warna khusus event —
kalau owner mengubah warna brand, tombol, label "BOOK YOUR SPOT", bar progres,
cincin tanggal terpilih, dan banner pemenang ikut berubah sekaligus. Nilai
kosong = indigo default aplikasi (`#6366f1`).

Di atas itu, tiap tim bisa diberi warna sendiri lewat `event_sessions.team_colors`
(JSONB, key = nomor tim, sejajar `team_labels`). Kosong = tim ikut warna brand.
Warna tim tampil di chip nama tim & avatar pemain — di Lobby publik maupun di
grid slot sisi manager, supaya identitas timnya sama di dua tempat.

Tiga aturan yang mengikat implementasinya:

1. **Warna brand bukan token UI.** Diterapkan lewat `style`, bukan kelas
   Tailwind; UI dashboard tetap `primary-*`. Lihat `docs/DESIGN_SYSTEM.md` §3.10.
2. **Warna teks di atas warna brand dihitung, bukan ditebak** —
   `readableTextColor()` di `src/lib/colorUtils.ts` memilih putih/near-black dari
   rasio kontras WCAG. Ambang luminansi sederhana gagal untuk kuning: `#eab308`
   luminansinya 0,50 sehingga mendapat teks putih dengan kontras 1,9:1.
3. **Kosong ≠ indigo.** `team_colors` yang tidak diisi berarti "ikut brand", jadi
   jangan menaruh default warna di kolomnya — kalau diisi indigo, tim itu akan
   tetap indigo saat owner mengganti warna brand.

Pengaturannya ada di dua tempat yang menulis field yang sama: Bisnis → Halaman
Publik → Warna Tombol Utama, dan langsung di panel Event Manager (`ColorPickerField`,
menyimpan lewat `upsertOmniChannel` dengan payload 4 kolom saja supaya setelan
halaman publik lain tidak ikut tertimpa).

**Bug nyata yang mengoreksi aturan #2 di atas**: `readableTextColor()` hanya
cukup untuk teks di atas warna SOLID. Tombol CTA memakai `brandGradient()`
(gradien turun ke 82% campur hitam), dan untuk warna gelap-medium titik BAWAH
gradien bisa membalik keputusan yang "benar" di titik ATAS — dilaporkan user
dua kali dengan dua warna brand berbeda (`#9b6a8f`, lalu `#b3729d`) sebelum
akar masalahnya ketemu. Perbaikannya `readableTextColorOnGradient()`, yang
mengevaluasi kontras TERLEMAH dari kedua ujung gradien, bukan cuma titik atas.
Detail lengkap & angka kontrasnya di `docs/DESIGN_SYSTEM.md` §3.10.

**Override manual warna teks tim (migr 138)**: `event_sessions.team_text_colors`
(JSONB, key = nomor tim, nilai `'light'|'dark'` saja — bukan hex bebas) mengizinkan
owner memaksa chip nama tim pakai teks hitam/putih pilihannya sendiri, menimpa
hasil hitungan kontras otomatis. Diklik langsung di chip pratinjau form Event
Manager; `resolveTeamTextColor()` yang membaca override-nya, dipakai konsisten
di 3 tempat (form admin, grid slot admin, Lobby publik) supaya pratinjau di form
benar-benar "persis yang dilihat pendaftar" — bukan pratinjau yang berbeda dari
kenyataan.

### 29.9 Bentuk UI Lobby: drill-down, bukan dua daftar bertumpuk

Versi pertama Lobby menampilkan daftar tanggal DAN grid slot di satu halaman
(grid di bawah daftar). Di layar HP itu gagal total: mengetuk tanggal tidak
mengubah apa pun yang terlihat — responsnya ada ratusan piksel di bawah lipatan,
jadi terbaca seperti tombol rusak.

Sekarang dua layar yang saling menggantikan:

1. **Pilih tanggal** — baris ringkas (hari, tanggal, bar + hitungan dalam SATU
   baris). Kepadatan ini bukan selera: perbandingan "tanggal mana yang paling
   cepat penuh" adalah inti fitur, dan perbandingan tidak terjadi kalau
   tanggalnya harus di-scroll satu per satu. Diverifikasi di viewport iPhone 13:
   4 tanggal + header muat dalam satu layar.
2. **Pilih slot** — grid tim × player, dengan header lengket berisi tanggal
   terpilih + jalan kembali ("Ganti tanggal"), dan banner konfirmasi setelah
   slot terkunci. Berpindah layar selalu me-reset scroll ke atas.

Sesi dengan satu tanggal saja langsung mendarat di layar slot dan tidak
menampilkan jalan kembali — tidak ada tanggal lain untuk dituju.

**Kunci tema terang.** Seluruh cabang `/[slug]` dipaksa light mode
(`app/[slug]/layout.tsx` + `PublicThemeLock`): halaman publik itu materi promosi
yang warnanya dirancang owner untuk latar terang, sementara pengunjung sering
membukanya dari in-app browser Instagram yang ikut dark mode HP. Implementasinya
skrip pra-paint (anti-kedip) + MutationObserver (bertahan setelah hidrasi) —
BUKAN `forcedTheme` bertingkat dari next-themes, karena efek komponen anak
berjalan sebelum efek induk sehingga provider root akan menimpanya kembali.
Kelas asli dikembalikan saat komponen dilepas supaya dashboard owner tidak ikut
jadi terang setelah menengok halaman publiknya.

### 29.7 Dampak akuntansi: nol

Modul ini tidak membuat, mengubah, atau menghapus baris `transactions` apa pun,
dan tidak menyentuh Chart of Accounts. Pendaftaran event bukan peristiwa
ekonomi — belum ada kas berpindah, belum ada kewajiban yang timbul. Kalau event
menghasilkan pemasukan (biaya ikut) atau biaya (sewa lapangan), pencatatannya
lewat transaksi biasa seperti sebelumnya; tidak ada penjurnalan otomatis dari
modul ini yang perlu direkonsiliasi.

### 29.10 Label eyebrow bisa diedit (migr 139)

Teks kecil di atas judul ("BOOK YOUR SPOT") — di kartu halaman publik dan
header Lobby — awalnya hardcode. Sekarang `event_sessions.eyebrow_text`
(TEXT, maks 40 karakter), diedit langsung di form Event Manager. NULL/kosong
= fallback ke default `'Book Your Spot'` yang di-resolve di komponen
(`event.eyebrow_text?.trim() || 'Book Your Spot'`), BUKAN di database — owner
tidak bisa "mengosongkan total" label ini lewat UI, field kosong selalu
berarti "pakai default", bukan "sembunyikan". Kolom TEXT biasa, bukan JSONB
seperti `team_labels`/`team_colors`, karena ini properti satu sesi — bukan
per-tim.

### 29.11 Avatar pendaftar dari galeri tetap (migr 140)

Audience yang mengunci slot bisa (opsional) memilih avatar dari galeri tetap
`public/persona/*.png` — katalognya di `src/lib/events/avatars.ts`
(`EVENT_AVATAR_OPTIONS`). **Bukan upload bebas** — dua alasan:

1. Halaman publik ini sudah nol-grant-anon (§29.4) — jalur upload butuh
   storage bucket + validasi file, permukaan baru sama sekali di luar
   cakupan "pilih avatar simpel".
2. `event_registrations.avatar_key` dijaga CHECK constraint ke set tetap —
   bahkan kalau validasi di route publik terlewat, DB tetap menolak nilai di
   luar set itu. Tak pernah bisa jadi jalur nyuntik URL/HTML sembarang.

**Set avatarnya sengaja sama dengan avatar karakter AXION Agent** (Bianca,
Sri Mulyani, Stanley, Concierge, agent) — keputusan eksplisit user, bukan
aset baru yang dibuat khusus. Konsekuensinya: pemain event bisa memilih wajah
yang di fitur lain merepresentasikan persona AI finansial. Kalau nanti mau
diganti aset khusus event, cukup ubah `EVENT_AVATAR_OPTIONS` + migrasi baru
untuk CHECK constraint-nya — dua tempat itu WAJIB selalu sinkron.

Kosong = fallback ke inisial nama (perilaku dari sebelum fitur ini ada, tidak
berubah). `register_event_slot` (migr 136) mendapat parameter baru
`p_avatar_key TEXT DEFAULT NULL` — ditambahkan lewat `DROP FUNCTION` eksplisit
dulu baru `CREATE FUNCTION`, bukan `CREATE OR REPLACE` langsung: Postgres
mencocokkan overload dari TIPE parameter, jadi menambah parameter membuat
signature baru yang BERBEDA dari signature lama, dan `CREATE OR REPLACE` saja
akan menambah overload menggantung alih-alih mengganti — jebakan yang sama
persis mengintai tiap kali menambah parameter ke RPC yang sudah punya
pemanggil di produksi.

Avatar yang dipilih tampil KONSISTEN di 3 tempat (pola yang sama seperti warna
tim §29.8): Lobby publik (menggantikan lingkaran inisial), grid slot admin
(menggantikan lingkaran nomor player — nomornya dipertahankan sebagai chip
kecil di pojok avatar, supaya info itu tidak hilang), dan tentu saja picker-nya
sendiri di form publik.

### 29.12 Satu kontak = satu slot per tanggal (migr 141)

`event_registrations_contact_per_date_unique` — index unik parsial pada
`(session_date_id, contact_value) WHERE status <> 'cancelled'`, pola identik
`event_registrations_slot_unique` (migr 136). Keputusan eksplisit user: **satu
kontak (WA/IG) tidak boleh berperan jadi 2 pemain di tanggal yang sama.**

Ini **mengganti** perilaku lama — sebelumnya route publik sengaja
membolehkan satu kontak mengambil sampai 6 slot per tanggal (`MAX_SLOTS_
PER_CONTACT`), didesain untuk pola "kapten tim mendaftarkan semua rekannya
pakai kontaknya sendiri". Pola itu **tidak didukung lagi** — tiap pemain
wajib kontaknya sendiri per tanggal. Alasannya dua: (a) hitungan
"X/12 terisi" per tanggal jadi mencerminkan orang unik, bukan satu kontak
mengembangkan diri jadi banyak "pemain hantu"; (b) manager benar-benar bisa
follow-up tiap pemain lewat kontaknya masing-masing.

Lintas **tanggal** sengaja TIDAK dibatasi (dikonfirmasi eksplisit terpisah
dari aturan di atas) — kontak yang sama boleh ambil slot di beberapa tanggal
kandidat berbeda dalam satu event yang sama, sambil menunggu manager
memutuskan pemenangnya (§29.6).

DB-lah wasit sesungguhnya (menutup race 2 submit hampir bersamaan dari
kontak yang sama); pre-check di route publik cuma UX. 23505 dari index ini
dibedakan dari 23505 index slot lewat substring nama constraint di pesan
error Postgres, karena sekarang ada DUA index unik berbeda yang sama-sama
bisa memicu kode error itu.

### 29.13 Jam & lokasi event + urutan tim dinamis (migr 144)

Dua perubahan pada layar Lobby, sama-sama soal **apa yang dilihat pendaftar
tepat saat dia menimbang mau ambil slot atau tidak.**

**(a) Baris kedua header = jam & lokasi, bukan ulangan judul.**
`event_sessions` dapat tiga kolom: `start_time TIME`, `end_time TIME`,
`location TEXT` (≤120 char). Sebelumnya baris kedua di layar slot mencetak
ulang `session.title` — padahal judul itu persis yang baru saja diklik
pendaftar di layar sebelumnya, jadi nol informasi baru. Yang belum dia tahu
justru **di mana** dan **jam berapa**, dan itu yang menentukan dia jadi ikut
atau tidak.
...
`TIME`, bukan `TEXT`: nilainya datang dari `<input type="time">` dan dirender
klien, jadi formatnya tidak boleh bergantung pada cara owner mengetik.
Perapihannya terpusat di `src/lib/events/schedule.ts` (dipakai bersama Lobby
& Event Manager, jadi tidak ada dua aturan render untuk hal yang sama):
- `formatEventTime` — potong detik dari `"19:00:00"` kiriman Postgres → `"19:00"`
- `formatEventTimeRange` — `"19:00–21:00"`; `end_time` NULL → `"19:00"` saja
  (NULL memang berarti "sampai selesai", bukan data hilang — jangan render
  dash menggantung)
- `formatEventSchedule` — `"GOR Sukapura · 19:00–21:00"`; lokasi didahulukan
  karena penyaringan pertama pendaftar adalah tempat, jamnya pertimbangan kedua

Dua constraint menjaga bentuk datanya: `event_sessions_location_length` dan
`event_sessions_end_time_needs_start` (`end_time` NOT NULL wajib punya
`start_time` — jam selesai sendirian tidak bisa dirender). Form Event Manager
mematikan field jam selesai selama jam mulai kosong dan ikut membersihkannya
saat jam mulai dihapus, jadi constraint itu tidak pernah benar-benar kena.

Ketiga kolom NULL → baris kedua **kembali ke `session.title`** seperti
sebelumnya. Sengaja ada teks cadangan, bukan baris kosong: tanpa itu tinggi
header berubah-ubah antar event dan header terlihat "melompat".

**(b) Kotak tim diurutkan by pendaftaran terakhir, bukan nomor tim.**
Urutannya dihitung di klien (`orderedTeamNumbers` di `EventLobby`), bukan di
DB: tim yang paling baru terisi naik ke atas, tim yang masih kosong turun ke
bawah dengan urutan nomor aslinya. Kuncinya waktu pendaftaran **terakhir** di
tim itu (bukan jumlah pendaftar), jadi tiap slot baru langsung menaikkan
timnya ke puncak.

Alasannya sosial, bukan teknis: yang membuat orang ikut mendaftar adalah
melihat orang lain sudah mendaftar. Dengan urutan nomor tim yang tetap, tim
yang ramai bisa terdampar di bawah lipatan dan pengunjung pertama kali
melihat tiga kotak kosong berturut-turut — persis sinyal sebaliknya.

Konsekuensi di jalur publik: `loadPublicSession` kini ikut memproyeksikan
`event_registrations.created_at` ke `PublicEventSlot`. Aman — itu stempel
waktu slot dikunci, bukan data pribadi; `contact_value` tetap tidak pernah
keluar (§29.4). Urutan hanya berubah saat polling membawa slot baru, dan
`key={teamNumber}` membuat React memindahkan kartu yang sudah ada alih-alih
me-mount ulang, jadi animasi `animate-rise-in` (entry-only) tidak terputar
lagi tiap 8 detik.

---

## 30. Backup Data Per-Bisnis

Jalur bagi pemilik bisnis untuk mengeluarkan seluruh datanya sendiri dari AXION.
Berbeda dari ekspor laporan (§ export PDF/Excel di `src/lib/export.ts`) yang
mengeluarkan *hasil kalkulasi*, backup mengeluarkan *data mentah* — transaksi,
jurnal, akun, kontak, katalog, booking, invoice — dalam bentuk utuh.

Fitur ini **murni baca**. Tidak ada migrasi, tidak menyentuh logic akuntansi,
dan tidak menulis apa pun ke database.

### 30.1 Bentuk keluaran

Dua format dari satu sumber data yang sama:

| Format | Sifat | Untuk |
|--------|-------|-------|
| JSON | Fidelitas penuh — JSONB, relasi, tipe data utuh | Format kanonik; fondasi restore di masa depan |
| Excel (.xlsx) | Lossy — kolom JSONB diringkas jadi teks | Dibaca & diaudit manusia |

Envelope JSON ber-versi (`schema_version`), berisi `counts` per tabel dan daftar
`excluded` — sehingga file backup bisa diverifikasi tanpa membedah isinya, dan
pembacanya tahu persis apa yang sengaja **tidak** ada di dalamnya.

Nama file: `axion-backup-{slug-bisnis}-{YYYY-MM-DD}.{json,xlsx}`

### 30.2 Manifest — aturan yang wajib dipatuhi

`src/lib/backup/manifest.ts` adalah sumber kebenaran tunggal.

> **SETIAP tabel baru di schema `public` wajib didaftarkan** — entah ke
> `BACKUP_MANIFEST` (ikut dibackup) atau ke `BACKUP_EXCLUSIONS` (tidak, dengan
> alasan tercatat). Tidak ada opsi ketiga.

Aturan ini ditegakkan mesin, bukan disiplin: `tests/unit/backupManifest.test.ts`
memegang snapshot seluruh tabel dan **gagal** begitu ada tabel yang belum
diputuskan nasibnya. Tanpa penjaga itu, backup akan membusuk diam-diam tiap kali
skema bertambah — dan baru ketahuan justru saat backup-nya dibutuhkan.

Per 28 Agustus 2026: 36 tabel ikut, 13 dikecualikan (total 49).

### 30.3 Scoping — tiga bentuk

| `scope.kind` | Cara menyaring | Contoh |
|--------------|----------------|--------|
| `root` | `.eq('id', businessId)` | `businesses` |
| `direct` | `.eq('business_id', businessId)` | `transactions`, `accounts` (27 tabel) |
| `via` | `.in(fk, <ID induk>)` | `journal_lines` lewat `transactions` (8 tabel) |

Entri `via` punya `order` lebih besar daripada induknya, dan
`orderedManifest()` menjamin induk selalu diambil lebih dulu.

`profiles` memakai `parentKey` khusus (`user_business_roles.user_id`) dan
**kolomnya dibatasi** ke `id, full_name, avatar_url` — cukup untuk menerjemahkan
`created_by`/`user_id` jadi nama terbaca, tanpa memuat PII lain.

### 30.4 Paginasi & chunking — dua kegagalan senyap

Keduanya menghasilkan backup yang tampak "sukses" tapi tidak lengkap, jadi
keduanya diuji di `tests/unit/backupCollect.test.ts`:

1. **Batas 1000 baris PostgREST.** Semua tabel diambil per halaman via `.range()`.
   `.order('id')` bukan kosmetik — tanpa ORDER BY yang stabil, LIMIT/OFFSET di
   Postgres boleh mengembalikan urutan berbeda tiap halaman, membuat baris
   terlewat atau terhitung dua kali. Semua tabel yang dibackup ber-PK `id`.
2. **Panjang URL pada `.in()`.** Filter PostgREST dikirim lewat query string.
   Bisnis terbesar di produksi punya >3.000 transaksi; daftar UUID sebanyak itu
   tidak muat di satu URL, jadi dipecah `IN_CHUNK_SIZE = 150`.

Logikanya sengaja tinggal di `src/lib/backup/collect.ts`, bukan di route handler,
supaya bisa diuji dengan client palsu tanpa database.

### 30.5 Yang sengaja tidak disertakan

| Alasan | Tabel |
|--------|-------|
| `credentials` | `channel_integrations`, `business_ecommerce_connections`, `telegram_connections`, `google_sheets_connections` |
| `cache` | `financial_summary_cache`, `business_transaction_versions`, `market_data_cache`, `ocr_scan_cache`, `ocr_usage`, `telegram_link_tokens`, `ecommerce_sync_logs` |
| `per-user` | `google_sheets_recent_files` |
| `deferred` | `audit_log` |

**`credentials` yang paling penting.** Tabel-tabel itu menyimpan token integrasi
terenkripsi. Kalau salah satunya ikut ke file yang diunduh user, itu bukan backup
— itu kebocoran kredensial. Tes manifest menguji keempatnya secara eksplisit,
termasuk memastikan alasannya tetap `credentials`.

**`audit_log` ditunda.** Tabel itu tidak punya `business_id`, hanya
(`table_name`, `record_id`). Menyaringnya per-bisnis berarti mencocokkan
`record_id` ke himpunan ID dari ~30 tabel lain: mahal, rawan meleset, dan
ukurannya (17,5rb baris) mendominasi file. Kandidat toggle opt-in tersendiri.

### 30.6 Excel — dua jebakan yang ditangani

- **Nama sheet maks 31 karakter.** Terpanjang saat ini `reconciliation_session_matches`
  (30) — muat, tapi guard truncate + dedupe tetap dipasang agar tabel baru tidak
  diam-diam menghasilkan file rusak.
- **Formula injection.** Nilai teks yang diawali `=`, `+`, `-`, `@`, tab, atau CR
  di-prefix apostrof. Data backup berisi input dari user dan pihak luar (nama
  kontak, isi pesan lead) yang tidak boleh tereksekusi saat file dibuka.

Konversi dikerjakan di browser (`src/lib/backup/toWorkbook.ts`, di-import
dinamis), bukan di server — route cukup mengirim JSON dan fungsi Vercel tetap
ringan.

### 30.7 Akses & lokasi UI

Route `GET /api/backup/[businessId]` memakai `createServerClient()` (RLS tetap
menyala sebagai jaring pengaman lapis kedua — **bukan** admin client) dan
menolak non-manager lewat `canManageBusiness()`. Investor bersifat read-only di
UI, tapi dump penuh tetap bukan haknya.

UI-nya di tab **Data** pada `/businesses/[id]/config` — bukan di `/settings`.
Konvensi repo: yang per-bisnis tinggal di konfigurasi bisnis (seperti
Instagram/WhatsApp), yang per-user tinggal di Pengaturan (Telegram, Google
Sheets). Lihat komentar di `src/components/google/GoogleSheetsCard.tsx`.

### 30.8 Belum dikerjakan

- **Restore.** Keputusan sadar: urutan FK, remap UUID, `created_by` yang menunjuk
  `auth.users` di tenant lain, dan trigger `log_audit_trail()` yang ikut menyala
  saat insert membuatnya jauh lebih berat dari backup. `schema_version` sudah
  disiapkan sebagai fondasinya. Bentuk teraman saat dibangun nanti: pulihkan
  sebagai bisnis **baru** (clone), bukan menimpa yang ada.
- **Backup terjadwal** (cron → Storage/Drive). Infrastruktur cron sudah ada
  (`/api/cron/sync-ical` + `vercel.json`), tinggal nebeng bila diperlukan.
- **Binary lampiran.** Mayoritas lampiran berupa URL eksternal; backup menyimpan
  URL-nya, bukan berkasnya.

### 30.9 File terkait

| File | Peran |
|------|-------|
| `src/lib/backup/manifest.ts` | Registry tabel + scope + urutan |
| `src/lib/backup/collect.ts` | Paginasi, chunking, eksekusi manifest |
| `src/lib/backup/toWorkbook.ts` | Envelope → workbook Excel |
| `src/lib/backup/types.ts` | Tipe envelope + `BACKUP_SCHEMA_VERSION` |
| `app/api/backup/[businessId]/route.ts` | Route handler (auth + gating) |
| `src/lib/api/backup.ts` | Klien browser + pemicu unduhan |
| `src/components/business/BackupCard.tsx` | UI tab Data |
| `tests/unit/backupManifest.test.ts` | Penjaga kelengkapan manifest |
| `tests/unit/backupCollect.test.ts` | Penjaga paginasi & chunking |

---

## 31. Export Transaksi (Import ⇄ Export)

Pasangan dari fitur import yang sudah ada. Sebelumnya data transaksi hanya bisa
**masuk** lewat Excel; sekarang bisa **keluar** juga.

Satu tombol di halaman Transaksi — label **"Import / Export"** — membuka modal
yang sama dengan empat tab: Smart Import, Import Lengkap, Channel CSV, dan
**Export**.

### 31.1 Kolom mengikuti data, bukan template import

Keputusan desain yang penting: kolom ekspor **tidak** meniru template Import
Lengkap.

Template import sengaja disederhanakan supaya orang mudah mengisi data baru —
ia tidak punya kolom kontak, nomor transaksi, mata uang asing, maupun baris
jurnal multi-line. Ekspor punya tugas kebalikannya: **mengeluarkan data yang
benar-benar ada**. Menyempitkan keluaran agar muat ke bentuk template justru
membuang data nyata (kontak yang tertaut ke transaksi, misalnya).

Konsekuensinya: file hasil ekspor **bukan** file yang dirancang untuk di-import
balik apa adanya. Itu pertukaran yang disengaja — kelengkapan didahulukan
daripada round-trip.

Sheet **Transaksi** (21 kolom): No Transaksi, Tanggal, Kategori, Kontak, Tipe
Kontak, Deskripsi, Jumlah, Mata Uang, Jumlah Asli, Kurs, Akun Debit + namanya,
Akun Kredit + namanya, Multi-line, Status, Kanal Penjualan, Rekonsiliasi,
Catatan, Akun (legacy), Dibuat.

Sheet **Jurnal** (hanya dibuat bila ada transaksi multi-line): satu baris per
baris jurnal — No Transaksi, Tanggal, Urutan, Kode & Nama Akun, Debit, Kredit,
Deskripsi Baris.

### 31.2 Multi-line: kolom kosong lebih jujur daripada kolom salah

Transaksi multi-line tidak muat di dua kolom Akun Debit/Kredit. Kolomnya
**dikosongkan**, bukan diisi salah satu baris jurnal — memaksakan satu baris ke
sana menghasilkan file yang terbaca sebagai transaksi yang salah. Kolom
`Multi-line` menandai barisnya, dan detail lengkapnya ada di sheet Jurnal.

### 31.3 Kontak: relasi diutamakan, `name` jadi cadangan

`contact.name` (relasi AR/AP) dipakai lebih dulu; kolom teks bebas `name`
warisan lama jadi cadangan, supaya transaksi lama yang belum ditautkan ke
kontak tidak keluar kosong.

**Kolom "Tipe Kontak" membedakan dua keadaan yang berbeda:**

| Keadaan | Isi sel |
|---------|---------|
| Tertaut ke kontak | tipe aslinya (`customer`, `vendor`, …) |
| Punya `name` tapi tanpa `contact_id` | `(belum ditautkan)` |
| Tidak ada nama sama sekali | kosong |

Per 28 Agustus 2026, **994 dari 1.495 transaksi aktif (66%) belum punya
`contact_id`** — hanya `name` teks bebas. Sel kosong akan terbaca ambigu
("datanya hilang?"), padahal jawabannya "transaksi ini belum ditautkan ke
kontak". Perbedaan itu ditulis eksplisit supaya pembacanya tahu ini soal
penautan, bukan data yang lenyap.

### 31.3a Relasi `contact` sempat tidak pernah di-join

Bug yang ditemukan saat memverifikasi hasil ekspor: **tidak satu pun** query
transaksi di `src/lib/api/transactions.ts` menyertakan embed `contact`, padahal
`Transaction.contact` sudah lama ada di tipe dan dipakai beberapa konsumen.
Akibatnya `tx.contact` selalu `undefined`, dan setiap pembacanya diam-diam
jatuh ke jalur cadangan.

Dampaknya melampaui fitur ekspor:

- `useArApAging.ts` — `contactType` di halaman AR/AP selalu `null`
- `invoiceFromTransaction.ts` — `customerPhone` pada invoice selalu kosong

Keempat query (`getTransactions`, `getTransactionsPaginated`,
`getTransactionsByDateRange`, `getTransactionsForExport`) kini menyertakan
`contact:business_contacts!transactions_contact_id_fkey(*)`.

> Embed PostgREST tidak diperiksa TypeScript maupun build — nama constraint yang
> salah baru gagal saat runtime, dan relasi yang **hilang** tidak gagal sama
> sekali: ia hanya mengembalikan `undefined` diam-diam. Saat menambah konsumen
> `tx.<relasi>`, pastikan query-nya benar-benar meng-embed relasi itu.

### 31.4 Pengambilan data — dua hal yang dijaga

1. **Paginasi eksplisit.** `getTransactionsForExport()` di
   `src/lib/api/transactions.ts` memakai `.range()` berulang, bukan
   `getTransactions()` yang tanpa batas. Response PostgREST terpotong di 1000
   baris; tanpa paginasi, bisnis yang tumbuh besar akan menghasilkan file yang
   tampak sukses padahal tidak lengkap. Urutannya `date` + `id` — `id` sebagai
   pemecah seri supaya dua baris bertanggal sama tidak berpindah halaman.
2. **Pratinjau pakai count murni.** `countTransactionsForExport()` memakai
   `head: true` + `count: 'exact'`. Pratinjau dihitung ulang tiap filter
   berubah; menariknya lewat query lengkap berarti mengunduh ribuan baris
   berikut join akun & jurnal hanya untuk menampilkan satu angka.

Tab Export mengambil datanya sendiri, **bukan** memakai daftar yang sedang
tampil di halaman — yang di layar sudah terpotong paginasi dan filter tab.

### 31.5 Filter & format

Periode (semua waktu / rentang tanggal) dan status (semua / posted / draft).
Jumlah yang akan terunduh ditampilkan sebelum tombol ditekan.

- **Excel (.xlsx)** — dua sheet, format lengkap.
- **CSV** — hanya sheet Transaksi; format satu-tabel tidak bisa membawa sheet
  Jurnal. Diberi BOM UTF-8 agar Excel di Windows membaca karakter non-ASCII
  dengan benar.

Sama seperti backup (§30), nilai teks yang diawali `=`, `+`, `-`, `@`, tab,
atau CR di-prefix apostrof — deskripsi, nama kontak, dan catatan berasal dari
input user.
...
| File | Peran |
|------|-------|
| `src/lib/transactionExport.ts` | Baris & workbook ekspor |
| `src/lib/api/transactions.ts` | `getTransactionsForExport()`, `countTransactionsForExport()` |
| `src/components/transactions/TransactionExportTab.tsx` | UI tab Export |
| `src/components/transactions/TransactionImportModal.tsx` | Modal induk (4 tab) |
| `tests/unit/transactionExport.test.ts` | Penjaga kolom & keamanan sel |

---

## 32. Galeri Lampiran (lintas transaksi)

Lampiran transaksi sebelumnya hanya bisa dilihat satu-per-satu: buka transaksi,
buka detailnya, baru bukti dokumennya terlihat. Galeri ini membalik arahnya —
**semua bukti dokumen satu bisnis dalam satu halaman**, dengan transaksi asalnya
sebagai keterangan, bukan sebaliknya.

Batas lampiran per transaksi juga dinaikkan dari **3 → 5 file** (`MAX_FILES` di
`src/lib/storage/attachments.ts`). Angka itu satu-satunya sumber kebenaran:
kuota, pesan error, dan teks "up to N files" di dropzone semuanya membacanya.
Tidak ada validasi jumlah di server — batasnya murni UI, sama seperti sebelumnya.

### 32.1 Lokasi: menumpang halaman Transaksi, bukan menu sidebar baru

Galeri duduk di `/transactions/attachments` — **sub-route** halaman Transaksi,
bukan entri sidebar. Alasannya: lampiran tidak pernah berdiri sendiri, ia selalu
milik sebuah transaksi. Menaruhnya sebagai menu tingkat atas akan menyiratkan
"arsip dokumen" yang berdiri sendiri, padahal isinya bergantung penuh pada
transaksi.

Tiga pintu masuk, tidak satu pun menambah baris di sidebar:

| Pintu | Di mana | Untuk siapa |
|-------|---------|-------------|
| Tombol "Galeri Lampiran" | Header halaman `/transactions` | Semua role — sengaja **di luar** gate `canManageTransactions`, investor boleh melihat bukti meski tidak boleh mencatat |
| Tautan di blok "Source of Document" | Modal detail transaksi | Saat melihat satu bukti, orang justru teringat mencari yang lain |
| Hasil pencarian ⌘K | `searchablePages` di `app/(dashboard)/layout.tsx` | Tanpa ini fitur praktis tersembunyi |

### 32.2 Dua tab: Gambar vs File

Tab memisahkan lampiran gambar (foto nota, struk, bukti transfer) dari lampiran
file (PDF: faktur, kuitansi, kontrak) — dua cara baca yang berbeda: gambar
dipindai mata sebagai grid thumbnail, PDF dibaca sebagai daftar nama file.

Penentuan jenis ada di `getAttachmentKind()` dan **tidak boleh** bersandar pada
satu field saja:

1. `mime_type` mengandung `pdf` → file. Dicek **pertama**.
2. `mime_type` berawalan `image/` → gambar.
3. Ekstensi nama file (`.jpg/.png/.webp/…`) → gambar. Cadangan untuk lampiran
   lama yang mime-nya kosong.
4. `resource_type` Cloudinary → cadangan terakhir.

Urutannya penting: di data produksi ada **41 lampiran PDF yang tersimpan dengan
`resource_type: 'image'`** (warisan upload sebelum `/auto/upload` dipakai).
Menguji `resource_type` lebih dulu akan melempar puluhan PDF ke tab Gambar dan
merender thumbnail yang tidak akan pernah muncul. Dijaga oleh
`tests/unit/attachmentGallery.test.ts`.

### 32.3 Pengambilan data — ramping, disaring di server, terpaginasi

`getTransactionsWithAttachments()` sengaja **bukan** `getTransactions()`:

- **Proyeksi ramping, tanpa join.** Galeri hanya perlu identitas transaksi
  sebagai keterangan kartu. Menarik akun debit/kredit + baris jurnal berarti
  mengunduh berkali lipat data untuk ditampilkan sebagai satu baris teks.
- **Filter di server** lewat `.or('meta->attachments.not.is.null,meta->attachment.not.is.null')`
  — pola yang sama dipakai `bookings.ts`. Pada data saat ini itu memangkas 1.587
  transaksi menjadi ~200 baris yang benar-benar punya lampiran.
- **Paginasi eksplisit** (500/halaman, urut `date` + `id`). Response PostgREST
  terpotong di 1000 baris; galeri yang diam-diam kehilangan lampiran lama adalah
  jenis kegagalan yang paling sulit disadari.

Format lama (`meta.attachment` tunggal) tetap dibaca sebagai fallback — 56
transaksi masih memakainya.

Urutan kartu mengikuti **tanggal transaksi**, bukan waktu unggah: galeri ini
dibaca sebagai arsip bukti, jadi nota bulan lalu yang baru diunggah hari ini
tetap duduk di bulan lalu.

### 32.4 Biaya signed URL — dua peredam

Setiap lampiran Cloudinary `authenticated` butuh satu permintaan tanda tangan ke
server sebelum bisa dirender. Satu halaman galeri karena itu berpotensi memicu
ratusan request sekaligus. Dua peredam:

1. **Cache signed URL** (`signedUrl.ts`) — signature Cloudinary di route
   `cloudinary-sign` bersifat statis (tanpa `auth_token`/TTL), jadi hasilnya aman
   disimpan selama tab hidup. Yang di-cache adalah *promise*-nya, supaya N kartu
   atas file yang sama hanya memicu satu request; entry dibuang kalau gagal.
   Legacy Supabase Storage **tidak** ikut di-cache — signed URL-nya ber-TTL.
2. **Thumbnail baru di-mount saat mendekati viewport** (IntersectionObserver,
   `rootMargin: 300px`) dan render dibatasi 48 kartu per klik "Muat lebih banyak".

### 32.5 File terkait

| File | Peran |
|------|-------|
| `app/(dashboard)/transactions/attachments/page.tsx` | Halaman galeri: tab, pencarian, grid, kartu |
| `src/hooks/useAttachmentGallery.ts` | Fetch + ratakan jadi daftar per-file, `getAttachmentKind()`, filter tab/pencarian |
| `src/lib/api/transactions.ts` | `getTransactionsWithAttachments()` |
| `src/components/transactions/AttachmentLightbox.tsx` | Penampil layar penuh (panah kiri/kanan menjelajah seluruh daftar terfilter) |
| `src/components/transactions/SignedAttachmentViewers.tsx` | Primitif render ber-signed-URL, dipakai bersama modal detail transaksi |
| `src/lib/storage/attachments.ts` | `MAX_FILES` (5), validasi file |
| `tests/unit/attachmentGallery.test.ts` | Penjaga klasifikasi jenis & batas 5 file |

---

## 33. Site CMS — Konten Halaman Publik (migr 151)

Landing page, metadata SEO, halaman legal, dan indeks blog/market-insights tadinya
hardcode di kode: mengganti satu kalimat butuh commit + deploy. Sekarang ISI-nya
di DB dan disunting platform admin dari `/admin`.

### 33.1 Pola: section slot, bukan page builder

Yang pindah ke DB hanya isi — teks per bahasa, urutan section, flag on/off,
target link, sumber gambar. Layout, animasi, warna, dan komponen interaktif
(`SingleSourceOfTruth`, `OmnichannelSection`, `HealthScoreCalculator`) tetap di
kode.

Alasannya: desain landing page ini bespoke (grid berbeda tiap section +
framer-motion). Kebebasan penuh membuatnya gampang rusak, dan biaya
implementasinya berlipat. Menambah tipe section = tambah entri di
`src/lib/site/types.ts` + renderer-nya.

**Link mengikuti tujuannya.** Menyembunyikan section otomatis menghilangkan menu
navbar dan tombol hero yang menunjuk ke jangkarnya — link yang diklik lalu tidak
terjadi apa-apa lebih membingungkan daripada link yang tidak ada. Aturannya satu
tempat di `src/lib/site/landingNav.ts`, dipakai renderer maupun editor; editor
menampilkan penanda kuning supaya centang "Tampilkan menu ini" tidak terlihat
rusak. Section dianggap absen kalau `visible` mati ATAU key-nya tidak ada di
`sectionOrder`. Link non-anchor (`/blog`) tidak terpengaruh.

**Konsekuensi yang diterima sadar:** kelas pembungkus section diikat ke KEY-nya
(`SECTION_SHELL` di `LandingPageClient.tsx`), bukan ke posisi. Kalau admin menukar
urutan sampai dua section bertinta (omnichannel & health) berdampingan, keduanya
tampil sebagai satu blok abu panjang. Mengikat tinta ke posisi akan mengubah
tampilan yang sekarang, karena urutan default bukan alternasi.

### 33.2 Dua otoritas yang sengaja dipisah

| Fungsi | Cakupan | Dipakai untuk |
|--------|---------|---------------|
| `is_business_manager(bid)` | per-bisnis | data & pengaturan satu bisnis |
| `is_platform_admin()` | seluruh platform | konten situs publik |

Migrasi 072 sengaja mencabut semua policy superadmin global sehingga superadmin
jadi **membership-scoped**: berkuasa di bisnis yang dia ikuti, bukan di seluruh
platform. Konten axionventura.com bukan milik bisnis mana pun, jadi butuh gerbang
tingkat platform tersendiri.

**Jangan gabungkan keduanya.** `is_business_manager` yang ikut memberi akses CMS
akan membuat setiap manager bisnis bisa mengubah landing page.

Gerbangnya aman dipakai karena migrasi 089 memasang trigger
`prevent_default_role_self_promotion` — user terautentikasi tidak bisa menaikkan
`profiles.default_role`-nya sendiri ke `superadmin`.

**Tiga lapisan, bukan satu:**
1. RLS `site_pages` / `site_posts` / `site_page_versions` → `is_platform_admin()`
2. `requirePlatformAdmin()` di setiap route handler
3. Gate Server Component di `app/(dashboard)/admin/layout.tsx` → `notFound()`

Menu sidebar yang disembunyikan lewat `isSuperadmin` cuma kerapian UI, **bukan
penjagaan** — itu keputusan di browser.

### 33.3 Skema (migrasi 151)

| Tabel | Isi |
|-------|-----|
| `site_pages` | Satu baris per halaman. `draft_content` disunting, `published_content` dibaca publik |
| `site_page_versions` | Snapshot tiap publikasi, append-only, untuk rollback |
| `site_posts` | Artikel `/blog` & `/market-insights`, status `draft`/`published` |

`page_key`: `landing`, `seo`, `privacy`, `terms`, `blog_index`, `market_insights`.

Konten disimpan sebagai SATU dokumen JSONB per halaman, bukan satu baris per
section — penyuntingan selalu menyentuh halaman utuh, jadi draft → publish atomik
(satu UPDATE) dan rollback cuma menyalin balik satu nilai.

**Publik TIDAK diberi SELECT sama sekali**, termasuk untuk konten yang sudah
dipublikasikan. Kalau anon boleh SELECT, `draft_content` ikut terbaca lewat REST
Supabase. Jalur baca publik lewat `createAdminClient()` di dalam `unstable_cache`
yang hanya pernah mengeluarkan `published_content` — pola yang sama dipakai
`/api/stats`.

### 33.4 Jaring pengaman: merge di atas default kode

Halaman publik SELALU dirender dari `mergeSiteContent(DEFAULTS, dbContent)`.
Baris DB kosong, key hilang karena skema berkembang, JSONB rusak sebagian, atau
Supabase tak terjangkau → halaman tetap tampil utuh dengan teks kode. Landing page
tidak pernah blank gara-gara CMS.

**Aturan merge: object di-merge rekursif, ARRAY DIGANTI UTUH.**

Array sengaja tidak di-merge per elemen. Kalau ikut di-merge, admin tidak akan
pernah bisa MENGHAPUS item — menghapus item ke-3 dari daftar fitur akan
memunculkan kembali item ke-3 milik default. Aturan ini juga yang membuat
`sectionOrder` benar-benar bisa menyembunyikan section.

Default ada di `src/lib/site/defaults/` — `landing.ts`, `seo.ts`, `privacy.ts`,
`terms.ts`, `collections.ts`. **Jangan mengosongkan nilai di situ untuk "menghapus"
sesuatu dari halaman** — kosongkan lewat `/admin`. Menghapus key di default malah
membuat nilai dari DB ikut diabaikan.

### 33.5 Alur draft → pratinjau → publikasikan

Tiga langkah, bukan "simpan = tayang". Landing page adalah muka publik
axionventura.com; typo yang langsung tayang mahal.

- `PUT /api/admin/site/[pageKey]` — simpan draft, tidak menyentuh yang tayang
- `POST .../publish` — draft → published + snapshot versi + `revalidateTag(tag, 'max')`
- `GET/POST .../versions` — daftar & pulihkan versi **ke draft**, bukan langsung tayang

Publish memvalidasi ULANG, tidak hanya saat simpan: draft bisa berasal dari
dokumen lama yang tersimpan sebelum skema berubah, dan publish adalah satu-satunya
titik yang menentukan apa yang dilihat publik.

Publish juga menormalkan `draft_content` ke hasil merge tervalidasi, supaya badge
"ada perubahan belum dipublikasikan" tidak menyala terus setelah publish.

Pratinjau di route sendiri (`/preview/landing`), **bukan `/?preview=1`**: membaca
searchParams di `app/page.tsx` akan memaksa landing page jadi dynamic rendering
untuk SEMUA pengunjung demi fitur yang dipakai satu orang. `/` tetap statis (○).

### 33.6 Keamanan input

Nilai dari form admin berakhir di atribut `href` halaman publik, jadi form ini
berpotensi jadi jalur **stored XSS** bagi setiap pengunjung. Yang menutupnya:

- `safeHref` (Zod, `validation.ts`) menolak `javascript:`, `data:`, dan
  protocol-relative `//evil.com`. Hanya path relatif, anchor, http(s), mailto.
- `isSafeHref` (`inlineMarkup.ts`) memvalidasi ULANG saat render — dokumen yang
  tersimpan sebelum aturan itu ada tidak pernah lewat validasi tulis.
- Teks panjang memakai subset markup inline (`**tebal**`, `*miring*`, `` `kode` ``,
  `[teks](url)`), **bukan HTML mentah**. Tidak ada `dangerouslySetInnerHTML` di
  jalur konten CMS mana pun.

Batasan yang disengaja: URL berhenti di tanda tutup kurung pertama (sama seperti
markdown), jadi URL ber-tanda kurung tidak didukung. URL yang terpotong pun tetap
gagal saringan, jadi tidak ada yang lolos ke `href`.

### 33.7 Halaman legal: satu bahasa, dan kenapa

`/privacy` & `/terms` **satu bahasa (Indonesia)**, sesuai versi yang selama ini
tayang. Menerjemahkan dokumen yang mengikat secara hukum butuh penerjemah, bukan
field kosong di form admin.

Karena itu `ContentBlock` juga satu bahasa. Kebahasaan ditangani satu tingkat di
atas: artikel menyimpan `body` terpisah per bahasa (`SitePostContent.locales`).
Menempelkan dua bahasa ke setiap blok akan memaksa keduanya punya struktur
paragraf identik — asumsi yang tidak berlaku untuk prosa panjang.

⚠️ **Kedua URL ini diverifikasi Google saat consent screen OAuth dipublish ke
production.** Menghapus bagian tentang data Google, izin `drive.file`, atau
komitmen *Limited Use* bisa membuat verifikasi ditolak dan login Google pengguna
ikut terdampak. Route-nya wajib tetap statis dan tetap `index: true`. Editor di
`/admin` memasang peringatan ini di layar.

### 33.8 Blog: artikel kode & artikel CMS hidup bersama

Artikel yang sudah ada **tetap di kode** (`src/lib/blog/posts.ts` +
`app/blog/<slug>/page.tsx`) karena body-nya JSX bespoke 400+ baris yang sudah
terindeks Google. Menyandikannya ulang jadi blok berarti mengorbankan halaman yang
sudah bekerja demi keseragaman internal.

Indeks `/blog` MENGGABUNGKAN dua sumber lewat `getPostSummaries()`, terurut dari
yang terbaru; bagi pembaca keduanya tidak terlihat bedanya. Sitemap juga memuat
keduanya.

**Jebakan yang ditutup:** route statis `app/blog/<slug>/page.tsx` menang atas
`[slug]` di Next.js, jadi artikel CMS dengan slug yang sama akan tersimpan rapi di
DB tapi **tidak pernah bisa dibuka** — gagal diam-diam. `isCodePostSlug()` menolak
slug itu di route admin.

Artikel memakai satu kolom `status`, bukan draft→publish dua tingkat seperti
halaman: halaman selalu ada di situs dan yang berubah hanya isinya, sedangkan
artikel bisa memang belum layak tayang.

`/market-insights` hanya kepala halamannya yang dikelola CMS. Isinya — kurs, makro,
berita — data hidup dari API eksternal lewat `src/lib/marketData/service.ts`.

### 33.9 Yang SENGAJA tidak dikelola CMS

| Hal | Alasan |
|-----|--------|
| `metadataBase`, `icons`, `alternates.canonical`, `verification` | Terikat domain & berkas `/public`; salah ketik merusak seluruh situs tanpa gejala yang kelihatan saat menyunting |
| Target link `/privacy` & `/terms` di footer | Diverifikasi Google; hanya labelnya yang bisa diubah |
| Angka statistik trust strip | Dihitung dari data platform (`/api/stats`); hanya labelnya |
| Kurasi logo marquee | Sudah ada: toggle `businesses.show_in_logo_slide` per bisnis |
| Layout, animasi, warna | Lihat §33.1 |

### 33.10 File kunci

| File | Peran |
|------|-------|
| `database/migrations/151_site_cms.sql` | `is_platform_admin()`, 3 tabel, RLS, seed baris halaman |
| `src/lib/site/types.ts` | Bentuk semua dokumen konten |
| `src/lib/site/merge.ts` | Jaring pengaman merge (array diganti utuh) |
| `src/lib/site/validation.ts` | Skema Zod ketat + `safeHref` |
| `src/lib/site/server.ts` | Jalur baca publik (`unstable_cache` + admin client) |
| `src/lib/site/posts.ts` | Artikel; penggabungan kode + CMS, `isCodePostSlug` |
| `src/lib/site/inlineMarkup.ts` | Parser subset markup, `isSafeHref` |
| `src/lib/site/cache.ts` | Tag cache CMS (terpisah dari `publicPageCache.ts`) |
| `src/lib/api/server/platformAdmin.ts` | `requirePlatformAdmin()`, `isPlatformAdmin()` |
| `src/components/landing/LandingPageClient.tsx` | Renderer landing page |
| `src/components/legal/ContentBlocks.tsx` | Renderer blok konten |
| `src/hooks/useSiteEditor.ts` | State editor: draft / savedDraft / isDirty / hasUnpublishedChanges |
| `tests/unit/siteCms.test.ts` | 22 test: `parseInlineMarkup` & `mergeSiteContent` |
