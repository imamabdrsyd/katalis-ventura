...
|-----------------|-------------------------|------|
| `bayar_hutang` | Hutang belum lunas (`isPayableTransaction` & belum settled, bukan entry pelunasan, bukan draft) | Pelunasan penuh via `settle_transaction` |
| `cicil_hutang` | Sama seperti di atas | Pelunasan sebagian (input nominal, harus < sisa) |
| `terima_kembali_talangan` | Piutang talangan belum kembali (`isAdvanceReceivableAccount` pada baris debit) | Pelunasan penuh via `settle_transaction` |
...
Aturan fallback: kalau daftarnya kosong (belum ada hutang/talangan outstanding, atau katalog masih kosong), form manual langsung ditampilkan seperti sebelumnya. Tiap picker juga punya escape hatch **"Catat manual"** untuk kasus yang memang tidak ada di daftar.
