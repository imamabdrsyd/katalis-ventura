...
lama. Manager-only (`isManagerRole`). Tiap **unit/kamar/villa** yang bisa dibooking adalah
satu `catalog_items` (`item_type='service'`, `unit='malam'`, `default_price`=harga/malam).

**Migrasi 113 — bookings:** tabel `bookings` (RLS pola `catalog_items`: SELECT semua anggota,
INSERT/UPDATE/DELETE manager). Kolom kunci: `catalog_item_id` (unit), `contact_id`+`guest_name`
(tamu), `transaction_id` (link EARN, UNIQUE), `check_in`/`check_out` DATE, `nights` **GENERATED**
...
`channel` (`manual|airbnb|booking_com|other`), `is_external` (blok impor iCal OTA — read-only,
tak pernah di-EARN), `ical_uid` (dedup impor). Juga menambah `catalog_items.ical_import_url`
(URL feed .ics OTA per unit) dan `businesses.ical_feed_token` (token feed .ics ekspor).

**Wiring revenue — `markBookingPaid()` (`src/lib/api/bookings.ts`).** Booking dibuat manual
oleh owner; saat **ditandai Lunas** (Tunai/QRIS) dirakit **transaksi EARN multi-line langsung
lunas** (`status: 'posted'`) — pola **identik** dengan `useCashier.checkout()`, resolver akun
di-share via `src/lib/accounting/salesCheckout.ts` (`resolveCashAccount` + `resolveDefaultRevenueAccount`):
- **Debit** Kas/Bank sesuai metode (Tunai→`1100`, QRIS→`1200`), total = `total_amount`.
- **Kredit** Pendapatan = `catalog_item.revenue_account_id` (fallback prefer `4100`).
...
- Booking yang sudah lunas: unit/tanggal/harga **dikunci** di UI (cegah desync ledger) — hanya
  status yang boleh berubah.
- **"Tandai Lunas" menyimpan form dulu (Issue #29):** `handleMarkPaid` di `BookingModal`
  ...
  bukan snapshot booking sebelum diedit. Guard tambahan: unit terpilih, tanggal valid, tidak
  bentrok, total > 0.
...
**Proteksi double-booking (2 lapis, migr 115):**
...
- **DB (backstop):** exclusion constraint `bookings_no_double_booking`
  (`EXCLUDE USING gist (catalog_item_id WITH =, daterange(check_in, check_out) WITH &&)`,
  butuh extension `btree_gist`) untuk booking aktif non-eksternal — menutup race dua sesi
  simultan yang lolos cek client. `catalog_item_id` NULL tidak saling match (booking hasil
  backfill tanpa unit tidak dibatasi, konsisten dgn cek client per-unit); blok `is_external`
  dikecualikan karena feed OTA sah beririsan dgn booking langsung (loop ekspor→impor).
  Pelanggaran (SQLSTATE `23P01`) dipetakan ke pesan ramah di `createBooking`/`updateBooking`.

**Unit kamar vs rate plan/add-on — `catalog_items.is_bookable_unit` (migr 115).** Item
katalog bisnis akomodasi bisa berupa kamar/villa ATAU rate plan/add-on (mis. "Cleaning
Service", "Monthly Rent" — data real Hillside Studio) yang tidak boleh dihitung sebagai unit.
Flag eksplisit (bukan klasifikasi keyword nama/satuan), default TRUE; checkbox di form Katalog
muncul hanya untuk sektor akomodasi. Dipakai di: daftar unit `CalendarLauncher` (dropdown
BookingModal + `unitsCount` denominator occupancy/RevPAR), tombol Lead→Booking (`/leads`),
dan unit default backfill (`reconcileStayTransactions`). Catatan: bisnis yang memodelkan
rate plan sebagai beberapa item (weekday/weekend) tetap perlu menyisakan SATU item ber-flag
unit per kamar fisik agar denominator benar.

**Metrik hospitality — `calculateBookingMetrics(bookings, monthCursor, unitsCount)`**
(`src/lib/calculations.ts`). Malam & pendapatan dikliping ke bulan aktif; `cancelled` & `is_external`
diabaikan:
...
- Link dua arah: `booking.transaction_id` ↔ `transaction.meta.booking_id`. Idempoten; melewati
  ...
  bisnis punya persis 1 unit aktif (multi-unit → null, atur manual).
...
> time-slot (tabel `bookings` sudah generik).

---

### 27.6 Kalender Harga (Airbnb-style Pricing Calendar, migr 116)

Kalender booking akomodasi juga berperan sebagai **katalog harga per tanggal** (mode **Harga**
di toolbar, `SegmentedToggle` Booking|Harga).

**Model data:**
- `businesses.calendar_rate_item_id` — item katalog yang DITUNJUK sebagai sumber harga dasar
  (mis. "Weekday Daily Rent"). Harga dasar = `default_price` item tsb.
- `unit_daily_rates` (business_id, catalog_item_id, date, price; UNIQUE per item+tanggal) —
  **layer override**: baris hanya ada untuk tanggal berharga beda; hapus baris = kembali default.
...
**UI (mode Harga):** harga final tampil di tiap sel tanggal (override = indigo semibold);
klik tanggal → klik tanggal lain = pilih rentang → `RateEditorPanel`: input harga + **filter
hari** (mis. hanya Jum+Sab dalam rentang panjang = pola harga weekend tanpa rules engine) +
Terapkan / Reset ke default. Picker item sumber muncul bila belum ditunjuk.

**Konsumen harga (4 titik, satu sumber kebenaran):**
1. **BookingModal (create)** — bila unit terpilih = item sumber, total otomatis = `quoteStay`
   (mis. 2 mlm × 350rb + 1 mlm × 750rb); field harga/malam menampilkan rata-rata; edit manual
   harga = keluar mode otomatis. `price_per_night` disimpan sebagai rata-rata dibulatkan,
   `total_amount` = Σ eksak (yang masuk ledger saat Tandai Lunas).
2. **AI concierge** (`leadAssistant.ts`) — prompt diberi blok "HARGA MENGINAP PER MALAM":
   harga normal + override 60 hari ke depan (diringkas per rentang) + instruksi menjumlah
   per malam; concierge bisa quote total menginap lintas tarif dengan benar.
3. **Halaman publik `/[slug]`** — `app/[slug]/page.tsx` mengonversi override → `pricing_rules`
   ...
   kalender. Tetap dihormati: toggle `show_pricing`; cache halaman `revalidate=300` (≤5 mnt).
4. **Grid kalender** sendiri (mode Booking & Harga sama-sama menampilkan harga).

> Catatan: `business_pricing_rules` (omni-channel lama) tetap ada sebagai fallback untuk bisnis
> tanpa `calendar_rate_item_id`; bila kalender harga aktif, rules turunannya diprepend (menang).
> DEFERRED: aturan weekend persisten (kini via filter hari saat set rentang), musiman berulang,
> length-of-stay discount, harga per unit utk bisnis multi-unit.
