...
tak pernah di-EARN), `ical_uid` (dedup impor). `businesses.ical_feed_token` = token feed .ics
ekspor (per bisnis, filter `?unit=` per unit fisik).

**Wiring revenue — `markBookingPaid()` (`src/lib/api/bookings.ts`).** Booking dibuat manual
oleh owner (di dalam kalender unit ybs — tidak ada lagi dropdown unit di `BookingModal`, unit
ditentukan oleh kalender mana yang sedang dibuka); saat **ditandai Lunas** (Tunai/QRIS) dirakit
**transaksi EARN multi-line langsung lunas** (`status: 'posted'`) — pola **identik** dengan
`useCashier.checkout()`, resolver akun di-share via `src/lib/accounting/salesCheckout.ts`
(`resolveCashAccount` + `resolveDefaultRevenueAccount`):
...
- Booking yang sudah lunas: tanggal/harga **dikunci** di UI (cegah desync ledger) — hanya
...
- **DB (backstop):** exclusion constraint `bookings_no_double_booking`
  ...
  langsung, loop ekspor→impor). **Dikecualikan juga `date_estimated=true`** (migr 117): dua
