...
| `sku` | TEXT NULL | kode item (produk saja); diinput/tampil sejak migr 120, uniqueness per bisnis dicek client-side (index parsial DB sudah ada sejak migr 099); auto-match import TikTok/Tokopedia masih fase berikutnya |
...
- **Form item** (`CatalogItemForm`): field `SKU` (opsional, unik per bisnis —
  dicek client-side lewat `existingSkus`, exclude diri sendiri saat edit) + toggle
...
- **Tampilan kartu/list**: baris kecil di bawah harga — sisa stok
  (`Stok: N unit`, merah "Stok habis" bila 0) + SKU (mono, uppercase).
