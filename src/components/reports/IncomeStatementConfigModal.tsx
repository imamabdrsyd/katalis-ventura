...
export function IncomeStatementConfigModal({ isOpen, onClose, accounts, businessId, onSaved }: Props) {
  ...
  // Nama kedua seksi sengaja diambil dari namespace laporan (`incomeStatement`)
  // supaya modal dan laporan tidak menyebut hal yang sama dengan istilah berbeda.
  const cogsName = t.incomeStatement.costOfRevenue;
  const opexName = t.incomeStatement.opex;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(t.incomeStatementConfig.saveFailed);
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-2">
          ...
          {overridden && (
            <span
              title={t.incomeStatementConfig.overrideTooltip}
            ...
            >
              {t.incomeStatementConfig.override}
            </span>
          )}
        </div>
      </button>
  ...
  const modalContent = (
    ...
    >
      <div
        className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-5xl max-h-modal overflow-hidden flex flex-col transition-all duration-200 ease-out ${isVisible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-2'}`}
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100">
              {t.incomeStatementConfig.title}
            </h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {t.incomeStatementConfig.subtitle.replace('{cogs}', cogsName).replace('{opex}', opexName)}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 min-h-0 overflow-hidden grid grid-cols-[1fr_1fr_1fr] gap-3 p-5">
          ...
          <div className="flex flex-col min-h-0 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50/50 dark:bg-gray-900/30">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {t.incomeStatementConfig.accountCount.replace('{n}', String(cogsAccounts.length))}
              </p>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {cogsAccounts.length === 0 ? (
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-6">
                  {t.incomeStatementConfig.noAccounts}
                </p>
              ) : (
          ...
          <div className="flex flex-col min-h-0 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100 uppercase">
                {t.incomeStatementConfig.detailTitle}
              </h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {t.incomeStatementConfig.detailHint}
              </p>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              {selected ? (
                <div className="space-y-3">
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t.incomeStatementConfig.accountCode}</p>
                    <p className="text-sm font-mono text-gray-800 dark:text-gray-100">
                      {selected.account_code}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t.incomeStatementConfig.accountName}</p>
                    <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
                      {selected.account_name}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t.incomeStatementConfig.defaultClassification}</p>
                    <p className="text-sm text-gray-700 dark:text-gray-200">
                      {defaultSection(selected) === 'cost_of_revenue' ? cogsName : opexName}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t.incomeStatementConfig.currentClassification}</p>
                    <p className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">
                      {assignments[selected.id] === 'cost_of_revenue' ? cogsName : opexName}
                    </p>
                  </div>
                  {selected.description && (
                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{t.incomeStatementConfig.description}</p>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-6">
                  {t.incomeStatementConfig.pickAccount}
                </p>
              )}
            </div>
            ...
            {selected && (
              <div className="border-t border-gray-200 dark:border-gray-700 p-3 flex items-center justify-center gap-2">
                <button
                  ...
                  title={t.incomeStatementConfig.moveToCogs.replace('{section}', cogsName)}
                ...
                <button
                  ...
                  title={t.incomeStatementConfig.moveToOpex.replace('{section}', opexName)}
                  className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-300 dark:hover:border-indigo-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
                  OpEx
                ...
                {isOverridden(selected) && (
                  <button
                    ...
                    title={t.incomeStatementConfig.resetDefault}
          ...
          <div className="flex flex-col min-h-0 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50/50 dark:bg-gray-900/30">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {t.incomeStatementConfig.accountCount.replace('{n}', String(opexAccounts.length))}
              </p>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-700 shrink-0">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {hasChanges ? t.incomeStatementConfig.unsaved : t.incomeStatementConfig.noChanges}
          </p>
