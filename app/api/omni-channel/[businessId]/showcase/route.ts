...
/**
 * POST /api/omni-channel/[businessId]/showcase
 * Terima { url, public_id } dari client setelah upload ke Cloudinary,
 * simpan ke showcase_images di DB.
 */
export async function POST(
  ...
  const { data: existing } = await supabase
    ...
    .select('id, showcase_images')
    .eq('business_id', businessId)
    .maybeSingle();

  if (!existing) {
    return NextResponse.json(
      { error: 'Simpan halaman publik terlebih dahulu sebelum upload showcase' },
      { status: 400 }
    );
  }

  const current: ShowcaseImage[] = Array.isArray(existing.showcase_images) ? existing.showcase_images : [];
  if (current.length >= MAX_SHOWCASE) {
    return NextResponse.json({ error: `Maksimal ${MAX_SHOWCASE} gambar per bisnis` }, { status: 400 });
  }

  const newImage: ShowcaseImage = { path: public_id, url, sort_order: current.length };
  const next = [...current, newImage];

  const { error: updateError } = await supabase
    .from('business_omni_channels')
    .update({ showcase_images: next, updated_by: user.id })
    .eq('id', existing.id);
  ...
  return NextResponse.json({ image: newImage, showcase: next });
}

/**
 * DELETE /api/omni-channel/[businessId]/showcase?path=<cloudinary_public_id>
 */
export async function DELETE(
  ...
  return NextResponse.json({ showcase: next });
}

/**
 * PATCH /api/omni-channel/[businessId]/showcase
 * Reorder gambar. Body: { paths: string[] }
 */
export async function PATCH(
  ...
  const next: ShowcaseImage[] = paths
    .map((p, i) => {
