-- Migration 107: Fix legacy partial settlement outstanding calculation
--
-- Konteks:
--   Transaksi lama (legacy) yang dibayar sebagian (partial settlement) sebelum
--   migration `073_atomic_settlement` hanya memiliki daftar ID pelunasan di
--   `meta.partial_settlements`, tanpa adanya field `meta.remaining_amount`.
--   Akibatnya, fungsi `settle_transaction` gagal menemukan `remaining_amount` 
--   dan menganggap sisa tagihan adalah full amount, yang berakibat pada
--   nominal pelunasan yang overstate (terlalu besar) saat dilakukan Full Settlement.
--
-- Perbaikan:
--   1. Data Migration (Backfill): Menghitung dan menyimpan `remaining_amount` 
--      untuk transaksi yang memiliki `partial_settlements` namun tidak memiliki 
--      `remaining_amount`.
--   2. RPC Update (`settle_transaction`): Menambahkan fallback di dalam perhitungan
--      `v_outstanding` untuk berjaga-jaga jika `remaining_amount` hilang, sistem 
--      akan mengurangi total partial paid dari `v_outstanding`.
--

-- ==============================================================================
-- 1. BACKFILL DATA (Data Migration)
-- ==============================================================================

WITH partials AS (
  SELECT
    t.id AS original_id,
    SUM(p.amount) AS total_partial_paid
  FROM transactions t,
  ...
  WHERE t.meta ? 'partial_settlements'
    AND NOT (t.meta ? 'remaining_amount')
...
SET meta = jsonb_set(
  ...
  '{remaining_amount}',
  to_jsonb(
    -- Jika is_multi_line, gunakan fallback nilai kotor dikurangi partials
    -- (ideal use case multi-line sudah dicover migration >= 100,
    -- ini hanya untuk legacy)
    t.amount - p.total_partial_paid
  )
)
FROM partials p
WHERE t.id = p.original_id;


-- ==============================================================================
-- 2. UPDATE RPC `settle_transaction`
-- ==============================================================================
...
SELECT 'Migration 107 complete - fix legacy partial settlement outstanding' as status;
